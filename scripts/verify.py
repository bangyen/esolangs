...
# Git runs this hook with its.
# Python block-buffers our own.
# same pipe and write to it.
# is that `scope:`, the.
# only when the run ends, after.
# buffering puts them in front.
...
# Steps a default run leaves to.
# they save at push time, and.
# still run under --full, under.
FULL_ONLY = frozenset(
    {
        # Re-deriving the table costs.
        # rarely move; CI's lint job.
        "ztoalc anchor table is reproducible",
    }
)

# Named once: the step table,.
# to this step, and a typo in.
LINE_STEP = "extra/line suites (uv)"

# How often the long step.
# the wait never looks stalled,.
# handful of lines.
HEARTBEAT_SECONDS = 20.0

# Not a STEPS entry: it reads.
# `pytest` is LONG_STEP --.
# short steps go by.
# file, so the gate is run by.
...
# Which paths each step.
# not touch cannot have been.
# A step absent from this table.
# matter or it guards the whole.
STEP_SCOPE: dict[str, tuple[str, ...]] = {
    "bandit": ("src/",),
    LINE_STEP: ("extra/line/",),
    # The check re-derives the.
    # file, importing nothing from.
    # can break it are the.
    # `interpreters/` instead both.
    # and missed a hand-edit of the.
    "ztoalc anchor table is reproducible": (
...
STEPS = [
    ...
    # The only mypy run.
    # env lacks the scripts'.
    # this run covers src/ and.
    ("mypy (src + scripts)", [*PY, "-m", "mypy"]),
    # `--cov-report=` writes no.
    # which the touched-file gate.
    # .
    # `--cov-branch` is free on.
    # 3.3x before it, where.
    # coverage falls back to the.
    # silently rather than.
    # the interpreter before the.
    # `no-sysmon` CoverageWarning.
    # ``docs/verification_tooling.md.
    ("pytest", [*PY, "-m", "pytest", "-q", "--cov", "--cov-branch", "--cov-report="]),
    ("bandit", ["uv", "run", "--with", "bandit", "bandit", "-r", "src", "-q"]),
    (
        # These also run under the.
        # under `--isolated.
        # green run proves the subtree.
        # The in-project run cannot.
        LINE_STEP,
        [
            "uv",
            "run",
    ...
    (
        # pylint's R0801 reports.
        # copy-pasted helpers like the.
        # tokenizer.
    ...
    # Twelve named lemmas behind.
    # one finite fact the.
    # it is a gate rather than the.
    # has to be (`just apa-proof`,.
    (
        "arrowqueue lemmas",
        [*PY, "scripts/arrowqueue_lemmas.py"],
    ),
    # Sieves every ZTOALC start.
    # capacity is not the 395 that.
    # cite.
    # silent staleness.
    (
        "ztoalc slot record",
    ...
    # The contract exceptions.py.
    # raw Python error to its.
    # this branch touched, which is.
...
# What a scoped pytest run.
# ``None``: ``_scoped_cmd``.
# the *opposite* of.
WHOLE_SUITE = "whole-suite"

# pyproject's ``python_files``.
# says is collectable but.
...
def _pytest_scope(changed: list[str]) -> list[str] | str:
    ...
    for f in changed:
        if f.startswith("tests/"):
            # A conftest configures every.
            # fixture data some unknown.
            # not collect is a helper.
            # Scoped to itself each.
            # non-zero for, so all three.
        ...
        if f.startswith("src/esolangs/interpreters/") and f.endswith(".py"):
            ...
            continue
        if f.startswith("src/"):
            return WHOLE_SUITE  # non-interpreter source: not.
        if f.startswith("scripts/") and f.endswith(".py"):
            # The scripts have unit tests.
            # follow the interpreter naming.
...
def _scoped_cmd(name: str, cmd: list[str], changed: list[str]) -> list[str] | None:
    ...
    if name == "pytest":
        ...
        if paths == WHOLE_SUITE:
            return cmd  # not localisable: run every.
        if not paths:
            return None  # nothing the Python tests.
        return [*cmd, *paths]
    return cmd
...
# The step that mutates the.
# whitespace hooks rewrite.
# to wait for it -- running it.
# the read.
...
# overlap work -- it.
# shadow is only free for steps.
# .
# Measured on a 10-core laptop.
# step inside the old.
# bandit 2.7s vs 11.2s, the.
# (`--all`) 35.6s vs 85.4s.
# contending.
...
# : Workers for the leak sweep.
# : default is a deliberate 2,.
# : heavy steps run alone with.
# : spare" case its comment.
# : `--all`, and past the 8.
# : same shape pyproject.
#: keeps their value.
LEAKSWEEP_JOBS = "6"
...
def _run_steps(
    ...
    *,
    ...
    def run_serial(name: str, cmd: list[str], step_env: dict[str, str]) -> None:
        start = time.time()
        # Only the captured branch has.
        # already wrote it straight to.
        captured: str | None = None
        if stream:
            returncode = subprocess.run(cmd, env=step_env).returncode
        else:
            # Captured, but not in one.
            # nothing until it ends, and.
            # minute.
    ...
    proc = None
    long_start = 0.0
    # Nothing to fill the shadow.
    # a single-step run (`just.
...
def main() -> int:
    ...
    # Probe PY rather than the.
    # by a different python than.
    # `uv run --with pylint python.
    # at .venv), and it is PY that.
    have_pylint = (
        subprocess.run(
            [*PY, "-c", "import pylint"],
    ...
    # Decide every step first, then.
    # lookups, tool probes) while.
    # the two apart lets the runner.
    # without the skip logic having.
    runnable: list[tuple[str, list[str], dict[str, str]]] = []
    for name, cmd in STEPS:
        ...
        if unaffected is not None:
            ...
            if narrowed is None:
                ...
                continue
            cmd = narrowed
        # The `slow` marker covers the.
        # whose cost is seconds each.
        # coverage, because CI's `test`.
        # every marked test still runs.
        # --full rather than on scoping.
        # everything -- a tooling.
        # not pay for them.
        # .
        # The extra/line suites carry.
        # (the eight-level nesting.
        # are 10.4s of that step's.
        # unfiltered on every push, so.
        # coverage either.
        step_env = env
        if name == LEAK_STEP and "LEAKSWEEP_JOBS" not in env:
            step_env = dict(step_env, LEAKSWEEP_JOBS=LEAKSWEEP_JOBS)
        if name == "pre-commit":
            # Skip the config's mypy hook:.
            # src/ *and* scripts/ from the.
            # pyproject config, so the hook.
            # pays for its own isolated env.
            # skips it; CI runs `pre-commit.
            # (ci.yml:28), so the hook.
            step_env = dict(step_env, SKIP="mypy")
        if only is None and not full:
            if name == "pytest":
                cmd = [*cmd, "-m", "not slow"]
            elif name == LINE_STEP:
                # Not argv: the command ends in.
                # --isolated`, so an appended.
                # argument and be read by uv's.
                # rest of the step's own.
                # by pytest itself wherever it.
    ...
    # The gate speaks only for the.
    # run deselects the `slow`.
    # would read as uncovered;.
    # fail on evidence it does not.
    # .
    # The deselection has two.
    # appends `-m "not slow"`.
    # PYTEST_ADDOPTS and passes.
    # pytest still reads the env.
    # alone would leave that path.
    # false failure --partial.
    gate: tuple[str, list[str], dict[str, str]] | None = None
    ...
    if timings:
        ...
        # Two totals, because they.
        # started running alongside the.
        # done, the wall is how long.
