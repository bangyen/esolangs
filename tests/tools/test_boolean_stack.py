r"""Unit tests for the stack-based boolean generators."""
...
def _grapheme_counted_side(table: str, n: int) -> str:
    r"""The build the retired row-count rule would have chosen."""
...
class TestGrapheme:
    ...
    def test_shorter_side_wins_not_the_sparser_one(self) -> None:
        r"""Row count is a proxy for length, and it picks wrong."""
        improved = 0
    ...
    def test_balanced_table_takes_the_cheaper_seed(self) -> None:
        r"""The old ``<=`` tie went to the 7-character seed every time."""
    ...
    def test_the_program_is_only_grapheme_commands(self) -> None:
        r"""Only the letters Grapheme reads as commands are emitted."""
...
class TestForth:
    def test_program_structure(self) -> None:
        r"""The program defines one function per surviving node, reading n bits."""
    ...
    def test_constant_subtrees_fold(self) -> None:
        r"""A constant subtree answers in place and drops its descendants."""
    ...
    def test_folded_subtree_leaves_no_orphans(self) -> None:
        r"""Folding drops the whole subtree, not just the two children."""
    ...
    def test_reordering_only_shrinks(self) -> None:
        r"""No table comes out longer than the stack-ordered program."""
    ...
    def test_rotations_are_interleaved_with_the_reads(self) -> None:
        r"""Weaving the rotations into the reads reaches more arrangements."""
    ...
    def test_sinking_a_bit_deeper_than_the_stack_is_skipped(self) -> None:
        r"""A sink needs values below it, so early reads have fewer choices."""
    ...
    def test_an_unreachable_order_returns_empty_rather_than_building(self) -> None:
        r"""An order the ops cannot stack is declined, not approximated."""
    ...
    def test_const_large(self) -> None:
        r"""Constants above 225 need multiple base-15 digits."""
        from esolangs.tools.boolean.stack import _forth_const
    ...
    def test_const_is_base_fifteen(self) -> None:
        r"""Digits are ``0-E`` and the radix is 15, not 16."""
        from esolangs.tools.boolean.stack import _forth_const
    ...
    def test_length_formula_matches_the_built_program(self) -> None:
        r"""The closed-form score equals the built length for every order."""
        import random
    ...
    def test_the_program_is_only_forth_commands(self) -> None:
        r"""Only the characters Forþ reads are emitted."""
...
class TestModulous:
    ...
    def test_structure(self) -> None:
        r"""A one-input function reads one input then branches on it."""
        assert boolean.modulous("10").startswith("[INP INT]")
        assert "[JMP F 2 IF 0]" in boolean.modulous("10")

    def test_constant_subtrees_fold(self) -> None:
        r"""A constant slice pushes its answer instead of branching further."""
        leaves = "[PRT INT]"
...
class TestBfstack:
    ...
    def test_encode_decode_structure(self) -> None:
        r"""The program encodes the inputs then tests the zero rows."""
    ...
    def test_the_program_is_only_bfstack_commands(self) -> None:
        r"""No character outside the eight commands is emitted."""
        for table in ("10", "0110", "0001", "11111110"):
...
class TestUnsquare:
    def test_program_shape(self) -> None:
        r"""The program reads n inputs and prints once."""
    ...
    def test_past_the_cap_only_the_natural_order_is_built(self) -> None:
        r"""Above ``_ORDER_SEARCH_MAX`` the arrangement search is skipped."""
    ...
    def test_reordering_never_grows_a_program(self) -> None:
        r"""Choosing the stack arrangement can only shrink the program."""
        from esolangs.tools.boolean.stack import (
    ...
    def test_sinks_are_interleaved_with_the_reads(self) -> None:
        r"""Weaving the sinks into the reads is what reaches the arrangements."""
    ...
    def test_two_place_sink_needs_the_leading_swap(self) -> None:
        r"""Sinking two places is ``SASP``; ``ASP`` alone does something else."""
    ...
    def test_decision_tree(self) -> None:
        r"""Each internal node branches on a bit with the flip primitive."""
        program = boolean.unsquare("0110")
    ...
    def test_the_program_is_only_unsquare_commands(self) -> None:
        r"""Only the characters Unsquare reads are emitted."""
        for table in ("10", "0110", "0001", "11111110"):
            assert set(boolean.unsquare(table)) <= set("+-<>AIOPSiox"), table


class TestGraphemeKeysAvoidItsOwnAlphabet:
    r"""Two variable keys collided with the language, and only one raised."""

    @staticmethod
    def _one_minterm(n: int) -> str:
        r"""A table whose single 1 makes every one of its ``n`` inputs matter."""
        return "1" + "0" * (2**n - 1)

    def test_no_key_letter_is_the_int_mode_delimiter(self) -> None:
        r"""``F`` framed the key, so it could not also be the digit inside it."""
        assert "F" not in stack._GRAPHEME_KEY_LETTERS  # noqa: SLF001

    def test_no_key_letter_aliases_the_constant(self) -> None:
        r"""The normalizing 65 owns key 90, and a slot must not store over it."""
        keys = [
    ...
    def test_a_table_using_every_input_still_computes(self, n: int) -> None:
        r"""Six essential inputs reached slot 5, and slot 5's key was ``FFF``."""
    ...
    def test_parity_at_six_inputs_computes(self) -> None:
        r"""Parity is the table with nothing to fold, so all six slots are live."""
    ...
    def test_the_constant_alias_returned_a_wrong_answer(
        ...
        r"""The positive control, and the reason ``I`` is filtered out."""
        monkeypatch.setattr(
            stack,
