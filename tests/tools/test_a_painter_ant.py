...
box, and the Boolean answer is the origin's colour after a whole cycle
(white is one, black is zero).  The tests read the origin colour from a
small semantic grid model, which is the only way to observe it (the box
output does not carry coordinates).  Every instantiated program is also run
through the real interpreter to confirm it is a fixed point: the box is
identical for any limit that is a whole number of cycles.
"""
