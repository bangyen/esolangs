...
class TestBrainIf:
    ...
    def test_every_j_split_computes_the_table(self) -> None:
        """All n+1 reachable orders are correct, not just the one dispatched."""
        from esolangs.tools.boolean.tape import _brainif_jsplit, _brainif_node_read

        for i in range(256):
            table = format(i, "08b")
            builds = [_brainif_node_read(table)]
            builds += [_brainif_jsplit(table, j) for j in range(1, 4)]
            for program in builds:
                for combo in range(8):
                    bits = [(combo >> (2 - k)) & 1 for k in range(3)]
                    got = run_brainif(program, [str(b) for b in bits])
                    assert got == table[combo], f"{table} {bits}"

    def test_reordering_only_shrinks(self) -> None:
        """No table comes out longer than the node-read build."""
        from esolangs.tools.boolean.tape import _brainif_node_read

        for i in range(256):
            table = format(i, "08b")
            got = len(boolean.brainif(table).splitlines())
            assert got <= len(_brainif_node_read(table).splitlines())

    def test_a_guarded_pair_double_fires_over_written_cells(self) -> None:
        """The law the whole construction rests on.

        Every BrainIf line is gated on an exact cell value, and the two lines
        of a guarded move test *different* cells -- the second runs after the
        first has moved.  Over written cells that means one intended step can
        move two, so a pair is sound only when the destination is still zero.
        Pinned as a characterization: an "optimizer" that shortened a walk to
        a bare pair would break every hoisted build.
        """
        program = "\n".join(
            [
                "if 0 input",
                ...
                "if 49 move right",
                "if 0 input",
                "if 48 move left",  # one intended step ...
                "if 49 move left",  # ... but this tests the cell just entered
                "if 48 output",
                "if 49 output",
            ]
        )
        # neighbouring digits differ: the pair fires twice and lands on cell 0
        assert run_brainif(program, ["1", "1", "0"]) == "1"
        # neighbouring digits agree: the second guard misses, one step taken
        assert run_brainif(program, ["1", "0", "0"]) == "0"

    def test_normalize_then_move_steps_exactly_once(self) -> None:
        """The 2-line idiom that makes a dead cell crossable.

        ``if 48 increment`` merges both digits onto 49, so the following
        ``if 49 move left`` fires exactly once whatever the cell held.  It is
        destructive, which is why it is only ever emitted over cells the tree
        has already tested -- and never over cell 0, which holds the answer.
        """
        program = "\n".join(
            [
                "if 0 input",
                ...
                "if 49 move right",
                "if 0 input",
                "if 48 increment",  # normalize, then
                "if 49 move left",  # exactly one step
                "if 48 output",
                "if 49 output",
            ]
        )
        for bits in ("110", "100", "111", "010", "001", "000", "011", "101"):
            # lands on cell 1 every time, so it prints that cell's own digit
            assert run_brainif(program, list(bits)) == bits[1]

    def test_constant_tables_keep_the_answer_byte(self) -> None:
        """A folded table never normalizes cell 0.

        The walk home crosses dead cells with the destructive idiom, but cell
        0 holds the answer byte and has to stay 48 until the leaf increments
        it.  A constant table folds at the root, so its whole program is that
        walk -- which is what makes it the case that catches the mistake.
        """
        for table in ("0" * 8, "1" * 8):
            program = boolean.brainif(table)
            for combo in range(8):
                bits = [(combo >> (2 - k)) & 1 for k in range(3)]
                assert run_brainif(program, [str(b) for b in bits]) == table[0]
