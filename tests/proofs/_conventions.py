...
one row per embedding generator with a measured convention still open --
no spaces, uniform -- with a verdict per column.  The other three (single
embed, constant width, slot order) are the template object's own shape and
are refused by its constructor, so they have no column.  Rows leave as
they close, so the table is exactly the open set, and a generator absent
from it is claimed to hold both.
...
class ConventionRow:
    ...
    def verdicts(self) -> tuple[str, str]:
        """The two cells, in column order."""
        return (self.no_spaces, self.uniform)
