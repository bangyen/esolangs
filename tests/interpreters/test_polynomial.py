r"""Unit tests for Polynomial interpreter."""
...
class TestPolynomialHelperFunctions:
    r"""Test helper functions for polynomial processing."""

    def test_prime_function(self) -> None:
        r"""Test prime number detection."""
    ...
    def test_factor_skips_non_instruction_quadratic(self) -> None:
        r"""A quadratic factor whose q is negative encodes no instruction."""
    ...
    def test_factor_skips_a_cubic_factor(self) -> None:
        r"""Only degree 1 and 2 factors encode instructions; higher ones do not."""
        from esolangs.interpreters.register_based.polynomial import _factor_roots
    ...
    def test_brackets_search_backwards_to_the_first_instruction(self) -> None:
        r"""A closer's partner may be instruction 0, which is in range."""
    ...
    def test_brackets_reject_an_unmatched_partner_at_either_end(self) -> None:
        r"""Running off the left end and the right end both raise."""
        for code in ([[2]], [[1]]):
...
class TestPolynomialValidation:
    r"""Test polynomial input validation."""

    def test_empty_program_validation(self) -> None:
        r"""Test that empty program is handled correctly."""
    ...
    def test_invalid_format_validation(self) -> None:
        r"""Test that invalid format raises ValueError."""
        from esolangs.interpreters.register_based.polynomial import run
...
class TestPolynomialExecution:
    r"""Test that valid polynomial programs actually execute."""

    def test_output_instruction(self) -> None:
        r"""A root of 2i encodes an output instruction (reg starts at 0)."""
        buffer = io.StringIO()
    ...
    def test_arithmetic_then_output(self) -> None:
        r"""Roots encoding reg += 65 followed by output produce 'A'."""
    ...
    def test_division_keeps_register_integer(self) -> None:
        r"""Reg /= a is integer division: 65 // 5 = 13, output as a char."""
        program = (
    ...
    def test_run_without_spaces(self) -> None:
        r"""The no-space form produced by run's sanitizer still executes."""
        buffer = io.StringIO()
        with redirect_stdout(buffer):
            run("f(x)=x^2+4", io=IO())
        assert buffer.getvalue() == "\x00"
    ...
    def test_control_flow_roots(self) -> None:
        r"""Real roots 2 and 4 encode an if-statement pair (reg is 0)."""
        buffer = io.StringIO()
        with redirect_stdout(buffer):
    ...
    def test_if_enters_when_condition_met(self) -> None:
        r"""If reg==0 { output } executes the body when reg is 0."""
        buffer = io.StringIO()
        with redirect_stdout(buffer):
    ...
    def test_if_skipped_when_condition_not_met(self) -> None:
        r"""The spec example: if reg>0 { output } skips the body when reg is 0."""
        buffer = io.StringIO()
        with redirect_stdout(buffer):
    ...
    def test_while_loop(self) -> None:
        r"""Add 3, while reg>0 { reg-=1 }, output decrements three times."""
        program = (
    ...
    def test_input_instruction(self) -> None:
        r"""A root of 4i encodes an input instruction (value stored in reg)."""
    ...
    def test_input_of_nul_reads_as_minus_one(self) -> None:
        r"""``ord(val[0]) or -1``: a NUL byte reads as 0, which the ``or``."""
        import unittest.mock
...
class TestPeelPrimePowerRoots:
    r"""The prime-power peel is a head start and must never change the."""

    @staticmethod
    def _reference(coefficients: tuple[int, ...]) -> list[_Root]:
        r"""Recover roots the way the interpreter did before the peel."""
        import math
    ...
    def test_roots_match_factoring_alone(self, description: str) -> None:
        r"""Every shape the peel cannot enumerate still comes back."""
        import sympy as sp
    ...
    def test_peel_leaves_what_it_cannot_take(self) -> None:
        r"""A root outside the enumeration stays in the remainder."""
        import sympy as sp
    ...
    def test_peeled_roots_are_exact_factors(self) -> None:
        r"""Anything peeled divides the polynomial with no remainder."""
        import sympy as sp
...
class TestPeelQuadraticsFallback:
    r"""When the field factorization fails there is nothing to peel."""

    def test_a_refused_factorization_returns_the_input(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        r"""A ``PolynomialError`` from sympy leaves the coefficients alone."""
        import sympy as sp
...
class TestDivideQuadratic:
    r"""Exact division by ``(x - real)**2 + square``, and its refusals."""

    def test_a_list_too_short_to_divide_is_refused(self) -> None:
        r"""Below three coefficients there is no room for the quotient."""
...
class TestPeelInstructionQuadratics:
    r"""The quadratic peel is a head start too, under the same rules."""
    ...
    def test_modulus_admits_every_encodable_square(self) -> None:
        r"""``_PEEL_MODULUS`` must be ``1 (mod 4)``, or the peel finds nothing."""
        import sympy as sp
    ...
    def test_nothing_skipped_is_lost(self, description: str) -> None:
        r"""What the peel cannot take stays in the remainder, exactly."""
        import sympy as sp
    ...
    def test_divide_quadratic_refuses_a_non_divisor(self) -> None:
        r"""The exact-division gate is what makes an accepted pair sound."""
        import sympy as sp
...
class TestNttRecovery:
    r"""The large-degree path: roots from exhaustive field evaluation."""

    @staticmethod
    def _program(pairs: list[tuple[int, int]], reals: list[int]) -> list[int]:
        r"""Integer coefficients of ``prod (x-a)^2+q * prod (x-r)``."""
        coefficients = [1]
        for real, square in pairs:
    ...
    def test_ntt_field_constants(self) -> None:
        r"""Each field is ``(c * 2**k + 1, c, k, g)``, prime, ``1 (mod 4)``."""
        import sympy as sp
    ...
    def test_roots_mod_is_the_exact_root_set(self) -> None:
        r"""Planted roots come back exactly -- no more, no fewer."""
        from esolangs.interpreters.register_based.polynomial import (
            _NTT_FIELDS,
            _roots_mod,
        )
    ...
    def test_zero_is_reported_when_x_divides(self) -> None:
        r"""The one point outside the multiplicative group is still seen."""
        from esolangs.interpreters.register_based.polynomial import (
            _NTT_FIELDS,
            _roots_mod,
        )
    ...
    def test_iter_bits_matches_bit_positions(self) -> None:
        r"""The byte-scan extraction is exactly ``bin()``'s set bits."""
    ...
    def test_a_large_program_is_recovered_exactly(self) -> None:
        r"""Past the threshold, every planted factor comes back and nothing."""
        import sympy as sp
    ...
    def test_what_the_large_path_cannot_take_reaches_factor_list(self) -> None:
        r"""The fallback stays wired on the NTT path."""
        import math
...
class TestPolynomialHighPrecisionRoots:
    r"""Wide codepoint deltas and pathological root spreads are recovered."""

    def _decode(self, text: str) -> str:
        r"""Run the fixture program for ``text`` and return what it printed."""
        buffer = io.StringIO()
        with redirect_stdout(buffer):
    ...
    def test_wide_codepoint_deltas_round_trip(self, text: str) -> None:
        r"""ASCII followed by CJK/emoji spans several orders of magnitude."""
        assert self._decode(text) == text

    def test_repeated_wide_deltas_round_trip(self) -> None:
        r"""The same wide delta repeated (a pathological root spread for any."""
        text = "aあbいcう" * 2
        assert self._decode(text) == text

    def test_single_corrupted_delta_round_trip(self) -> None:
        r"""A mixed program where float64 silently corrupted one delta (19977."""
        text = "aWg{<$中Z一t"
        assert self._decode(text) == text

    def test_a_real_root_past_float64_still_decodes(self) -> None:
        r"""``251**8`` exceeds 2**53, where ``complex`` would round it."""
        from esolangs.interpreters.register_based.polynomial import (
            _Root,
            convert,
        )
    ...
    def test_non_prime_power_roots_produce_no_instruction(self) -> None:
        r"""Roots that do not map to an instruction (not a prime power, or a."""
    ...
    def test_unmatched_bracket_still_raises(self) -> None:
        r"""A real root that is a prime power but has no matching bracket is."""
        import pytest
...
class TestConvertRealRoots:
    r"""Real roots encode instructions reached through later primes."""

    def test_prime_root(self) -> None:
        r"""A prime root like 5 matches at its own prime (skipping composites)."""
        assert convert([5]) == [[1]]

    def test_prime_power_root(self) -> None:
        r"""A prime power like 4 (2^2) encodes an if-statement code."""
        assert convert([4]) == [[2]]
