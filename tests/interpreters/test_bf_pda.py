r"""Unit tests for the BF-PDA interpreter."""
...
class TestBrackets:
    def test_loop_skipped_when_top_zero(self) -> None:
        r"""``[`` jumps past the body when the top bit is 0."""
        assert run_program("<[.]") == ""

    def test_loop_runs_while_top_nonzero(self) -> None:
        r"""The body repeats while the top bit is 1, popping the bit out."""
        assert run_program("<@[>]") == ""
    ...
    def test_loop_terminates_on_empty_stack(self) -> None:
        r"""A ``[`` on an empty stack reads 0 and skips the body."""
        assert run_program("<@[>.]") == "0"
    ...
    def test_skip_over_nested_bracket(self) -> None:
        r"""``[`` with a zero top skips past nested ``[`` brackets."""
        assert run_program("[[.]]") == ""

    def test_jump_back_over_nested_bracket(self) -> None:
        r"""``]`` with a one top jumps back across a nested ``]`` exactly once."""
        assert run_program("<@<@[<@[>@]>]") == ""

    def test_forward_scan_skips_an_empty_inner_loop(self) -> None:
        r"""The forward scan counts an inner ``[`` even with nothing inside it."""
        assert run_program(".[[]@]") == "0"
        assert run_program(".[[].]") == "0"

    def test_backward_scan_lands_on_the_matching_open(self) -> None:
        r"""``]`` resumes at its own ``[``, counting a nested ``]`` on the way."""
        assert run_program("@<@.[>][]") == "1"
        assert run_program("@<@[>.][]") == "10"
...
class TestHalting:
    def test_halts_at_end_of_program(self) -> None:
        r"""The IP running off the end halts the machine, not a command bound."""
...
class TestEmptyStack:
    def test_pop_empty_stack_is_noop(self) -> None:
        r"""Popping an empty stack does nothing rather than halting."""
        assert run_program(">") == ""
        assert run_program(">>") == ""
        assert run_program("<>>") == ""

    def test_top_access_on_empty_stack_reads_zero(self) -> None:
        r"""Peeking an empty stack reads 0; ``@`` pushes and flips it."""
        assert run_program(".") == "0"
        assert run_program("@.") == "1"
        assert run_program("[<@.>]") == ""
        assert run_program("<@>.") == "0"
...
class TestMalformed:
    ...
    def test_unmatched_open_bracket_reports_the_last_one(self) -> None:
        r"""The reported position is the *last* unmatched ``[``, not the first."""
    ...
    def test_unmatched_close_bracket_reports_its_own_position(self) -> None:
        r"""A stray ``]`` names the index it sits at, checked exactly."""
