r"""``scripts/mutate_one.py`` repoints imports at the bundled."""
...
class TestRewriteImports:
    def test_the_interpreter_module_import_becomes_the_bundle(self) -> None:
        r"""``import <interp> as m`` is an alias for the module being mutated."""
        script = load_script()
    ...
    def test_a_generator_sharing_the_interpreters_name_is_left_alone(self) -> None:
        r"""A ``tools`` import is not the interpreter, even spelled alike."""
        script = load_script()
        for line, module in (
    ...
    def test_the_vm_and_registry_imports_are_left_alone(self) -> None:
        r"""Nothing outside the bundle is mutated, so those keep resolving."""
        script = load_script()
        for line in (
    ...
    def test_an_ordinary_package_import_is_repointed(self) -> None:
        r"""The names a bundled suite needs come from the bundle itself."""
        script = load_script()
        out = script._rewrite_imports(  # noqa: SLF001
...
class TestDropUnbundledTests:
    r"""What counts as reaching past the bundle, and what only looks like."""

    def test_a_reach_is_dropped_however_it_is_spelled(self) -> None:
        r"""Every syntax that actually leaves the bundle still cuts the test."""
        script = load_script()
        reaches = (
            "from esolangs.vm import run_until_halt_or_cycle",
    ...
    def test_naming_a_module_in_prose_is_not_a_reach(self) -> None:
        r"""A comment or docstring must not cut the test that carries it."""
        script = load_script()
        mentions = (
    ...
    def test_a_module_named_in_a_docstring_is_not_a_reach(self) -> None:
        r"""The near-miss that prompted this: a test explaining its own copy."""
        script = load_script()
        quotes = '"' * 3
        src = (
            "\nclass TestX:\n"
    ...
    def test_a_test_calling_a_reaching_helper_is_dropped(self) -> None:
        r"""The reach can be one call deep, and the caller carries no marker."""
        script = load_script()
        src = (
