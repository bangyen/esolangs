r"""Feeding a language its input bits is answerable without reading."""
...
def _rows(language: str, table: str) -> str:
    r"""Run every row of ``table`` through a freshly generated program."""
...
class TestTheExceptionalLanguages:
    r"""The four whose input shape is not a line per bit."""
    ...
    def test_the_encoding_computes_the_table(self, language: str, table: str) -> None:
        r"""Taglate at three inputs used to fail outright; the rest lied."""
    ...
    def test_every_reading_language_has_an_encoding(self) -> None:
        r"""``encode_inputs`` indexes the example table, which must cover all."""
...
class TestTemplatesAreNotWrapped:
    r"""A ``{Xi}`` slot is not a token any wrapper knows."""
    ...
    def test_a_width_leaves_a_template_intact(self, language: str) -> None:
        r"""A narrow width used to cut ``{X1}`` in half, silently."""
...
class TestRemainingGuards:
    r"""Small refusals added with these fixes."""

    @pytest.mark.parametrize("width", [0, -5])
    def test_a_nonpositive_width_is_refused(self, width: int) -> None:
        r"""It returned the unwrapped program, looking like it had honoured it."""
    ...
    def test_a_non_string_language_is_refused_by_name(self) -> None:
        r"""It leaked ``'NoneType' object has no attribute 'replace'``."""
    ...
    def test_the_cli_hint_passes_through_an_unrelated_message(self) -> None:
        r"""Only the slot refusal names a Python call worth rewriting."""
...
class TestAnswerMode:
    r"""The structured counterpart to the ``answer_convention`` prose."""
    ...
    def test_taglates_shape_names_its_padding(self) -> None:
        r"""Plain ``line_per_bit`` hid the pad digit its n=3 program needs."""
    ...
    def test_output_mode_means_the_printed_answer_is_the_table(self) -> None:
        r"""The classification is held to the interpreters, not just asserted."""
        wrong = []
        for name in esolangs.list_languages():
...
class TestEveryDeliberateErrorHasTheBase:
    r"""``except EsolangError`` is the whole handler, option errors."""
    ...
    def test_an_invalid_option_is_an_esolangerror(self, call: object) -> None:
        r"""These were bare ValueErrors, so the documented base class missed."""
    ...
    def test_encode_inputs_refuses_a_non_bit(self) -> None:
        r"""A 2 encoded as a 1 and answered a different row, in silence."""
