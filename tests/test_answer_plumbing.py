r"""What a caller needs to feed a program and judge what comes back."""
...
# : How each exceptional shape.
# : prose.
# : changes shape changes which.
_SHAPE_PROSE = {
...
class TestTheProseMatchesTheData:
    r"""Three documents named the shapes; one of the four names was wrong."""

    def test_exactly_three_languages_have_an_exceptional_shape(self) -> None:
        r"""Plus Grapheme's alphabet, which is the fourth exception."""
        odd = {
    ...
    def test_each_document_describes_each_shape_correctly(self, document: str) -> None:
        r"""Fargo was lumped in with Clockwise in all three at once."""
        # Whitespace collapsed first:.
        # newline lands in the middle.
        ...
        for name in esolangs.list_languages():
            ...
            # Some sentence has to describe.
            assert any(re.search(_SHAPE_PROSE[str(shape)], s) for s in naming), (
                document,
                name,
                naming,
            )
            # ...and none may describe it.
            # second half is the one that.
            # test: "Clockwise and Fargo.
            # Fargo in a sentence matching.
            # shape.
            # since the same documents also.
    ...
    def test_the_row_index_really_is_a_row_index(self) -> None:
        r"""The claim the prose got wrong, stated as an executable fact."""
    ...
    def test_following_the_old_sentence_would_now_be_caught(self) -> None:
        r"""A bit-per-line Fargo input is the wrong row, and only n>=4 shows it."""
...
class TestEncodeInputsCanCheckItsArity:
    r"""The one function whose purpose is to stop a silent mis-encoding."""

    def test_a_wrong_bit_count_is_refused_when_the_table_is_given(self) -> None:
        r"""Three bits at a four-row table encoded as cheerfully as two."""
...
class TestATemplateKnowsWhoseItIs:
    ...
    def test_its_own_language_still_fills_it(self) -> None:
        r"""And the filled program answers the row it was asked for."""
        template = esolangs.generate("Minifuck", "0110")
...
class TestExamplePathsWorkFromAnywhere:
    r"""The recipe this package advertises worked from one directory."""

    def test_they_are_absolute(self) -> None:
        r"""``run(lang, Path(describe(lang)["examples"][0]))`` is documented."""
        for name in esolangs.list_languages():
    ...
    def test_the_advertised_recipe_runs_from_another_directory(
        ...
        r"""A chdir away it was ``cannot read examples/boolean/brainfuck.txt``."""
...
class TestTheVerifierIsShipped:
    r"""Two blind readers and the test suite each wrote this same function."""

    def test_evaluate_returns_the_table_the_program_computes(self) -> None:
        r"""So a mismatch is locatable rather than summarized to False."""
        assert esolangs.evaluate("brainfuck", "0110") == "0110"

    def test_verify_is_the_comparison(self) -> None:
        r"""The verdict, for callers who only want the verdict."""
    ...
    def test_it_covers_the_languages_that_need_per_language_knowledge(
        ...
        r"""The four odd input shapes and two template languages."""
        assert esolangs.verify(name, "0110")

    def test_it_reads_the_termination_polarity_as_data(self) -> None:
        r"""Rather than assuming halting is the zero."""
    ...
    def test_a_malformed_table_is_refused_before_anything_runs(self) -> None:
        r"""Named as a table, not as whichever generator saw it first."""
        with pytest.raises(esolangs.TruthTableError):
            esolangs.evaluate("brainfuck", "011")

    @pytest.mark.slow
    def test_every_language_verifies(self) -> None:
        r"""69/69, through the public function rather than a local copy."""
        failed = [
...
class TestTheNewChecksRefuseTheirOwnBadInput:
    r"""The arguments the round's new parameters can be given wrongly."""

    def test_encode_inputs_names_a_non_string_table(self) -> None:
        r"""It is a table's *type* that is wrong, not the bit count."""
    ...
    def test_evaluate_names_a_non_string_table(self) -> None:
        r"""Reported before any generator sees it, for the same reason."""
    ...
    def test_machine_traits_refuses_an_unregistered_name(self) -> None:
        r"""Same contract as ``make_vm``, which is the only other reader."""
...
class TestADeliberateRefusalIsAnEsolangError:
    r"""The package promises it, and the five refusals broke the promise."""

    # : The five that stop rather.
    # : Only these are built here:.
    # : several take minutes to do.
    # :.
    # : Factor's is 13 rather than.
    # : integer's* digit count and.
    # : tree moved the trip point.
    # : n=12 table in under three.
    # : as one table, because a.
    # : all: the ``pytest.raises``.
    ...
    def _big_table(arity: int = 11) -> str:
        r"""Return a dense table at ``arity``, past the cap it is used for."""
        import random
    ...
    def test_the_refusal_is_catchable(self, name: str) -> None:
        r"""And by the documented base class, not only the specific one."""
    ...
    def test_the_documented_idiom_catches_it(self, name: str) -> None:
        r"""``except EsolangError`` is what the package docstring promises."""
        with pytest.raises(esolangs.EsolangError):
            esolangs.generate(name, self._big_table(self._REFUSERS[name]))

    def test_it_is_still_a_value_error(self) -> None:
        r"""Callers catching ValueError must not be broken by the new class."""
    ...
    def test_it_is_exported(self) -> None:
        r"""A refusal nobody can name is a refusal nobody can catch."""
        assert "GeneratorCapError" in esolangs.__all__

    @pytest.mark.slow
    def test_no_private_name_leaks_into_a_message(self) -> None:
        r"""WII2D's named its own module-private constant at the reader."""
...
class TestWidthEffectSaysWhatWidthDoes:
    r"""One flag, three behaviours, and no way to tell them apart."""

    def test_every_language_declares_one_of_three(self) -> None:
        r"""A fourth value would be a behaviour nobody documented."""
    ...
    def test_layout_is_exactly_the_width_aware_generators(self) -> None:
        r"""The old field is the new field's `layout` case, and only that."""
        for name in esolangs.list_languages():
    ...
    def test_the_declaration_matches_what_width_actually_does(self) -> None:
        r"""The drift guard: `none` must really be a no-op."""
        wrong = []
        for name in esolangs.list_languages():
            facts = esolangs.describe(name)
            ...
            # `wrap` and `layout` may.
            # than the width, so only the.
        assert not wrong, "\n".join(wrong)

    def test_a_wrapping_language_really_reflows(self) -> None:
        r"""The positive control for the check above, which only tests `none`."""
...
class TestEveryAuditedCapIsCatchable:
    r"""The n=11 probe that found the first five was bounded by n=11."""

    def test_nocomment_is_catchable_at_the_size_it_refuses(self) -> None:
        r"""The confirmed escape, at the first arity that triggers it."""
    ...
    def test_it_is_catchable_through_evaluate_too(self) -> None:
        r"""It leaked through ``evaluate`` identically."""
        with pytest.raises(esolangs.GeneratorCapError):
            esolangs.evaluate("NoComment", "01" * (1 << 11))

    @pytest.mark.slow
    def test_nothing_escapes_the_contract_at_twelve_inputs(self) -> None:
        r"""A periodic table, so the generators that blow up stay small."""
        table = "0010" * (1 << 10)
        escaped = []
        for name in esolangs.list_languages():
            try:
    ...
    def test_the_docstring_states_no_count(self) -> None:
        r"""It said "Five do" and six do; a tally in prose is a second copy."""
...
class TestGraphemeReadsWhatTheDocsNowSay:
    r"""The stated mechanism was wrong, and so was its stated direction."""

    def test_only_an_a_line_reads_as_one(self) -> None:
        r"""'0', '1', 'x' and ' ' are all non-empty and all read as 0."""
    ...
    def test_the_convention_no_longer_claims_truthiness(self) -> None:
        r"""It said a 0/1 line reads as a 1; it reads as a 0."""
    ...
    def test_naive_input_answers_the_all_zeros_row(self) -> None:
        r"""The true consequence: every bit reads 0, so you get row 0."""
...
class TestBreakAtChecksTheKindOfPosition:
    r"""Both wrong-kind breakpoints were stored and could never fire."""

    def test_a_tuple_is_refused_where_the_ip_is_an_index(self) -> None:
        r"""brainfuck's ip is an int."""
    ...
    def test_the_right_kind_is_accepted(self) -> None:
        r"""And still fires, which is the point of checking the other."""
        program = esolangs.generate("brainfuck", "0110")
        debugger = esolangs.make_debugger("brainfuck", program, "0\n1\n")
        debugger.break_at(0)
    ...
    def test_the_arity_is_not_checked(self) -> None:
        r"""It varies within a run, so checking it would refuse valid ones."""
        program = esolangs.generate("Alight", "0110")
        debugger = esolangs.make_debugger("Alight", program, "0\n1\n")
        debugger.break_at((1, 2))  # wrong arity for Alight,.


@pytest.mark.filterwarnings("ignore::UserWarning")
class TestWhatHappensWhenAProgramIsUnderfed:
    r"""``run`` promised an exception for all sixty-nine."""

    TABLE = "10010110"  # n = 3.

    def _underfed(self, name: str) -> tuple[str, str | None]:
        r"""Return ``(outcome, answer)`` for ``name`` fed one bit too few."""
    ...
    def test_only_the_declared_languages_answer_an_underfed_program(self) -> None:
        r"""The census, as a sweep: the flag and the behaviour must agree."""
        mismatched = []
        for name in esolangs.list_languages():
            facts = esolangs.describe(name)
            ...
            if facts["input_shape"] == "one_line":
                # Underfeeding a one-line.
                # string*, not a read past an.
                # declare and nothing that.
                # its shape rather than by its.
                continue
    ...
    def test_most_languages_raise(self) -> None:
        r"""The norm, counted, so a regression that erodes it is visible."""
        raised = sum(
            1
            for name in esolangs.list_languages()
        ...
        # 43 of the 52 that read stdin,.
        # a change which quietly moves.
        # here rather than in a.
        assert raised == 43

    def test_the_trait_is_reported_by_describe(self) -> None:
        r"""A caller must be able to learn this without underfeeding one."""
    ...
    def test_clockwise_is_not_marked_because_it_never_reads_past_an_end(
        ...
        r"""Its underfed input is a shorter one-line string: no EOF happens."""
    ...
    def test_run_no_longer_promises_the_exception_everywhere(self) -> None:
        r"""The sentence that was false for seven languages."""
...
class TestTheTerminationVocabularyIsExported:
    r"""A reader hand-copied this tuple and said so."""

    def test_it_matches_what_describe_reports(self) -> None:
        r"""The same two strings, in the order the polarity is read from."""
    ...
    def test_it_is_exported(self) -> None:
        r"""Beside ``STOP_REASONS``, which closed the same gap for stopping."""
        assert "TERMINATION_OUTCOMES" in esolangs.__all__


class TestCapMessagesNameOnlyReachableRemedies:
    r"""Factor's told you to pass a parameter the public API has not got."""

    @pytest.mark.slow
    def test_factor_does_not_name_a_private_knob(self) -> None:
        r"""`generate(language, truth_table, width)` has no `max_digits`."""
        import random
...
class TestBreakAtNamesTheKindNotTheValue:
    r"""It said "ip is 0" where it meant "ip is an index"."""

    def test_the_message_describes_the_kind(self) -> None:
        r"""A reader cannot generalize from one position's value."""
        program = esolangs.generate("brainfuck", "0110")
        debugger = esolangs.make_debugger("brainfuck", program, "0\n1\n")
...
class TestTheStdinJudgeIsReachableFromPython:
    r"""The one place the API was weaker than the command line."""

    def test_it_accepts_what_encode_inputs_builds(self) -> None:
        r"""The check must never fire on this package's own encoding."""
        wrong = []
        for name in esolangs.list_languages():
            facts = esolangs.describe(name)
    ...
    def test_it_catches_the_wrong_alphabet(self) -> None:
        r"""0/1 lines into Grapheme, the sharpest edge in the package."""
    ...
    def test_it_catches_the_wrong_alphabet_on_one_line_too(self, stdin: str) -> None:
        r"""Clockwise's declared alphabet was enforced nowhere at all."""
        with pytest.raises(esolangs.ArgumentError, match="spells its bits"):
            esolangs.check_stdin("Clockwise", stdin, "10010110")

    def test_a_good_one_line_stdin_is_still_accepted(self) -> None:
        r"""A guard that refused the correct input would be worse."""
    ...
    def test_the_run_path_warns_about_it_as_well(self) -> None:
        r"""Both documented routes, since both were silent."""
    ...
    def test_it_catches_a_surplus_line(self) -> None:
        r"""Six lines into a three-input program answered the first three."""
    ...
    def test_it_catches_a_missing_line(self) -> None:
        r"""The direction that already errored at run time, now before it."""
        with pytest.raises(esolangs.ArgumentError, match="reads 3 line"):
            esolangs.check_stdin("brainfuck", "1\n0\n", "00010111")

    def test_it_catches_an_out_of_range_row_index(self) -> None:
        r"""What `run` could not check, because it does not know the arity."""
    ...
    def test_it_catches_taglates_pad(self) -> None:
        r"""Its odd input count costs an extra line, and the shape says so."""
    ...
    def test_it_refuses_a_language_with_no_stdin(self) -> None:
        r"""A template language reads none, so there is nothing to judge."""
    ...
    def test_the_table_is_optional(self) -> None:
        r"""Shape and alphabet are checkable without knowing the arity."""
        esolangs.check_stdin("brainfuck", "1\n0\n")

    def test_a_non_string_stdin_is_named(self) -> None:
        r"""A caller who passes the bit list itself, which is an easy slip."""
    ...
    def test_a_one_line_language_has_its_bits_counted(self) -> None:
        r"""Clockwise's underfeed is a shorter string, not a missing line."""
    ...
    def test_the_closed_sets_are_exported(self) -> None:
        r"""A verifier branching on these should not spell a magic string."""
        modes = {
...
class TestRunSaysWhenStdinLooksWrong:
    r"""Silence was indistinguishable from correctness, from Python."""
    ...
    def test_a_program_that_reads_nothing_is_not_warned_about(self) -> None:
        r"""Reading none of what it was given is not an arity mistake."""
        import warnings
    ...
    def test_taking_a_value_from_past_the_end_is_warned_about(self) -> None:
        r"""The six that answer an underfed program now say they did."""
    ...
    def test_forgetting_stdin_entirely_is_warned_about(self) -> None:
        r"""Fargo answered row 0 -- the starkest case, since nothing was fed."""
        with pytest.warns(UserWarning, match="past the end"):
    ...
    def test_a_language_whose_documented_stop_is_eof_is_not_warned_about(
        ...
        r"""Suffolk's programs end *by* running out of input."""
        import warnings
    ...
    def test_no_language_warns_on_its_own_encoding(self) -> None:
        r"""The sweep that decides whether any of this is worth having."""
        import warnings
...
class TestInstantiateCanCheckProvenance:
    r"""A tag cannot survive a file, but a table can be compared against."""

    def test_a_hand_written_template_is_refused_given_the_table(self) -> None:
        r"""`"hello {X0}"` filled to `'hello [<'` and ran to nothing."""
    ...
    def test_the_real_template_passes(self) -> None:
        r"""And still fills, and still answers its row."""
        template = esolangs.generate("Minifuck", "0110")
    ...
    def test_the_table_stays_optional(self) -> None:
        r"""Every existing caller passes three arguments."""
        template = esolangs.generate("Minifuck", "0110")
        assert esolangs.instantiate("Minifuck", template, [0, 1])


class TestSnapshotSaysItIsOpaque:
    r"""Its positions are not a schema and cannot be."""

    def test_the_docstring_says_so(self) -> None:
        r"""A reader asked what the fields were; there are no fields."""
...
class TestTheApiNameListsCannotDriftAgain:
    r"""Three rounds running, a reader found a public name in neither list."""

    @staticmethod
    def _public_callables() -> set[str]:
        r"""The exported names that are functions a caller would call."""
        import inspect
    ...
    def test_the_readme_sentence_names_every_public_function(self) -> None:
        r"""The sentence that introduces the API has to introduce all of it."""
        readme = (pathlib.Path(__file__).parents[1] / "README.md").read_text()
        # The paragraph, not the first.
        # ``esolangs.run`` and made.
    ...
    def test_the_module_docstring_names_every_public_function(self) -> None:
        r"""Same for the thing ``help(esolangs)`` shows first."""
...
class TestDivergenceIsProvenNotWaitedOut:
    r"""A repeated state settles it exactly, and in milliseconds."""
    ...
    def test_the_proven_answer_is_the_table(self, name: str, table: str) -> None:
        r"""The answers must be the ones the clock used to give, exactly."""
        assert esolangs.evaluate(name, table) == table

    def test_it_no_longer_costs_a_timeout_per_row(self) -> None:
        r"""It was five seconds per 1-row: twenty seconds for this call."""
        import time
...
class TestUnknownLanguageAlwaysOffersANextStep:
    r"""A near miss suggested; a far one was a dead end."""

    def test_a_far_miss_names_the_listing_command(self) -> None:
        r"""Someone misremembering a name has nothing to be suggested."""
    ...
    def test_a_near_miss_still_suggests(self) -> None:
        r"""The better hint must win where there is one."""
...
class TestTheWarningHasItsOwnClass:
    r"""So a sweep can escalate exactly these to errors."""

    def test_it_is_a_user_warning_subclass(self) -> None:
        r"""Existing ``UserWarning`` filters must keep working."""
    ...
    def test_run_raises_it_by_class(self) -> None:
        r"""Which is what makes ``filterwarnings("error", ...)`` targeted."""
...
class TestARowIndexNeverHasALeadingZero:
    r"""`0010` fed to a 16-row program parses as ten and answers row 10."""

    def test_a_bit_string_typed_as_an_index_is_caught(self) -> None:
        r"""No table needed: the leading zero alone decides it."""
...
class TestATimeoutCannotKillTheProcess:
    ...
    def test_the_caller_gets_their_disposition_back(self) -> None:
        r"""Including ``SIG_DFL``, which the previous fix kept for itself."""
        import signal
    ...
    def test_a_custom_handler_is_given_back_too(self) -> None:
        r"""The case that always worked, kept so the fix cannot regress it."""
        import signal

        def _mine(_signum: object, _frame: object) -> None:
            r"""A handler a caller might have installed."""
    ...
    def test_the_timer_is_always_disarmed(self) -> None:
        r"""A timer left armed is the next run's stray alarm."""
        import signal
    ...
    def test_many_runs_at_the_floor_neither_die_nor_leak(self) -> None:
        r"""The stress the floor was chosen against, in process."""
        import signal
...
class TestErrorsSurviveAProcessBoundary:
    r"""The library's commonest error could not come home from a worker."""

    def test_input_exhausted_round_trips(self) -> None:
        r"""It built its message in ``__init__``, so unpickling passed one arg."""
        import pickle
    ...
    def test_unknown_language_does_not_grow_its_message(self) -> None:
        r"""It re-applied its prefix on every hop: "unknown language: " twice."""
        import pickle
    ...
    def test_every_error_class_round_trips(self) -> None:
        r"""The two above were found one at a time; this is the class."""
        import pickle
...
class TestTheCallersSignalsAreTheirOwn:
    r"""The fix for the death took the caller's SIGALRM hostage."""

    def test_a_pending_alarm_survives_a_timed_run(self) -> None:
        r"""Arming ours cancelled theirs, and nothing put it back."""
        import signal
    ...
    def test_the_default_disposition_is_restored(self) -> None:
        r"""It was left as a no-op, which swallowed the caller's later alarms."""
        import signal
    ...
    def test_a_bound_too_short_to_service_is_refused(self) -> None:
        r"""Measured: at 100us, 19 of 20 processes died; at 1ms, none of 4000."""
        with pytest.raises(esolangs.ArgumentError, match=r"at least 0\.001"):
            esolangs.run("brainfuck", "+.", "", 0.0001)

    def test_the_floor_applies_to_evaluate_too(self) -> None:
        r"""Its termination path never reaches ``run``, so it checked nothing."""
        with pytest.raises(esolangs.ArgumentError, match=r"at least 0\.001"):
            esolangs.evaluate("123", "0110", 1e-06)


class TestEvaluateCanRunOffTheMainThread:
    r"""The wall-clock guard is a signal, and there was no way to opt out."""

    def test_an_explicit_none_means_unbounded(self) -> None:
        r"""As it does in ``run``; here the same word meant "use the default"."""
...
class TestAPaintersMarkMustBeInAGrid:
    ...
    def test_a_real_grid_still_reads(self) -> None:
        r"""The check is worth nothing if it costs the actual answers."""
        assert esolangs.evaluate("A Painter Ant", "0110") == "0110"


class TestTheDebuggerMirrorsSnapshot:
    r"""Reaching through ``.vm`` is what the mirrors exist to avoid."""

    def test_it_matches_the_wrapped_machine(self) -> None:
        r"""And is the thing a caller most wants: a repeated state."""
        program = esolangs.generate("brainfuck", "0110")
        stdin = esolangs.encode_inputs("brainfuck", [0, 1], "0110")
...
class TestAnInterpreterLimitIsStillAnEsolangError:
    r"""Qoibl's interpreter recurses, and Python's stack is finite."""

    @staticmethod
    def _parity(n: int) -> str:
        r"""Return the parity table of arity ``n`` -- reliably a hard one."""
        return "".join(str(bin(i).count("1") % 2) for i in range(2**n))

    def test_a_recursion_error_does_not_escape(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        r"""It was the one exception in the package that was not ours."""
    ...
    def test_qoibl_no_longer_hits_the_wall(self) -> None:
        r"""The six-input table this class was built around now computes."""
        assert esolangs.verify("Qoibl", self._parity(6), timeout=300)

    def test_it_is_a_halt_error(self) -> None:
        r"""The run ended abnormally, which is what that base means."""
    ...
    def test_it_is_not_a_generator_cap(self) -> None:
        r"""A cap declines to build; this built and could not be run."""
...
class TestAWidthAwareGeneratorCanStillOverrun:
    r"""The docstring named LaserFuck; Streetcode is the worse of the two."""

    @staticmethod
    def _overruns(name: str, table: str) -> tuple[int, int]:
        r"""Return how many widths overran, and by the worst margin."""
        counted = [
    ...
    def test_both_width_aware_generators_can_overrun(self) -> None:
        r"""Which is the claim; the numbers live here rather than in prose."""
    ...
    def test_streetcode_is_the_worse_of_the_two(self) -> None:
        r"""The specific thing the docstring got backwards."""
    ...
    def test_the_pair_is_still_width_aware(self) -> None:
        r"""The two this class measures must stay in the group it measures."""
        aware = {
            name
            for name in esolangs.list_languages()
...
class TestTheCheckProgramExampleRuns:
    r"""Its inline one-liner raised for two of the three languages it names."""

    @pytest.mark.parametrize("name", ["CV(N)(C)", "Grapheme", "NoComment"])
    def test_reading_a_committed_example_works(self, name: str) -> None:
        r"""The newline fix is real; the snippet showing it was not runnable."""
...
class TestEvaluateNoLongerClaimsToPayTheTimeout:
    r"""Its docstring and ``Debugger.snapshot``'s disagreed about the same."""

    def test_a_termination_table_returns_far_inside_the_bound(self) -> None:
        r"""Five seconds per 1-row would be twenty for this table."""
        import time
    ...
    def test_the_docstring_says_the_proof_is_the_mechanism(self) -> None:
        r"""Prose, checked, because it was prose that had gone stale."""
        doc = esolangs.evaluate.__doc__
        assert doc is not None
        # Not a search for the old.
        # order to retract it, so an.
        # has to be there is the.
        assert "repeated machine state" in doc
        assert "do not pay it" in doc


class TestDescribeDocumentsWhatItReturns:
    r"""A key you get back should be findable in the docstring you read."""

    def test_every_key_is_named_in_the_docstring(self) -> None:
        r"""Naming, not explaining: a grep for the key has to land somewhere."""
    ...
    def test_every_language_returns_the_same_keys(self) -> None:
        r"""The guard above reads one language, so the keys must not vary."""
        keys = {frozenset(esolangs.describe(n)) for n in esolangs.list_languages()}
        assert len(keys) == 1


class TestTheTwoWidthKeysCannotDrift:
    r"""``width_aware`` is exactly ``width_effect == "layout"``."""

    def test_width_aware_is_the_layout_case(self) -> None:
        r"""Every language, not a sample: the old duplication was per-language."""
        for name in esolangs.list_languages():
    ...
    def test_all_three_effects_are_represented(self) -> None:
        r"""A guard over a field with one value in practice guards nothing."""
        counts: dict[str, int] = {}
        for name in esolangs.list_languages():
...
class TestEveryDumpSaysWhereTheAnswerIs:
    r"""A dump prints the whole final state, so "where" is the question."""

    def test_a_dump_has_a_pattern_or_a_note(self) -> None:
        r"""Either a regex that finds the answer, or prose that locates it."""
        silent = [
            name
            for name in esolangs.list_languages()
    ...
    def test_bitdeques_note_is_true(self) -> None:
        r"""It claims the whole dump is the answer bit."""
...
class TestABadStdinIsAnArgumentFault:
    r"""Four entry points filed it as a *program* fault, and one did not."""
    ...
    def test_every_entry_point_agrees(self, call: object) -> None:
        r"""One fault, one class -- ``except ArgumentError`` has to cover all."""
        with pytest.raises(esolangs.ArgumentError, match="stdin must be a string"):
            call()  # type: ignore[operator]

    def test_a_bad_program_is_still_a_program_error(self) -> None:
        r"""The change must not blur the distinction the other way."""
...
class TestASurroundingSpaceResolves:
    r"""67 of 69 names already tolerated one, and the two that did not."""

    @pytest.mark.parametrize("pad", [" {}", "{} ", " {} ", "\t{}\n"])
    def test_every_language_tolerates_surrounding_space(self, pad: str) -> None:
        r"""All 69, because the two that failed were not the obvious two."""
        for name in esolangs.list_languages():
...
class TestATableLengthNamesTheNearestLegalOnes:
    r"""The rule without the arithmetic, on the likeliest first error."""
    ...
    def test_the_brackets_are_named(self, table: str, expected: str) -> None:
        r"""And singular where it should be: "1 input", not "1 inputs"."""
    ...
    def test_an_empty_table_gets_no_brackets(self) -> None:
        r"""``2 ** -1`` is 0.5, so the arithmetic does not apply to nothing."""
    ...
    def test_the_brackets_are_actually_legal_lengths(self) -> None:
        r"""The message would be worse than none if it named an unusable size."""
        with pytest.raises(esolangs.TruthTableError) as caught:
...
class TestFillingSomethingWithNoSlots:
    ...
    def test_filling_twice_says_the_same_thing(self) -> None:
        r"""The other way to get here, and it looks identical from inside."""
        template = esolangs.generate("Minifuck", "0110")
    ...
    def test_a_real_slot_mismatch_still_counts(self) -> None:
        r"""The count is the right answer when there *are* slots."""
        template = esolangs.generate("Minifuck", "0110")
...
class TestASuggestionIsWorthLessThanSilence:
    r"""0.6 offered ``Sophie`` for ``nope``."""

    # : Single-edit slips of a real.
    @staticmethod
    def _typos(name: str) -> list[str]:
        r"""Dropped and transposed characters, keeping only real misses."""
    ...
    def _score(self, cutoff: float) -> tuple[int, int, int]:
        r"""Return (typos rescued, typos tried, junk words given a guess)."""
        rescued = tried = 0
        for name in esolangs.list_languages():
    ...
    def test_a_word_that_is_not_close_gets_no_guess(self, word: str) -> None:
        r"""It gets the command that lists them, which is the honest answer."""
        with pytest.raises(esolangs.UnknownLanguageError) as caught:
    ...
    def test_a_real_typo_is_still_rescued(self, typo: str, wanted: str) -> None:
        r"""The half of the trade that raising a cutoff can quietly cost."""
        with pytest.raises(esolangs.UnknownLanguageError) as caught:
            esolangs.describe(typo)
        assert wanted in str(caught.value)

    def test_the_cli_shares_the_number(self) -> None:
        r"""Its docstring promised the same cutoff while keeping its own copy."""
...
class TestReadAnswerExplainsInWords:
    ...
    def test_a_plain_language_is_unchanged(self) -> None:
        r"""No pattern, no note, and nothing to add -- it was already clear."""
        with pytest.raises(esolangs.ProgramError) as caught:
    ...
    def test_every_dump_language_says_something_in_words(self) -> None:
        r"""The general claim, not the two cases that prompted it."""
        for name in esolangs.list_languages():
            facts = esolangs.describe(name)
...
class TestAFailedRowSaysWhichRow:
    r"""``execution exceeded the 0.2-second timeout`` and nothing else."""

    def test_the_note_names_the_row_and_the_inputs(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        r"""Failure forced at a chosen row, since a real one lands on row 0."""
        ...
        assert "01101" in note  # the answers that did come.

    def test_a_real_timeout_carries_one_too(self) -> None:
        r"""Not only the stand-in: the path a caller actually hits."""
    ...
    def test_a_clean_evaluate_adds_nothing(self) -> None:
        r"""A note is for a failure; a success must not grow one."""
        assert esolangs.evaluate("brainfuck", "0110") == "0110"

    def test_the_note_survives_the_exception_type(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        r"""The reason it is a note: the class and its arguments are untouched."""
        real_run = esolangs.run
...
class TestEvaluateTakesAWidth:
    r"""Checking a wrapped program meant reimplementing the loop."""

    def test_every_language_survives_a_wrap(self) -> None:
        r"""All 69, because a wrapper that broke one would break it quietly."""
        wrong = [
            name
            for name in esolangs.list_languages()
    ...
    @pytest.mark.parametrize(
        ("name", "effect"),
        # Clockwise used to stand for.
        # width, which is what pinning.
        # for: it failed here rather.
        # "wrap" twice.
    ...
    def test_it_works_for_each_width_effect(self, name: str, effect: str) -> None:
        r"""The three things a width can do, one language each."""
    ...
    def test_a_template_language_gets_the_width_too(self) -> None:
        r"""The width has to land after the bits are in, on ``instantiate``."""
    ...
    def test_the_width_actually_reaches_the_program(self) -> None:
        r"""Otherwise this would pass with the argument thrown away."""
        wide = esolangs.generate("brainfuck", "10010110")
        narrow = esolangs.generate("brainfuck", "10010110", 30)
        assert "\n" not in wide  # the unwrapped default is one.
        assert "\n" in narrow
    ...
    def test_verify_takes_one_as_well(self) -> None:
        r"""It is ``evaluate`` with the comparison done, so it must pass it on."""
        assert esolangs.verify("brainfuck", "10010110", width=30)

    def test_no_width_is_unchanged(self) -> None:
        r"""The default has to stay exactly what it was."""
        assert esolangs.evaluate("brainfuck", "0110") == "0110"
