...
What remains of \eqref{eq:shift} is $|A|\ge2$.  There the chord through
$F_{A+1}$ and $p_{g,A+1}$ is not enough on its own: the analogous
inequality fails without the hypothesis of \eqref{eq:shift}, and the single
hypothesis $\Tail(B_0\cup A)\le\Tail(B_0\cup(A+1))$ is not enough either,
so the per-move hypotheses have to enter the count.
