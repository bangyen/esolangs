...
Sliding down the \emph{smallest} zero above $g$ keeps the displacement.
...
some $N>\max A$ makes the full certificate on $y_{k+1},\ldots,y_L$ with zero
set $A\cup\{N\}$ have tail at most $\Tail(v^-)$.
\end{conjecture}
...
The displacement stayed short in everything we measured, $N\le\max A+3$, so the
rule leaves a bounded search rather than an unbounded one.

A one-parameter search over $N$ is forced only after the slide; the minimizing
zero set itself is not a consecutive block with one displaced zero.  At
...
covers the straddling case.  We tested \eqref{eq:step} at $47638$ instances
across two sweeps --- integer root sets to $L=6$, rational ones, and sets
clustered just above the cutoff --- with no violation, $1017$ of them
straddling and every one covered by the slide-down rule.  The two alternatives
are refuted above: deleting $\min X$ rather than $\max X$ fails at $(2,3,4,5)$,
and freeing $\max Z^-$ rather than the smallest zero above $g$ fails by a
factor $1.40$.  Proposition~\ref{prop:head} states the same question
