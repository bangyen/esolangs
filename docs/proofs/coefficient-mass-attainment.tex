...
$r_1=2$.  Repeated roots beyond the confluent pair $(3,3)$ also remain open, and the
obstruction is a single quantitative step.  In the Hermite basis $d^ay_i^d$,
...
$\{1,\ldots,13\}$, and none with $\min E\ge21$.  The decay also degrades.  When
the cut falls inside a multiplicity block the supremum of $\eta$ over the sets
of size $u$ in a window is $\Theta(1/\min E)$---exactly $1/(n_0+1)$ at $(3,3)$,
$(2,2)$, and $(3,3,5)$ and $(3,3,4,4)$ with $u=1$---against the geometric rate
at an aligned cut, where it is $2.1\cdot10^{-23}$ at $(2,3,3)$ and
$3.0\cdot10^{-30}$ at $(3,3,5)$ with $u=2$, both at $n_0=140$.  The conclusion
$\eta_\nu\to0$ survived in every configuration we computed, so the lemma is
expected to hold; what is missing is a proof of it that tolerates a degenerate
alternant, in place of the geometric estimate.
