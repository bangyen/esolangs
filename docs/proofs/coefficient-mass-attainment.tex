...
is an infimum in general, with root sets where the criterion holds and where it fails.
...
behind the infimum in general, and reports root sets where the criterion
...
It is at least a recognizable question.  Heuristically, in the limit the $u$
exempt coefficients are spent absorbing the $u$ smallest roots, so an
admissible $W$ vanishes at the surviving reciprocal nodes, and dividing by
$\widetilde P_u$ leaves a function analytic on the disc: an admissible $W$
should be $\widetilde P_uH$ with $H(0)=1$, the choice $H=1/(1-y)$ being
\eqref{eq:wgen}.  The converse is then expected to reduce to
\[
...
minimizes $\ell^1$ over normalized multiples~\cite{schinzel}, its
integer-polynomial $\ell^2$ analogue, the minimal Euclidean norm of an
algebraic number, is effectively computable~\cite{frw}, and the logarithmic
...
at $L=4$, as $(3,4,5,6)$ shows.  The paragraph
before Proposition~\ref{prop:partial} records root sets of each kind.
Beyond the root sets covered by Proposition~\ref{prop:partial} and the
confluent pair $(3,3)$, whether \eqref{eq:order} is an infimum at a given
root set and $k\ge2$ is open in general.
