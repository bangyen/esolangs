...
$B_0=\{1,\ldots,k\}$, $g=k+1$, where it is an identity for arbitrary nodes;
there it is Lemma~1.5 of the confluent computation above, and
Proposition~\ref{prop:shift}(3) is its consequence.  It is tied to the shift
by one: the analogous relation with $M+2$ in place of $M+1$, or with $g\ne k+1$
in the block case, fails in both directions, and the chord statement with
several zeros above $g$ fails without the hypothesis of \eqref{eq:shift}.
