...
We have no proof of \eqref{eq:shift}, and no counterexample: it holds in all
...
instances without the hypothesis violate the conclusion; for $|A|=1$ it holds
in $6015$ further instances with $L\le7$.  The largest ratio seen is
$0.9993$, at the clustered nodes with $B_0=\{1,2,3,4\}$, $g=5$, $A=\{10\}$,
against $1$ in the confluent limit.
...
The hypothesis can be separated from the geometry.  The three points
$F_{M+1}$, $p_{M,M+1}$ and $w=p_{g,M+1}$ lie on $\ell_{M+1}$ in that order,
so $p_{M,M+1}=\lambda F_{M+1}+(1-\lambda)w$ for one $\lambda\in(0,1)$, which
is $(M-g)/M$ in the confluent limit.  If
\begin{equation}\label{eq:chord}
  \Tail(F_M)\ \ge\ \lambda\,\Tail(F_{M+1})+(1-\lambda)\,\Tail(w),
\end{equation}
then $\Tail(F_M)\le\Tail(F_{M+1})$ gives $\Tail(w)\le\Tail(F_M)$ at once.
\eqref{eq:chord} carries no hypothesis, and it is exact: writing
$\Tail(F_M)-\Tail(w)$ through the deletion step from $F_M$ to $p_{M,M+1}$
and the convexity of $\Tail$ along $\ell_{M+1}$,
\[
  \Tail(F_M)-\Tail(w)
  =\frac{\lambda}{1-\lambda}\bigl(\Tail(F_{M+1})-\Tail(F_M)\bigr)
   +\frac{\delta-\gamma}{1-\lambda},
\]
with $\delta=\Tail(F_M)-\Tail(p_{M,M+1})>0$ the gain of the deletion step and
$\gamma=\lambda\Tail(F_{M+1})+(1-\lambda)\Tail(w)-\Tail(p_{M,M+1})\ge0$ the
convexity gap, so \eqref{eq:chord} says $\delta\ge\gamma$: the deletion
step earns at least the convexity gap.  It holds in $4632$ exact instances
with $L\le7$ and every $M>g$, with equality exactly in the block case
$B_0=\{1,\ldots,k\}$, $g=k+1$ with $y_{k+1}=\tfrac12$, where it is an
identity for arbitrary nodes below the cutoff and strict once
$y_{k+1}<\tfrac12$; in the confluent limit it is the closed form of
$\E|D-M|$ in the proof of Proposition~\ref{prop:shift}, whose part (3) is its
consequence.  It is tied to the shift by one: the analogous relation with
$M+2$ in place of $M+1$, or with $g\ne k+1$ in the block case, fails in both
directions, and the chord statement with several zeros above $g$ fails
without the hypothesis of \eqref{eq:shift}.  It is also tied to the integers:
with the zeros of $B_0$ allowed to be real, \eqref{eq:chord} fails already in
the confluent limit at $B_0=\{1,\tfrac52\}$, $g=3$, $M=6$, so no argument by
convexity or continuity in the zero positions can prove it, and a proof must
use the empty integer slots below $g$.
...
The second was reduced to one inequality, and the inequality has the right
hypothesis but no proof.  By Proposition~\ref{prop:step} it is the single
...
supplies (Proposition~\ref{prop:shift}); it holds in every one of $7023$
instances with the hypothesis at $L\le9$, fails without it, and is sharp at
the confluent block, where it is proved.  Three alternatives are refuted
