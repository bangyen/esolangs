...
exceeds their total, repeated roots included.  When this criterion fails and
the roots are distinct, the placement in which the exempt coefficients escape
...
two-sided bound $b_{u+1}\ge|f_D|\min(|P(1)|,|P_u(1)|)$, and for distinct
roots we determine exactly when each is the infimum.  We also extend the tail
...
\emph{The value.}  For distinct $2\le r_1<\cdots<r_L$ let $\tau^*$ be the
least tail $\sum_{d\ge1}|v_d|$ of an exponential sum
$v_d=\sum_{i>u}\lambda_ir_i^{-d}$ with $v_0=1$.  Then
\[
...
Proposition~\ref{prop:attain}): the worst placement of the $u$ largest
...
improves \eqref{eq:repaired} exactly when $r_1<2<r_u$.  For distinct roots,
Theorem~\ref{thm:twosidedsharp} and Corollary~\ref{cor:repairedsharp} say
exactly when these bounds are the infimum.  At any distinct roots above $1$
...
(Theorem~\ref{thm:converse}, Corollary~\ref{cor:charrep}), and for distinct
roots so is the value when it is not (Theorem~\ref{thm:extremal}).  Below
$2$, \eqref{eq:repaired} and \eqref{eq:twosided} hold for every $r_1>1$, and
for distinct roots Theorem~\ref{thm:twosidedsharp} and
Corollary~\ref{cor:repairedsharp} say when they are attained.
...
\item[(ii)] \emph{Repeated roots.}  The criterion is settled at repeated roots, but
Theorem~\ref{thm:extremal}, Theorem~\ref{thm:twosidedsharp} and the first
part of Corollary~\ref{cor:repairedsharp} are proved for distinct roots only:
the lifting, the pencil and the run chord all use full certificates on
distinct nodes.
\item[(iii)] \emph{The shift at run ends.}  The hypothesis of
