...
and no infimum of $b_{u+1}$ over monic multiples ever is
(Theorem~\ref{thm:noattain}).
...
extremal; the value is still a finite computation, a maximum over placements
in an explicit window of minima over finitely many zero sets
(Proposition~\ref{prop:uoneall} for $u=1$, Theorem~\ref{thm:finite} for
every $u$).
...
through Lemma~\ref{lem:infinity} rather than by estimating minors.

Lemma~\ref{lem:infinity} is qualitative, but its proof carries a rate,
because in $\mathcal C$ a ratio that tends to $0$ does so at least like
$1/x$: if $R(x)w^x\to0$ with $R\in\R(x)$, then either $w<1$, and
$|R(x)|w^x\le C_\theta\theta^x$ for every $\theta\in(w,1)$, or $w=1$ and
$\deg R<0$, so $|R(x)|\le C/x$.  Call $w$ the \emph{base} of $Rw^x$; it is
well defined, since $R_1w_1^x=R_2w_2^x$ forces $w_1=w_2$, and bases multiply.

\begin{lemma}[Descartes at infinity with a rate]\label{lem:infinityrate}
Let $\varphi_1,\ldots,\varphi_M\in\mathcal C$ satisfy
$\varphi_{j+1}/\varphi_j\to0$, let $1\le k<M$, and let $w\le1$ be the base
of $\varphi_{k+1}/\varphi_k$.  Put $\omega(n)=1/n$ if $w=1$, and
$\omega(n)=\theta^n$ for a fixed $\theta\in(w,1)$ if $w<1$.  Then there are
$x_0$ and $C$ such that for every $n\ge x_0$ and every $c\in\R^M$ for which
$f=\sum_jc_j\varphi_j$ has $k$ distinct zeros in $[n,\infty)$,
\[
  \max_{j\le k}|c_j|\le C\,\omega(n)\max_{j>k}|c_j| .
\]
\end{lemma}

\begin{proof}
Choose $x_0$ beyond every real zero and pole of the finitely many members of
$\mathcal C$ that occur below (the $\varphi_j$, their ratios, and the
families built from them in at most $k$ steps), so that all are smooth and
of constant sign on $[x_0,\infty)$.  Two bounds on $[x_0,\infty)$ are used.
First, $|\varphi_j/\varphi_1|\le C$ for every $j$, as these ratios tend to
$0$ or equal $1$.  Second, $|\varphi_j(x)/\varphi_1(x)|\le C\omega(x)$ for
$j>k$: if $w=1$ this is the remark above, since the ratio tends to $0$; if
$w<1$, write
$\varphi_j/\varphi_1=(\varphi_j/\varphi_{k+1})(\varphi_{k+1}/\varphi_k)
(\varphi_k/\varphi_1)$, where the outer factors are bounded and the middle
one has base $w$.  As $\omega$ decreases, $\sup_{x\ge n}|\varphi_j/\varphi_1|
\le C\omega(n)$ for $j>k$.

Induct on $k$, for all such families at once.  Let $e_1<\cdots<e_k$ be zeros
of $f$ in $[n,\infty)$.  If $k=1$, $f(e_1)=0$ gives
$c_1=-\sum_{j\ge2}c_j\varphi_j(e_1)/\varphi_1(e_1)$, so
$|c_1|\le(M-1)C\omega(n)\max_{j>1}|c_j|$.  If $k\ge2$, put
$\chi_j=(\varphi_j/\varphi_1)'$ for $2\le j\le M$.  By the proof of
Lemma~\ref{lem:infinity}, $\chi_j\in\mathcal C$ and
$\chi_{j+1}/\chi_j\to0$.  The base of $\chi_{k+1}/\chi_k$ is that of
$(\varphi_{k+1}/\varphi_1)/(\varphi_k/\varphi_1)=\varphi_{k+1}/\varphi_k$,
namely $w$, because differentiation keeps the base: $(Rw^x)'=(R'+R\log w)w^x$.
By Rolle's theorem $(f/\varphi_1)'=\sum_{j\ge2}c_j\chi_j$ has a zero in each
$(e_i,e_{i+1})$, so $k-1$ distinct zeros in $[n,\infty)$, and the induction
hypothesis for $\chi_2,\ldots,\chi_M$, whose cut after its $(k-1)$-st member
has base $w$ again, gives $\max_{2\le j\le k}|c_j|\le
C\omega(n)\max_{j>k}|c_j|$.  Finally $f(e_1)=0$ gives
\[
  |c_1|\le\sum_{j=2}^{k}|c_j|\Bigl|\frac{\varphi_j(e_1)}{\varphi_1(e_1)}\Bigr|
  +\sum_{j>k}|c_j|\Bigl|\frac{\varphi_j(e_1)}{\varphi_1(e_1)}\Bigr|
  \le C'\omega(n)\max_{j>k}|c_j| .
\]
\end{proof}

\begin{proposition}[The rate of far zeros]\label{prop:farrate}
In the setting of Proposition~\ref{prop:farrep}, let $E$ range over
$k$-sets of positive integers and put $n=\min E$.  There are $n_0$ and $C$,
depending on the roots and $k$, such that for $n\ge n_0$ one has
$\alpha_{J^*}(E)\ne0$ and
\[
  \eta(E)=\max_{J\ne J^*}\Bigl|\frac{\alpha_J(E)}{\alpha_{J^*}(E)}\Bigr|
  \le\frac Cn .
\]
If the cut after $\psi_k$ falls between blocks, so that $\psi_k=z_q^d$ and
$\psi_{k+1}=d^{m_{q+1}-1}z_{q+1}^d$, then $\eta(E)\le C_\theta\theta^n$ for
every $\theta\in(z_{q+1}/z_q,1)$.
\end{proposition}

\begin{proof}
The $\psi_j$ lie in $\mathcal C$ with $\psi_{j+1}/\psi_j\to0$ (proof of
Proposition~\ref{prop:farrep}(a)), and $\psi_{k+1}/\psi_k$ is $1/x$ when
the cut falls inside a block and has base $z_{q+1}/z_q<1$ when it falls
between blocks.  Let $x_0,C$ be as in Lemma~\ref{lem:infinityrate}, with
$\omega(n)\le C''/n$ in either case, and $n\ge x_0$.  A combination of
$\psi_1,\ldots,\psi_k$ vanishing on $E$ has, by the lemma with $c_j=0$ for
$j>k$, all coefficients $0$; so $\alpha_{J^*}(E)\ne0$.  For $l>k$ let
$T_{jl}$ be the entries of the matrix $T$ of the proof of
Proposition~\ref{prop:farrep}(b); then
$\psi_l-\sum_{j\le k}T_{jl}\psi_j$ vanishes on $E$, and the lemma gives
$|T_{jl}|\le C\omega(n)$.  By Cramer's rule $\alpha_J/\alpha_{J^*}$ is the
minor of $T$ on the columns $J$; the columns in $J\cap J^*$ are unit vectors,
so up to sign the minor is the determinant of the $s\times s$ block of $T$ on
the rows $J^*\setminus J$ and the columns $J\setminus J^*$, $s\ge1$, which is
at most $s!(C\omega(n))^s\le k!\,C\omega(n)$ once $C\omega(n)\le1$.
\end{proof}

\paragraph{The rate in part (c).}\label{rem:farratec}
The same bound gives a rate in Proposition~\ref{prop:farrep}(c):
$\Tail(v^{(\nu)}-v^\infty)=O(\omega(\min E_\nu))$.  Indeed $v^{(\nu)}$
vanishes on $E_\nu$ and its coefficient vector is bounded (proof of (c)), so
its first $k$ coordinates are $O(\omega)$ by Lemma~\ref{lem:infinityrate};
the others then solve the interpolation problem of $v^\infty$ at the $n$
fixed points $\{0\}\cup X_0\cup Q$ in $\mathcal H_k$ with data moved by
$O(\omega)$, and that problem has a fixed invertible matrix.  With $X_0$
empty and $k=u$, this is the rate $\tau(E_\nu)\to\tau^*$ at which the
escaping placement is approached from above.

For distinct roots every cut falls between blocks, and the proposition gives
the geometric rate with any $\theta>y_{k+1}/y_k$; the direct estimate of
the minors (a Schur polynomial against a Vandermonde product) gives it with
$\theta=y_{k+1}/y_k$ and an explicit constant.  At repeated roots the rate
$1/n$ cannot be improved when the cut falls inside a block: for $k=1$ and
$\psi_1=d^az^d$, $\psi_2=d^{a-1}z^d$ one has $\alpha_{\{2\}}/\alpha_{\{1\}}
=1/e$ at $E=\{e\}$.
...
The hypothesis of Corollary~\ref{cor:shiftrun} moves the start of each run
past its end.  Moving the end up by one instead gives a stronger hypothesis,
so the shift holds under it too.  The reduction needs a bound on three values
of a full sum.

\begin{lemma}[Three values of a full sum]\label{lem:threepoint}
Let $x_1,\ldots,x_m$ be distinct positive reals, $m\ge2$.  Let $K_0$ be a set
of $m-1$ integers, and let $R(d)=\sum_i\lambda_ix_i^d$ be nonzero with $R$
vanishing on $K_0$.  If integers $s<e$ satisfy $s,e,e+1\notin K_0$ and
$\{s+1,\ldots,e-1\}\subseteq K_0$, then
\[
  |R(e)|<|R(s)|+|R(e+1)| .
\]
\end{lemma}
...
\begin{proof}
Put $l=e-s+1\ge2$ and $\sigma=(-1)^l$.  By the zero bound $R$ has no zeros
besides $K_0$, and each is simple.  So $R(e+1)$ has the sign of $R(e)$, and
$\sigma R(s)$ does too: exactly $l-2$ zeros lie between $s$ and $e$.

For $0\le t\le1$ put $g_t(x)=\sigma x^s-tx^e+x^{e+1}$.  Take the following
$m$ functions of $x\in(0,\infty)$, in this order: first the $x^k$ with
$k\in K_0$, $k<s$, by increasing $k$; then $x^{s+1},\ldots,x^{e-1}$; then
$g_t$; and last the $x^k$ with $k\in K_0$, $k>e+1$, by increasing $k$.

\emph{The initial Wronskians are positive.}  For monomials,
\[
  W(x^{\gamma_1},\ldots,x^{\gamma_j})
  =\prod_{a<b}(\gamma_b-\gamma_a)\,x^{\gamma_1+\cdots+\gamma_j-j(j-1)/2},
\]
which is positive when $\gamma_1<\cdots<\gamma_j$.  This covers every initial
segment that stops before $g_t$.  Otherwise let $E'$ be the exponents of the
monomials in the segment and $V(E')=\prod|\gamma_b-\gamma_a|$ over pairs from
$E'$.  Expand the Wronskian along the column of $g_t$.  In the term with
$x^s$, bringing $x^s$ to its sorted position moves it past the $l-2$
exponents $s+1,\ldots,e-1$, and $(-1)^{l-2}\sigma=1$.  Hence the Wronskian is
\[
  V(E')\,x^{c}\bigl(\alpha-t\beta x^{l-1}+\delta x^l\bigr),\qquad
  \alpha=\prod_{k\in E'}|s-k|,\ \ \beta=\prod_{k\in E'}|e-k|,\ \
  \delta=\prod_{k\in E'}|e+1-k|,
\]
for some real $c$.  For $t>0$ the bracket $p$ is smallest at
$x_*=(l-1)t\beta/(l\delta)$, where $p(x_*)=\alpha-t\beta x_*^{l-1}/l$.  It is
therefore positive on $(0,\infty)$ whenever
\begin{equation}\label{eq:threeratio}
  \frac{l^l\,\alpha\,\delta^{l-1}}{(l-1)^{l-1}\beta^l}
  =\frac{l^l}{(l-1)^{l-1}}\prod_{k\in E'}
   \frac{|s-k|\,|e+1-k|^{l-1}}{|e-k|^l}>1 .
\end{equation}
The factors of \eqref{eq:threeratio} fall into three groups.
\begin{itemize}
\item[(a)] Every segment containing $g_t$ contains all of $s+1,\ldots,e-1$.
The factor for $k=s+j$ is $j(l-j)^{l-1}/(l-1-j)^l$, and the product over
$1\le j\le l-2$ telescopes to $(l-1)^{l-1}$.
\item[(b)] For $k=s-u$ below $s$, with $u\ge1$, the factor is
$f(u)=u(u+l)^{l-1}/(u+l-1)^l=(1-(l-1)a)(1+a)^{l-1}$ with $a=1/(u+l-1)$.
\item[(c)] For $k=e+1+v$ above $e+1$, with $v\ge1$, the factor is
$h(v)=(v+l)v^{l-1}/(v+1)^l=(1+(l-1)b)(1-b)^{l-1}$ with $b=1/(v+1)$.
\end{itemize}
In (b) and (c) each factor is a product of $l$ positive numbers with
arithmetic mean $1$, not all equal, so AM--GM puts it in $(0,1)$.  The $u$
are distinct and at most some $U$, and the $v$ are distinct and at most some
$V$.  Telescoping gives
\[
  \prod_{u}f(u)\ \ge\ \prod_{u=1}^Uf(u)
  =\frac{(l-1)!\,(U+l)^{l-1}}{l^{l-1}(U+1)\cdots(U+l-1)}>\frac{(l-1)!}{l^{l-1}},
  \qquad
  \prod_{v}h(v)\ \ge\ \prod_{v=1}^Vh(v)=\frac{(V+2)\cdots(V+l)}{l!\,(V+1)^{l-1}}
  >\frac1{l!}.
\]
An empty product is $1$, which exceeds either bound.  So the left side of
\eqref{eq:threeratio} is more than
$l^l\cdot(l-1)!/l^{l-1}\cdot1/l!=1$, and every initial Wronskian is positive
on $(0,\infty)$.

\emph{Zero bound for the system.}  Positive initial Wronskians bound the
zeros: a nonzero combination of the $m$ functions has at most $m-1$ zeros in
$(0,\infty)$, counted with multiplicity~\cite[Part~Five,
Ch.~1]{polya-szego}.  The proof is an induction on $m$.  If $f$ has $N$
zeros, then $(f/u_1)'=W(u_1,f)/u_1^2$ has at least $N-1$.  The functions
$W(u_1,u_j)$, $j\ge2$, have initial Wronskians $u_1^{j-2}W(u_1,\ldots,u_j)>0$.

\emph{Conclusion.}  Suppose $\Phi(t)=\sigma R(s)-tR(e)+R(e+1)$ vanished for
some $t\in[0,1]$.  Then $\Phi(t)=\sum_i\lambda_ig_t(x_i)$, and the nonzero
vector $(\lambda_i)$ would annihilate the columns of the $m\times m$ matrix of
the $m$ functions at $x_1,\ldots,x_m$.  So that matrix would be singular, and
a nonzero combination of the functions would vanish at the $m$ distinct points
$x_i$, which is impossible.  So the affine function $\Phi$ has no zero on
$[0,1]$.  Now $\Phi(0)=\sigma R(s)+R(e+1)$ has the sign of $R(e)$, so $\Phi(1)$
does too.  That is $|R(s)|-|R(e)|+|R(e+1)|>0$.
\end{proof}

\begin{lemma}[Run ends control run starts]\label{lem:runend}
Let $n=L-k-1\ge3$ and let $Z$ be a set of $n-1$ positive integers.  Let
$s<e$ be integers with $\{s,\ldots,e\}\subseteq Z$ and $e+1\notin Z$.  On
$y_{k+2},\ldots,y_L$,
\[
  \Tail(Z)\le\Tail\bigl(Z\setminus\{e\}\cup\{e+1\}\bigr)
  \quad\Longrightarrow\quad
  \Tail(Z)<\Tail\bigl(Z\setminus\{s\}\cup\{e+1\}\bigr).
\]
\end{lemma}

\begin{proof}
\emph{Two half-mass conditions.}  Apply Proposition~\ref{prop:median} and
Lemma~\ref{lem:ratio} with $k+1$ in place of $k$ to the set $C=Z\setminus\{e\}$
of $n-2$ integers.  This gives a sum $V\ne0$ on $y_{k+2},\ldots,y_L$ with
$V(0)=0$ and $V$ vanishing on $C$, and numbers $\theta_N$ strictly decreasing
in $N\notin C$, such that for $N\notin C$
\[
  \Tail(C\cup\{N\})=\sum_{d\ge1,\ d\notin C}|V(d)|\,|\theta_N-\theta_d| ;
\]
the terms with $d\in C$ are absent because $V$ vanishes there.
Since $e+1\notin Z$, the points $e$ and $e+1$ are consecutive off $C$.  On
$[\theta_{e+1},\theta_e]$ the right side is linear in $\theta$ with slope
$\sum_{d>e}|V(d)|-\sum_{1\le d\le e}|V(d)|=:h_V$.  So
\[
  \Tail(Z\setminus\{e\}\cup\{e+1\})-\Tail(Z)=-(\theta_e-\theta_{e+1})\,h_V .
\]
In the same way let $U$ be the sum for $C'=Z\setminus\{s\}$.  Its
consecutive points $s$ and $e+1$ off $C'$ bracket $s+1,\ldots,e\in C'$, and
\[
  \Tail(Z\setminus\{s\}\cup\{e+1\})-\Tail(Z)=-(\theta'_s-\theta'_{e+1})\,h_U,
  \qquad h_U=\sum_{d>e}|U(d)|-\sum_{1\le d\le s}|U(d)| .
\]
So the hypothesis says $h_V\le0$, and we must show $h_U<0$.

\emph{The difference $R$.}  The weight of $U$ on $y_{k+2}$ is nonzero.
Otherwise $U$ would be a sum on the $n-1$ nodes $y_{k+3},\ldots,y_L$ with the
$n-1$ zeros $\{0\}\cup C'$.  The same holds for $V$, so scale both to weight
$1$ on $y_{k+2}$ and put $R=V-U$.  Then $R$ is a sum on the $m=n-1\ge2$ nodes
$y_{k+3},\ldots,y_L$ and vanishes on the $m-1$ points
$K_0=\{0\}\cup(Z\setminus\{s,e\})$.  It is nonzero, since $R(s)=-U(s)\ne0$.
By the zero bound, $U$, $V$ and $R$ have exactly their prescribed zeros, all
simple.  Also $U$ and $V$ are positive at infinity, so the sign of each at a
point $d$ that is not a zero is $-1$ to the number of its zeros above $d$.
\begin{itemize}
\item[(a)] For an integer $d>e$ with $d\notin Z$, $U$, $V$ and $R$ all have
the sign $(-1)^{|Z\cap(d,\infty)|}$.  For $U$ and $V$ this is the rule above.
For $R$: $R(e)=V(e)$, and between $e$ and $d$ the function $R$ changes sign
exactly at the points of $Z\cap(e,d)$.  Hence $|V(d)|=|U(d)|+|R(d)|$.
\item[(b)] For $1\le d<s$ with $d\notin Z$, $U$ and $V$ have the sign
$-(-1)^{|Z\cap(d,\infty)|}$.  For $R$: $R(s)=-U(s)$ has the sign
$-(-1)^{|Z\cap(s,\infty)|}$, and between $d$ and $s$ the function $R$ changes
sign exactly at the points of $Z\cap(d,s)$.  So $R(d)$ has the sign
$(-1)^{|Z\cap(d,\infty)|}$, opposite to $U(d)$ and $V(d)$.  Hence
$|U(d)|=|V(d)|+|R(d)|$.
\end{itemize}
On $Z\setminus\{s,e\}$ all three vanish.  Moreover $|U(s)|=|R(s)|$,
$V(s)=0$, $|V(e)|=|R(e)|$ and $U(e)=0$.  Summing (a) and (b) gives
\[
  h_U=h_V+2|R(e)|-\sum_{d\ge1}|R(d)| .
\]
All the series converge, since the nodes lie in $(0,1)$.

\emph{Conclusion.}  Lemma~\ref{lem:threepoint} applies to $R$ with the
points $s<e$: $s,e,e+1\notin K_0$, since $s\ge1$, and $s+1,\ldots,e-1\in K_0$.
It gives
\[
  2|R(e)|-\sum_{d\ge1}|R(d)|\le|R(e)|-|R(s)|-|R(e+1)|<0,
\]
so $h_U<h_V\le0$.
\end{proof}

The converse fails.  On $y=(\tfrac13,\tfrac14,\tfrac15)$ the zero set
$\{1,2\}$ has tail $\tfrac1{24}$.  Moving the start of the run gives
$\{2,3\}$ with tail $\tfrac{25}{288}$, which is larger, but moving the end
gives $\{1,3\}$ with tail $\tfrac{35}{1128}$, which is smaller.  So the
hypothesis at run ends is the stronger one.

\begin{corollary}[The shift at run ends]\label{cor:shiftend}
In the setting of Proposition~\ref{prop:runchord},
\[
  \Tail\bigl(B_0\cup\{g\}\cup(A+1)\bigr)\le\Tail(B_0\cup A)
  \quad\text{whenever}\quad
...
Corollary~\ref{cor:shiftrun}, the left side is on $y_{k+1},\ldots,y_L$ and
the others on $y_{k+2},\ldots,y_L$.
\end{corollary}

\begin{proof}
The $a$ in question are the ends $e$ of the runs $[s,e]$ of $A$.  For a run
with $s=e$ the two neighbours coincide.  For $s<e$, Lemma~\ref{lem:runend}
with $Z=B_0\cup A$ turns the hypothesis at $e$ into the hypothesis of
Corollary~\ref{cor:shiftrun} for that run.  Here $n=|Z|+1\ge3$ and
$e+1\notin A$.  Corollary~\ref{cor:shiftrun} then gives the claim.
\end{proof}

Only Lemma~\ref{lem:threepoint} goes beyond the pencil, and it uses no
cutoff.  Its bound has room to spare.  In about $2\times10^4$ exact cases,
some with nodes above $1$, the ratio $|R(e)|/(|R(s)|+|R(e+1)|)$ stayed below
$0.79$.
...
\]
with $T$ as in Theorem~\ref{thm:valuerep}.  In particular the infima of
Propositions~\ref{prop:values} and~\ref{prop:valuesrep}, and the
bound \eqref{eq:order} whenever it is the infimum, are not attained.
\end{theorem}
...
sentence holds because each of those infima is $1/T$
(Theorems~\ref{thm:valuerep} and~\ref{thm:extremalrep}, and
Corollary~\ref{cor:charrep}).
\end{proof}

The only input is that a certificate has infinite support while a multiple
has finite degree.  So attainment is never the question; what remains is
whether a finite placement already costs the full $\tau^*$, that is whether
$T=\tau^*$ is a maximum, which for distinct roots is what the strict clause
of Theorem~\ref{thm:lift} decides in Proposition~\ref{prop:values}.
...
Neither infimum is attained (Theorem~\ref{thm:noattain}), and more:
$\tau(X)<\tau^*$ for every $X$.  Each
...
\paragraph{Strict covering.}  Keep $2\le r_1\le\cdots\le r_L$, repetitions
allowed, and the Hermite spaces $\mathcal H_k$; write $m=L-u$ and
$B=1/|P_u(1)|$.  Two strict steps carry the strict clause of
Theorem~\ref{thm:lift} to repeated roots.  The first is the deletion
step of Theorem~2.2 with its margin kept through the coalescence limit.

\begin{lemma}[Strict deletion at confluent nodes]\label{lem:deletestrict}
In the setting of Lemma~\ref{lem:coalesce} let $y_c\le\tfrac12$, put
$n=\dim\mathcal H$, and let $Z^+$ be a set of $n-1$ positive integers with
$z=\max Z^+$ and $Z'=Z^+\setminus\{z\}$.  Let $v^+\in\mathcal H$ and
$v^-\in\mathcal H_-$ be the full certificates with zero sets $Z^+$ and $Z'$,
and $G\in\mathcal H$ the element with $G|_{\{0\}\cup Z'}=0$ and $G_z=1$.  If
$Z^+\ne\{1,\ldots,n-1\}$, then
\[
  \Tail(v^+)\le\Tail(v^-)-|v^-_z|\sum_{1\le d<z}|G_d|<\Tail(v^-);
\]
if $Z^+=\{1,\ldots,n-1\}$, then $\Tail(v^+)=\tfrac{y_c}{1-y_c}\Tail(v^-)$.
\end{lemma}

\begin{proof}
For $Z^+\ne\{1,\ldots,n-1\}$ the proof of Lemma~\ref{lem:confstrict}, at
the perturbed nodes $N_\varepsilon$ of Lemma~\ref{lem:coalesce}, gives
$\Tail(v^{+,\varepsilon})\le\Tail(F^{(\varepsilon)})-\mu_\varepsilon
A_{G^{(\varepsilon)}}$, where $F^{(\varepsilon)}$ is the full certificate on
$N^-_\varepsilon$ with zero set $Z'$.  Instead of bounding
$\Tail(F^{(\varepsilon)})$ by a product, let $\varepsilon\to0$: by
Lemma~\ref{lem:coalesce}, applied to $\mathcal H$ and to $\mathcal H_-$,
$\Tail(v^{+,\varepsilon})\to\Tail(v^+)$ and
$\Tail(F^{(\varepsilon)})\to\Tail(v^-)$, while
$\mu_\varepsilon=|F^{(\varepsilon)}_z|\to|v^-_z|$ and the finite sums
$A_{G^{(\varepsilon)}}$ tend to $\sum_{1\le d<z}|G_d|$.  That margin is
positive exactly as in Lemma~\ref{lem:confstrict}: $v^-_z\ne0$ by the
zero bound, and $G$ vanishes only on $\{0\}\cup Z'$ while $|Z'|=n-2<z-1$.
For the consecutive set, Lemma~2.1 with repetitions (the paragraph after
Lemma~\ref{lem:improve}) gives the two tails as $1/|Q(1)|$ for the monic
polynomials $Q$ of the two multisets of roots, which differ by the factor
$x-1/y_c$.
\end{proof}

The second step handles a placement at the first gap of a certificate's zero
set, where the run chord is an equality at $r=2$.

\begin{lemma}[The first gap]\label{lem:firstgap}
Let $0\le k<L-1$, $n=\dim\mathcal H_{k+1}=L-k-1$, and let $F$ be a full
certificate on $\mathcal H_{k+1}$ whose zero set $Z$ is not
$\{1,\ldots,n-1\}$, with leading run $f$.  Then some certificate on
$\mathcal H_k$ vanishing on $\{1,\ldots,f+1\}$ has tail less than
$\Tail(F)$.
\end{lemma}

\begin{proof}
Let $y=1/r_{k+1}\le\tfrac12$ be the node that $\mathcal H_k$ has in addition,
and $\Pi_k(x)=\prod_{i>k}(1-x/r_i)$.  The elements of $\mathcal H_k$ are the
sequences with generating function $N/\Pi_k$, $\deg N\le n$, and
$\Pi_k=(1-yx)\Pi_{k+1}$.  With $F(x)=\sum_dF_dx^d$, as in the paragraph after
Theorem~\ref{thm:extremalrep} put
\[
  v(x)=\frac{1-yxF(x)}{1-yx}\in\mathcal H_k,\qquad
  v_0=1,\quad v_d=-\sum_{1\le j<d}F_jy^{d-j}\ (d\ge1).
\]
Since $F_1=\cdots=F_f=0$, $v$ vanishes on $\{1,\ldots,f+1\}$, and
\[
  \Tail(v)\le\sum_{j\ge1}|F_j|\sum_{d>j}y^{d-j}=\frac{y}{1-y}\Tail(F)
  \le\Tail(F),
\]
the first inequality being strict as soon as the $F_j$, $j\ge1$, take both
signs, since then some $v_d$ sums terms of both signs.  So
$\Tail(v)<\Tail(F)$ unless $y=\tfrac12$ and $F_j$ has one sign $\sigma'$ for
all $j\ge1$ with $F_j\ne0$; assume this.  Then $v_d$ has the sign
$\sigma=-\sigma'$ for every $d\ge f+2$, as $F_{f+1}\ne0$.  As
$Z\ne\{1,\ldots,n-1\}$, $f\le n-2$, so $h=x^{f+2}/\Pi_k$ lies in
$\mathcal H_k$; it vanishes at $0,\ldots,f+1$, and
$\sum_{d\ge0}h_d=1/\Pi_k(1)>0$.  We claim $|h_d|\le C|v_d|$ for $d\ge f+2$.
Let $e\ge1$ be the multiplicity of $y$ among $r_{k+1},\ldots,r_L$; all
nodes are at most $y$, so $|h_d|\le C(1+d)^{e-1}y^d$.  If $e=1$,
$|v_d|\ge|F_{f+1}|y^{d-f-1}$.  If $e\ge2$, $y$ is the largest node of
$\mathcal H_{k+1}$ with multiplicity $e-1$, and the coordinate $\omega$ of $F$
on $d^{e-2}y^d$ is nonzero, for otherwise $F$ would lie in $\mathcal H_{k+2}$,
of dimension $n-1$, with the $n-1$ zeros $Z$; so $|F_j|\ge c\,j^{e-2}y^j$ for
large $j$, and $|v_d|=\sum_{j<d}|F_j|y^{d-j}\ge c'd^{e-1}y^d$ for large $d$.
As $v_d\ne0$ for all $d\ge f+2$, the claim follows.  For $|t|<1/C$ every
$v_d+th_d$, $d\ge f+2$, keeps the sign $\sigma$, so
\[
  \Tail(v+th)=\sigma\sum_{d\ge f+2}(v_d+th_d)
  =\Tail(v)+\frac{t\sigma}{\Pi_k(1)} ,
\]
and $t=-\sigma/(2C)$ gives a certificate $v+th$ on $\mathcal H_k$, vanishing
on $\{1,\ldots,f+1\}$, with tail below $\Tail(v)\le\Tail(F)$.
\end{proof}

\begin{theorem}[Strict covering at repeated roots]\label{thm:strictrep}
Let $2\le r_1\le\cdots\le r_L$, repetitions allowed, and $1\le u\le L-1$.  If
\eqref{eq:partial} fails, or $r_u>2$, then $\tau(X)<\tau^*$ for every set
$X$ of $u$ positive integers.
\end{theorem}

\begin{proof}
Let $x=\min X$.  Theorem~\ref{thm:extremalrep} chains
$\tau(X)=\tau_0(X)\le\tau_1(\cdot)\le\cdots\le\tau_{u-1}(\{x\})$, each step
deleting the largest remaining element, so it suffices to show
$\tau_{u-1}(\{x\})<\tau_u(\varnothing)=\tau^*$.  Let $F$ be a full
minimiser for $\tau^*$ on $\mathcal H_u$ (Proposition~\ref{prop:attainrep}),
with zero set $Z^*$ of size $m-1$, $g=\max Z^*$ ($g=0$ if $m=1$) and leading
run $f$.  If $Z^*=\{1,\ldots,m-1\}$ then $\tau^*=\Tail(F)=B$, which forces
\eqref{eq:partial} (when it fails, Lemma~\ref{lem:improve} with
repetitions gives $\tau^*<B$), and hence $r_u>2$.  Apply
Lemma~\ref{lem:deletestrict} to $\mathcal H=\mathcal H_{u-1}$, whose largest
node is $1/r_u$, and $\mathcal H_-=\mathcal H_u$.

\emph{$x\in Z^*$ or $x>g$.}  Put $Z^+=Z^*\cup\{g+1\}$ in the first case and
$Z^+=Z^*\cup\{x\}$ in the second.  The full certificate $v^+$ on
$\mathcal H_{u-1}$ with zero set $Z^+$ vanishes at $x$, and
$Z^+\setminus\{\max Z^+\}=Z^*$, so Lemma~\ref{lem:deletestrict} gives
$\Tail(v^+)<\Tail(F)$: either $Z^+$ is not consecutive, or it is, and then
$Z^*=\{1,\ldots,m-1\}$, $r_u>2$, and the factor is
$1/(r_u-1)<1$.

\emph{$x\notin Z^*$ and $x<g$, with $Z^*\cap(0,x)\ne\{1,\ldots,x-1\}$.}  Put
$B_0=Z^*\cap(0,x)$ and $A=Z^*\cap(x,\infty)\ne\varnothing$.  By
Proposition~\ref{prop:runchordrep} with $k=u-1$ and $g$ replaced by $x$,
which is strict because $B_0\ne\{1,\ldots,x-1\}$,
$\Tail(F)>\sum_j\lambda_j\Tail(F_j)+\mu\Tail(w)$, and every neighbour $F_j$
is a certificate for $\varnothing$ on $\mathcal H_u$, so
$\Tail(F_j)\ge\tau^*=\Tail(F)$.  Hence $\Tail(w)<\Tail(F)$, and $w$ vanishes
at $x$.

\emph{$x\notin Z^*$, $x<g$ and $Z^*\cap(0,x)=\{1,\ldots,x-1\}$.}  Then
$x=f+1$ and $Z^*\ne\{1,\ldots,m-1\}$, and Lemma~\ref{lem:firstgap} with
$k=u-1$ gives a certificate on $\mathcal H_{u-1}$ vanishing at $x$ with tail
below $\Tail(F)$.
\end{proof}

When \eqref{eq:partial} holds and $r_u=2$ a placement can cost $\tau^*$
exactly.  Put $Q_k=(x-2)^{u-k}P_u$; if $r_1=\cdots=r_u=2$ this is
$\prod_{i>k}(x-r_i)$, and $|Q_k(1)|=|P_u(1)|$.

\begin{theorem}[When a placement costs $\tau^*$]\label{thm:attainchar}
Let $2\le r_1\le\cdots\le r_L$, repetitions allowed, $1\le u\le L-1$, and
$P=\prod_i(x-r_i)$.  Some set $X$ of $u$ positive integers has
$\tau(X)=\tau^*$ if and only if $r_u=2$ and
\begin{equation}\label{eq:partialP}
  |S_j(P)|\le|P(1)|\qquad(1\le j\le m-1),
\end{equation}
and then $X=\{m,m+1,\ldots,L-1\}$ is the only one.
\end{theorem}

\begin{proof}
For a monic $R$ with roots above $1$, the proof of
Theorem~\ref{thm:converse} gives
$|S_j((x-2)R)|=|S_j(R)|+2|S_{j-1}(R)|$ for $1\le j\le\deg R$ and
$|((x-2)R)(1)|=|R(1)|$.  So \eqref{eq:partialP} implies \eqref{eq:partial},
and if $r_u>2$ or \eqref{eq:partial} fails, Theorem~\ref{thm:strictrep}
leaves no such $X$.  Assume now $r_1=\cdots=r_u=2$ and \eqref{eq:partial};
then $\tau^*=B$ (proof of Corollary~\ref{cor:charrep}), and every
$\tau_k(X)\le B$ by the chain of Theorem~\ref{thm:extremalrep}.  Write
$\mathcal A(R)$ as in the proof of Theorem~\ref{thm:converse}.  The same
identity gives $\deg R\in\mathcal A((x-2)R)$, since
$|S_{\deg R}((x-2)R)|=|R(1)|+2|S_{\deg R-1}(R)|$, and it puts $j,j+1$ in
$\mathcal A((x-2)R)$ for $j\in\mathcal A(R)$; so
$\mathcal A(Q_k)\supseteq\{m,\ldots,L-k-1\}$.  We show by descending
induction on $k\le u$: for every set $X$ of $u-k$ positive integers,
$\tau_k(X)=B$ if and only if $X=\{m,\ldots,L-k-1\}$ and
\eqref{eq:partialP} holds with $Q_k$ in place of $P$.  At $k=u$ this is
$\tau^*=B$ and \eqref{eq:partial}.  Let $k<u$.

\emph{$X\subseteq\{1,\ldots,L-k-1\}$.}  If $X\ne\{m,\ldots,L-k-1\}$, some
$j\in\{m,\ldots,L-k-1\}\subseteq\mathcal A(Q_k)$ lies outside $X$, as both
sets have $u-k$ elements; if $X=\{m,\ldots,L-k-1\}$ and the condition fails
for $Q_k$, some $j\le m-1$ lies in $\mathcal A(Q_k)\setminus X$.  Either
way Lemma~\ref{lem:improve} with repetitions, for $Q_k$ and $X_0=X$,
gives $\tau_k(X)<1/|Q_k(1)|=B$.  If $X=\{m,\ldots,L-k-1\}$ and the condition
holds, put $W(y)=\widetilde Q_k(y)/(1-y)$: its coefficients are $w_0=1$,
$w_d=S_d(Q_k)$, and $w_d=Q_k(1)$ for $d\ge L-k$, so $|w_d|\le|Q_k(1)|$ for
every $d\ge1$ outside $X$.  Since $\widetilde Q_k$ divides $W$,
$\sum_dw_dv_d=0$ for every $v\in\mathcal H_k$ (proof of
Corollary~\ref{cor:charrep}), and for a certificate for $X$ this gives
$1\le|Q_k(1)|\Tail(v)$.  So $\tau_k(X)\ge B$, hence $=B$.

\emph{$\max X>L-k-1$.}  Put $X'=X\setminus\{\max X\}$.  By
Theorem~\ref{thm:extremalrep}, $\tau_k(X)\le\tau_{k+1}(X')$, which by
induction is below $B$ unless $X'=\{m,\ldots,L-k-2\}$.  In that case the
consecutive full certificate $F$ on $\mathcal H_{k+1}$ with zero set
$\{1,\ldots,L-k-2\}\supseteq X'$ has tail $1/|Q_{k+1}(1)|=B$ by Lemma~2.1, and
$Z^+=\{1,\ldots,L-k-2\}\cup\{\max X\}$ is not consecutive, so
Lemma~\ref{lem:deletestrict} gives a certificate for $X$ on $\mathcal H_k$
with tail below $B$.  Such $X$ are not $\{m,\ldots,L-k-1\}$, as claimed.

At $k=0$, $Q_0=P$ and the claim is the theorem.
\end{proof}

\begin{corollary}\label{cor:valuesrepstrict}
At $(2,3,3,5)$ and $(2,3,3,3)$ with $u=1$, every placement has
$\tau(\{x\})<\tau^*$, so every monic multiple has $b_2>24$, resp.\
$b_2>\tfrac{27}2$, already because of its placement.
\end{corollary}

\begin{proof}
\eqref{eq:partial} fails at both (partial sums $1,-10,29,-16$ and
$1,-8,19,-8$), so Theorem~\ref{thm:strictrep} applies; the covering is
\eqref{eq:weak}.
\end{proof}

At $(2,4,5)$ with $u=1$, by contrast, \eqref{eq:partialP} reads
$|S_1(P)|=10\le12=|P(1)|$, and $\tau(\{2\})=\tau^*=\tfrac1{12}$; the
infimum $12$ is still not attained, by Theorem~\ref{thm:noattain}.  The
condition \eqref{eq:partialP} holds at $(2,3)$, $(2,6,6)$, $(2,7,8,9)$ and
$(2,2,4,5)$ with $u=2$, and fails at $(2,3,4)$ and $(2,5,6,7)$, where
\eqref{eq:partial} holds.
...
cutoff Proposition~\ref{prop:step} can therefore not be applied as is;
Theorem~\ref{thm:finite} below replaces the search by a finite computation
for every $u$.

\subsection*{The value below the cutoff is a finite computation}

Theorem~\ref{thm:value} gives the infimum as $1/T$ at all distinct roots
above $1$, and below the cutoff $T$ can exceed $\tau^*$.  We show that $T$ is
nevertheless a maximum over placements in an explicit window of minima over
finitely many zero sets, every one of which is an exact finite sum plus a
geometric tail.  Placements that leave the window are handled by
Proposition~\ref{prop:farrep}: a block of $j$ exempt coefficients that
escapes costs $\tau_j$ of what is left, and once the block is far enough out
it costs strictly less.  The one-node pencil of Lemma~\ref{lem:pencil}
drives both steps, and the far zeros enter only through a perturbation
estimate for generalized Vandermonde determinants.

Throughout, $1<r_1<\cdots<r_L$ are distinct, $y_i=1/r_i$, and for an
exponential sum $H$ and a set $I\subseteq\N_{>0}$ we write
$\mu_H(I)=\sum_{d\in I}|H_d|$, so that $\Tail(H)=\mu_H([1,\infty))$.  Put
\[
  q=\max_{1\le i<L}\frac{y_{i+1}}{y_i}<1,\qquad
  C_n=\prod_{1\le a<b\le n}\bigl(1-q^{b-a}\bigr),\qquad
  K=\frac{(L-1)!}{C_{L-1}^2},
\]
let $W_p=\{y_{L-p},\ldots,y_L\}$ be the $p+1$ smallest nodes, and
\[
  \Gamma=\max_{1\le p\le L-1}\ \sum_{\nu\in W_p}\
  \prod_{\mu\in W_p\setminus\{\nu\}}\frac{1+\mu}{|\nu-\mu|}.
\]
Let $\Delta$ be the least integer $\Delta\ge L$ with
\begin{equation}\label{eq:Delta}
  \frac{y_1^{\Delta}}{1-y_1}\le\frac1{3\Gamma},\qquad
  Kq^{\Delta+1}\le\frac12,\qquad
  Kq^{\Delta+1}\Bigl(8+\frac{4(L-1)}{y_L}\Bigr)\le\frac{1-y_1}{2\Gamma}.
\end{equation}
It depends on the roots alone; at $(\tfrac{11}{10},3,5,7)$ it is $111$, at
$(\tfrac65,\tfrac32,3,5)$ it is $103$ and at $(\tfrac32,3,5,7)$ it is $64$.

\begin{lemma}[The pencil step]\label{lem:pencilstep}
In the notation of Lemma~\ref{lem:pencil}, let $N\ge1$ with $N\notin A$ and
$\theta_N=|F_N|/|H_N|$.  Then
\[
  \Tail\bigl(A\cup\{N\}\bigr)-\Tail(F)
  =\theta_N\bigl(\mu_H((N,\infty))-\mu_H([1,N])\bigr)
   -2\sum_{d>N}|F_d| ,
\]
so $\Tail(A\cup\{N\})<\Tail(F)$ whenever
$\mu_H((N,\infty))\le\mu_H([1,N])$.  If the full certificate with zero set
$A\cup\{N\}$ has the least tail among the certificates on
$y_{k+1},\ldots,y_L$ vanishing on $A$, then
$\mu_H([N,\infty))\ge\mu_H([1,N))$.
\end{lemma}

\begin{proof}
Put $\rho_d=|F_d|/|H_d|$ for $d\ge1$ outside $A$.  By
Lemma~\ref{lem:ratio} $\rho_d>\theta_N$ for $d<N$ and $\rho_d<\theta_N$ for
$d>N$, so \eqref{eq:median} gives
\[
  \Tail\bigl(A\cup\{N\}\bigr)
  =\sum_{d<N}\bigl(|F_d|-\theta_N|H_d|\bigr)
   +\sum_{d>N}\bigl(\theta_N|H_d|-|F_d|\bigr),
\]
the sums over $d\ge1$ outside $A$, where $H$ vanishes anyway.  Subtracting
$\Tail(F)=\sum_{d<N}|F_d|+\theta_N|H_N|+\sum_{d>N}|F_d|$ gives the identity.
The last sum is positive, since $F$ has finitely many zeros.  For the second
claim, the certificates vanishing on $A$ with $tH$ opposing $F$ have tail
$\varphi(\theta)=\sum_d|H_d|\,|\theta-\rho_d|$ by the proof of
Proposition~\ref{prop:median}, and for $\theta$ slightly above $\theta_N$,
\[
  \varphi(\theta)-\varphi(\theta_N)
  =(\theta-\theta_N)\bigl(\mu_H([N,\infty))-\mu_H([1,N))\bigr),
\]
because $\rho_d\le\theta_N$ exactly for $d\ge N$.  Minimality at
$\theta_N$ makes the bracket nonnegative.
\end{proof}

The next lemma is the quantitative form of Proposition~\ref{prop:farrep}(a)
for the pencil: zeros placed far out, together with as many new top nodes,
barely change it.  Fix a set $P_0\ni0$ of $p\le L-1$ nonnegative integers
with $x=\max P_0$, put $W=W_p$ with largest node $w_0=y_{L-p}$, and let $Q$
be the sum on $W$ vanishing on $P_0$ with coefficient $1$ on $w_0$.  For
$0\le s\le L-1-p$ let $Z=\{y_{L-p-s},\ldots,y_{L-p-1}\}$ be the $s$ nodes
just above $W$, and for a set $E$ of $s$ integers above $x$ let $H$ be the
sum on $Z\cup W$ vanishing on $P_0\cup E$ with coefficient $1$ on $w_0$.
Both exist and are unique: a sum on $p$ (respectively $p+s$) of the nodes
vanishing at $p$ (respectively $p+s$) points is zero by the zero bound, so
the coefficient on $w_0$ cannot vanish and the remaining coefficients solve a
nonsingular system.  Write $Q=\sum_{\nu\in W}q_\nu\nu^d$,
$H=\sum_{\nu\in Z\cup W}h_\nu\nu^d$ and $S_Q=\sum_{\nu\in W}|q_\nu|\nu$.

\begin{lemma}[Far zeros barely move the pencil]\label{lem:laplace}
In this setting $S_Q\le\Gamma\,\mu_Q([1,p+1])$.  If $s\ge1$, $e_1=\min E$
and $\rho=Kq^{e_1-x}\le\frac12$, then
\[
  \sum_{d\ge1}|H_d-Q_d|
  \le\frac{S_Q}{1-y_1}\Bigl(4\rho+\frac{2(L-1)Kq^{e_1}}{y_L}\Bigr).
\]
\end{lemma}

\begin{proof}
\emph{The mass of $Q$.}  Put $c_\nu=q_\nu\nu$, so that
$Q_d=\sum_\nu c_\nu\nu^{d-1}$.  The Lagrange polynomial
$\ell_\nu(t)=\prod_{\mu\ne\nu}(t-\mu)/(\nu-\mu)=\sum_i\ell_{\nu,i}t^i$ of the
nodes $W$ satisfies $c_\nu=\sum_{i=0}^{p}\ell_{\nu,i}Q_{i+1}$, and its
coefficients have absolute sum at most $\prod_{\mu\ne\nu}(1+\mu)/|\nu-\mu|$.
Summing over $\nu$ gives $S_Q\le\Gamma\max_{1\le d\le p+1}|Q_d|
\le\Gamma\mu_Q([1,p+1])$.

\emph{Determinants.}  For a set $R$ of distinct nonnegative integers and a
set $S$ of $|R|$ nodes let $D(R,S)=\det(\sigma^r)$, and let $m(R,S)$ be the
product of $\sigma^r$ over the pairing of the $t$th largest node with the
$t$th largest exponent.  Every term of the determinant is at most $m(R,S)$
by the rearrangement inequality, so $|D(R,S)|\le|S|!\,m(R,S)$.  Conversely
$D(R,S)$ is, up to sign, a Schur polynomial, which has nonnegative
coefficients and contains the monomial of the pairing above, times the
Vandermonde product $\prod_{a<b}(\sigma_a-\sigma_b)$ (nodes decreasing); so
$|D(R,S)|\ge c(S)\,m(R,S)$ with $c(S)=\prod_{a<b}(1-\sigma_b/\sigma_a)$.
Consecutive nodes of any subset of our list have ratio at most $q$, so
$c(S)\ge C_{|S|}\ge C_{L-1}$.

\emph{The monomial comparison.}  Let $Y$ be a set of $p+s$ nodes with $s$
largest $Z_Y$ and $p$ smallest $B_Y$, and let $S\subseteq Y$ have $s$
elements, $S^c=Y\setminus S$.  The $t$th largest of $S$ is at most that of
$Z_Y$, the $t$th largest of $S^c$ at least that of $B_Y$, and
$\prod S\prod S^c=\prod Z_Y\prod B_Y$.  Since every exponent of $E$ is at
least $e_1$ and every exponent of $P_0$ at most $x$,
\begin{equation}\label{eq:monocomp}
  \frac{m(E,S)\,m(P_0,S^c)}{m(E,Z_Y)\,m(P_0,B_Y)}
  \le\Bigl(\frac{\prod S}{\prod Z_Y}\Bigr)^{e_1}
     \Bigl(\frac{\prod S^c}{\prod B_Y}\Bigr)^{x}
  =\Bigl(\frac{\prod S}{\prod Z_Y}\Bigr)^{e_1-x}\le q^{e_1-x}
\end{equation}
when $S\ne Z_Y$, as some element of $S$ is then at most $q$ times the
corresponding element of $Z_Y$.

\emph{Cramer and Laplace.}  Let $N'=(Z\cup W)\setminus\{w_0\}$ and
$W'=W\setminus\{w_0\}$.  The coefficients $h_\nu$, $\nu\in N'$, solve the
system with matrix $\mathsf M=(\nu^r)_{r\in P_0\cup E,\ \nu\in N'}$ and
right-hand side $(-w_0^r)$, and the $q_\nu$, $\nu\in W'$, the system with
matrix $\mathsf M_0=(\nu^r)_{r\in P_0,\ \nu\in W'}$.  By Cramer's rule
$h_\nu=-\det\mathsf M^{(\nu)}/\det\mathsf M$ and
$q_\nu=-\det\mathsf M_0^{(\nu)}/\det\mathsf M_0$, where ${}^{(\nu)}$
replaces the column of $\nu$ by that of $w_0$.  Expand $\det\mathsf M$ and
$\det\mathsf M^{(\nu)}$ along the $s$ rows $E$: each is a signed sum, over the
$s$-sets $S$ of columns, of $D(E,S)$ times the complementary minor on the
rows $P_0$, the sign depending only on the positions.

For $\det\mathsf M$ ($Y=N'$, $Z_Y=Z$, $B_Y=W'$) the term $S=Z$ is
$\pm D(E,Z)\det\mathsf M_0$, and by \eqref{eq:monocomp} and the two
determinant bounds every other term is at most
\[
  \frac{s!\,p!\,q^{e_1-x}}{c(Z)\,c(W')}\,\bigl|D(E,Z)\det\mathsf M_0\bigr| ;
\]
there are $\binom{p+s}{s}-1$ of them, $(p+s)!\le(L-1)!$ and
$c(Z)c(W')\ge C_{L-1}^2$, so
\[
  \det\mathsf M=\pm D(E,Z)\det\mathsf M_0\,(1+\epsilon_0),
  \qquad|\epsilon_0|\le\rho .
\]
For $\nu\in W'$ ($Y=(Z\cup W)\setminus\{\nu\}$,
$Z_Y=Z$, $B_Y=W\setminus\{\nu\}$) the term $S=Z$ is
$\pm D(E,Z)\det\mathsf M_0^{(\nu)}$ with the same sign, and
$\det\mathsf M_0^{(\nu)}=\pm D(P_0,W\setminus\{\nu\})\ne0$; the same count
gives the same form with some $|\epsilon_\nu|\le\rho$.  As the signs agree,
\[
  h_\nu=q_\nu\frac{1+\epsilon_\nu}{1+\epsilon_0},\qquad
  |h_\nu-q_\nu|\le|q_\nu|\frac{2\rho}{1-\rho}\le4\rho|q_\nu|
  \qquad(\nu\in W').
\]
For $\nu\in Z$ no term of $\det\mathsf M^{(\nu)}$ has $S=Z$.  Its columns are
the nodes $Y=(Z\cup W)\setminus\{\nu\}$, whose $p$ smallest are $W'$, and
now $\prod S\prod S^c=\prod Z\prod W'\cdot w_0/\nu$ while
$\prod S\le\prod Z\cdot w_0/\nu$.  The argument of \eqref{eq:monocomp}
bounds each term's monomial by
$(\prod S/\prod Z)^{e_1-x}(w_0/\nu)^x\,m(E,Z)m(P_0,W')\le
q^{e_1}m(E,Z)m(P_0,W')$, so
\[
  |h_\nu|\le\binom{p+s}{s}\frac{s!\,p!\,q^{e_1}}{c(Z)c(W')(1-\rho)}
  \le2Kq^{e_1}\qquad(\nu\in Z).
\]
Finally, as $h_{w_0}=q_{w_0}=1$, $\nu/(1-\nu)\le1/(1-y_1)$ and
$S_Q\ge w_0\ge y_L$,
\[
  \sum_{d\ge1}|H_d-Q_d|\le\sum_{\nu\in W'}|h_\nu-q_\nu|\frac{\nu}{1-\nu}
  +\sum_{\nu\in Z}|h_\nu|\frac{\nu}{1-\nu}
  \le\frac{4\rho S_Q+2sKq^{e_1}}{1-y_1},
\]
and $s\le L-1$, $1\le S_Q/y_L$.
\end{proof}

\begin{lemma}[Mass criterion]\label{lem:masscrit}
In the setting of Lemma~\ref{lem:laplace}, let $M\ge x+\Delta$.  For every
$0\le s\le L-1-p$, every set $E$ of $s$ integers above $M$ and every $N>M$,
\[
  \mu_H([N,\infty))<\mu_H([1,N)).
\]
\end{lemma}

\begin{proof}
Let $\varepsilon=\sum_{d\ge1}|H_d-Q_d|$, which is $0$ if $s=0$.  Otherwise
$e_1-x\ge\Delta+1$, so $\rho\le Kq^{\Delta+1}\le\frac12$ by \eqref{eq:Delta},
and Lemma~\ref{lem:laplace} with $q^{e_1}\le q^{e_1-x}$ and the third
condition of \eqref{eq:Delta} gives $2\varepsilon\le S_Q/(2\Gamma)$.  Next,
$w_0\le y_1$ and $M\ge\Delta$ give
\[
  \mu_Q((M,\infty))\le\sum_{\nu\in W}|q_\nu|\frac{\nu^{M+1}}{1-\nu}
  \le\frac{w_0^{M}}{1-w_0}S_Q\le\frac{S_Q}{3\Gamma},
\]
while $M\ge\Delta\ge L\ge p+1$ and Lemma~\ref{lem:laplace} give
$\mu_Q([1,M])\ge S_Q/\Gamma$.  As $N>M$,
\[
  \mu_H([1,N))-\mu_H([N,\infty))
  \ge\mu_Q([1,M])-\mu_Q((M,\infty))-2\varepsilon
  \ge\frac{S_Q}{\Gamma}\Bigl(1-\frac13-\frac12\Bigr)>0 .
\]
\end{proof}

The pencils met below are all of this form.  If $F$ is a full certificate
on $y_{k+2},\ldots,y_L$ with zero set $A$, its pencil $H$ of
Lemma~\ref{lem:pencil} lives on the $L-k$ smallest nodes and vanishes on
$\{0\}\cup A$; splitting $A$ into the part $A_0$ at most some $x$ and the
part $E$ above it puts $H$ in the setting of Lemma~\ref{lem:laplace} with
$P_0=\{0\}\cup A_0$ and $s=|E|$, since $p+1+s=|A|+2=L-k$.

\begin{lemma}[Zero sets of minimisers]\label{lem:zerobound}
Let $0\le k<L$, let $X$ be a set of at most $L-k-1$ positive integers, and
$x_0=\max(X\cup\{0\})$.  Every full certificate on $y_{k+1},\ldots,y_L$
attaining $\tau_k(X)$ has its zero set in
$\{1,\ldots,x_0+(L-k-1-|X|)\Delta\}$.  In particular $\tau_k(X)$ is the least
tail of the finitely many full certificates on $y_{k+1},\ldots,y_L$ whose zero
sets contain $X$ and lie in that window.
\end{lemma}

\begin{proof}
Let $v$ attain $\tau_k(X)$ with zero set $A\supseteq X$, $|A|=L-k-1$, and
put $n=|A\setminus X|$ and $M_i=x_0+i\Delta$.  If each of the $n$ windows
$(M_i,M_{i+1}]$, $0\le i<n$, meets $A$, all $n$ elements of $A\setminus X$
lie in $(M_0,M_n]$ and we are done.  Otherwise let the window $(M_i,M_{i+1}]$
miss $A$, put $A_0=A\cap[1,M_i]\supseteq X$ and $A_1=A\cap(M_{i+1},\infty)$,
and suppose $A_1\ne\varnothing$.  Its largest element $a^*=\max A$ exceeds
$x_0$, so $a^*\notin X$.  Let $F'$ be the full certificate on
$y_{k+2},\ldots,y_L$ with zero set $A\setminus\{a^*\}$ and $H'$ its pencil.
Every certificate vanishing on $A\setminus\{a^*\}\supseteq X$ is one for $X$,
so $v$ has the least tail among them, and Lemma~\ref{lem:pencilstep} gives
$\mu_{H'}([a^*,\infty))\ge\mu_{H'}([1,a^*))$.  But $H'$ is the sum of
Lemma~\ref{lem:laplace} with $P_0=\{0\}\cup A_0$, whose maximum is at most
$M_i$, and $E=A_1\setminus\{a^*\}$, above $M_{i+1}=M_i+\Delta$; so
Lemma~\ref{lem:masscrit} with $M=M_{i+1}$ and $N=a^*$ gives the reverse
strict inequality.  Hence $A_1=\varnothing$ and $A\subseteq[1,M_i]$.  The
last sentence follows from Lemma~\ref{lem:attainX}.
\end{proof}

\begin{lemma}[Far blocks]\label{lem:farblock}
Let $1\le j\le L-1$, let $X'$ be a set of at most $L-1-j$ positive integers,
$x_0=\max(X'\cup\{0\})$, and let $E$ be a set of $j$ integers above
$x_0+(L-j-|X'|)\Delta$.  Then $\tau(X'\cup E)<\tau_j(X')$.
\end{lemma}

\begin{proof}
By Lemmas~\ref{lem:attainX} and~\ref{lem:zerobound}, $\tau_j(X')$ is
attained by a full certificate $F^0$ on $y_{j+1},\ldots,y_L$ with zero set
$A\supseteq X'$ and $\max A\le x_0+(L-j-1-|X'|)\Delta=:M-\Delta$.  Let
$e_1<\cdots<e_j$ be the elements of $E$, all above $M$.  For
$0\le s<j$ let $F^{s+1}$ be the full certificate on $y_{j-s},\ldots,y_L$
with zero set $A\cup\{e_1,\ldots,e_{s+1}\}$.  It is obtained from $F^s$ by
adding the node $y_{j-s}$ and the zero $e_{s+1}\notin A\cup\{e_1,\ldots,e_s\}$,
and the pencil of that step is the sum of Lemma~\ref{lem:laplace} with
$P_0=\{0\}\cup A$, $p=L-j$, and the $s$ far zeros $e_1,\ldots,e_s$.  By
Lemma~\ref{lem:masscrit} with $N=e_{s+1}$ its mass satisfies
$\mu_H((N,\infty))\le\mu_H([N,\infty))<\mu_H([1,N))\le\mu_H([1,N])$, so
Lemma~\ref{lem:pencilstep} gives $\Tail(F^{s+1})<\Tail(F^s)$.  After $j$
steps $F^j$ is a certificate on all $L$ nodes for $X'\cup E$, and
$\tau(X'\cup E)\le\Tail(F^j)<\Tail(F^0)=\tau_j(X')$.
\end{proof}

\begin{lemma}[Escaped blocks are limits]\label{lem:escape}
Let $0\le j\le u\le L-1$ and let $X'$ be a set of $u-j$ positive integers.
Then $T\ge\tau_j(X')$.
\end{lemma}

\begin{proof}
For $j=0$ this is the definition of $T$.  For $j\ge1$ put
$E_\nu=\{\nu+1,\ldots,\nu+j\}$; it suffices that
$\limsup_\nu\tau(X'\cup E_\nu)\ge\tau_j(X')$.  This is the second half of the
proof of Theorem~\ref{thm:value} with $X'$ carried along.  If
$\tau(X'\cup E_\nu)\le\tau_j(X')-\varepsilon$ for all large $\nu$, take
certificates $v^\nu=\sum_i\lambda^\nu_iy_i^d$ for $X'\cup E_\nu$ with tails at
most $\tau_j(X')-\varepsilon/2$.  Their values at $L$ fixed points of
$\{0,1,\ldots\}\setminus X'$, one of them $0$, are bounded once $E_\nu$ lies
beyond those points, so the $\lambda^\nu$ are bounded and a subsequence
converges to some $\lambda$.  The equations $v^\nu_e=0$, $e\in E_\nu$, read
$A_\nu\lambda^\nu_{\le j}=-R_\nu\lambda^\nu_{>j}$, and the entries of
$A_\nu^{-1}R_\nu$ are ratios $\alpha_J/\alpha_{J^*}$ of
Proposition~\ref{prop:farrep}(b) with $k=j$, which tend to $0$; so
$\lambda_{\le j}=0$.  The limit is then a certificate for $X'$ on
$y_{j+1},\ldots,y_L$ with tail at most $\tau_j(X')-\varepsilon/2$, by
Fatou's lemma, which is absurd.
\end{proof}

\begin{theorem}[The value below the cutoff is finite]\label{thm:finite}
Let $1<r_1<\cdots<r_L$ and $0\le u\le L-1$, and put $N=u(L-u)\Delta$ with
$\Delta$ as in \eqref{eq:Delta}.  Then
\[
  T=\max\Bigl\{\tau_j(X'):\ 0\le j\le u,\ X'\subseteq\{1,\ldots,N\},\
  |X'|=u-j\Bigr\},
\]
where each $\tau_j(X')$ is the least tail of the full certificates on
$y_{j+1},\ldots,y_L$ whose zero sets $A$ satisfy
$X'\subseteq A\subseteq\{1,\ldots,\max(X'\cup\{0\})+(L-1-u)\Delta\}$.  So
\[
  \inf_Qb_{u+1}\Bigl(Q\prod_{i=1}^L(x-r_i)\Bigr)=\frac1T
\]
is a maximum of minima over explicit finite families of tails, each an exact
finite sum plus a geometric tail, rational when the roots are.  Moreover
$\tau(X)<T$ for every placement $X$ of size $u$ not contained in
$\{1,\ldots,N\}$, so the supremum defining $T$ is attained by a placement
exactly when the maximum is attained with $j=0$.
\end{theorem}

\begin{proof}
Every member of the family is at most $T$ by Lemma~\ref{lem:escape}.
Conversely let $|X|=u$ and $N_i=i(L-u)\Delta$.  If each of the $u$ windows
$(N_i,N_{i+1}]$, $0\le i<u$, meets $X$, then $X\subseteq[1,N_u]$ and $\tau(X)$
is a member with $j=0$.  Otherwise take the least window $(N_i,N_{i+1}]$
missing $X$, and put $X'=X\cap[1,N_i]$ and $E=X\cap(N_{i+1},\infty)$.  If
$E=\varnothing$, again $X\subseteq[1,N]$.  If not, $j=|E|\ge1$,
$|X'|=u-j\le L-1-j$, and
$\max(X'\cup\{0\})+(L-j-|X'|)\Delta\le N_i+(L-u)\Delta=N_{i+1}<\min E$, so
Lemma~\ref{lem:farblock} gives $\tau(X)<\tau_j(X')$, a member.  Hence $T$ is
the maximum, and $\tau(X)<T$ whenever $X\not\subseteq[1,N]$, because then
$E\ne\varnothing$.  The description of $\tau_j(X')$ is
Lemma~\ref{lem:zerobound} with $k=j$, since $L-j-1-|X'|=L-1-u$.  The tail of
a full certificate with zero set $A$ is the finite sum up to $\max A$ plus the
modulus of a geometric series, as it keeps one sign beyond $\max A$.  The
value is Theorem~\ref{thm:value}.  Finally, a placement $X$ with
$\tau(X)=T$ lies in $\{1,\ldots,N\}$ and is a member with $j=0$, and a
member with $j=0$ is a placement.
\end{proof}

The infimum itself is never attained (Theorem~\ref{thm:noattain}); the
last clause says whether some finite placement already costs the full
$T$.

\paragraph{Examples.}  At $u=1$ the theorem is a cruder, uniform form of
Proposition~\ref{prop:uoneall}, and run literally it returns the values in
the table above.  At $u=2$ the maximiser can be a finite placement with no
escaping block, at a value strictly between the two-sided bound and the
escaping value.  At $(\tfrac{11}{10},3,5,7)$, where $\Delta=111$ and $N=444$,
the family has $98791$ members; the maximum is $\tau(\{2,4\})$, attained by
the full certificate with zero set $\{2,4,5\}$ and certified by the
multipliers of Proposition~\ref{prop:attain}, so
\[
  \inf_Qb_3\Bigl(Q\,(x-\tfrac{11}{10})(x-3)(x-5)(x-7)\Bigr)
  =\frac{294156}{21995}=13.37\ldots,
\]
against the two-sided bound $\tfrac{24}5$ and the escaping value
$1/\tau^*=24$.  At $(\tfrac65,\tfrac32,3,5)$ ($\Delta=103$) the maximum is
$\tau(\{1,2\})$, with zero set $\{1,2,9\}$, and the infimum of $b_3$ is
$\tfrac{5678061913476}{1142868634255}=4.968\ldots$, against $\tfrac45$ and
$8$.  At $(\tfrac32,3,5,7)$ the maximum $\tfrac1{24}$ is attained both by the
escaping pair and by $X=\{2,3\}$, and the infimum of $b_3$ is the two-sided
bound $24$, as Theorem~\ref{thm:twosidedsharp} predicts; at $(2,3,5,7)$
only the escaping pair attains $\tfrac1{24}$, as Theorem~\ref{thm:extremal}
requires.  The window is far
from sharp: in all three cases every minimising zero set lies within $9$ of
its placement, against the proven $(L-1-u)\Delta\ge64$.
...
Corollary~\ref{cor:repairedsharprep}) say when they are attained.  The shift of Corollary~\ref{cor:shiftrun}
also holds with its hypothesis at run ends (Corollary~\ref{cor:shiftend}).
No infimum is attained (Theorem~\ref{thm:noattain}); at roots at least~$2$ a
placement reaches $\tau^*$ exactly in the case of
Theorem~\ref{thm:attainchar}, and Proposition~\ref{prop:farrate} gives
the rate $O(1/\min E)$ of far zeros.  Below $2$ the value is a finite
computation for every $u$ (Theorem~\ref{thm:finite}).

\paragraph{Open questions.}  None of the following is needed for the results
above.  Theorem~\ref{thm:finite} makes $T$ a finite computation for every
$u$, but its window $N=u(L-u)\Delta$ is far from sharp: at
$(\tfrac{11}{10},3,5,7)$ with $u=2$ it is $444$, while every minimising zero
set lies within $9$ of its placement.  Whether the maximising pair
$(j,X')$ can be located as directly as the half-mass point of
Proposition~\ref{prop:uoneall} does for $u=1$, and whether
Theorem~\ref{thm:finite} holds at repeated roots below the cutoff, are open.
