...
\[
  ...
  \qquad \Tail\bigl(B\cup\{6\}\bigr)=\tfrac{35}{1716}.
\]
\end{proposition}
...
$\Tail(F_M)$ exactly and $p_{g,52}$ falls below it.  What does hold, at every
$g$ and $M$, is the slide from a zero set whose part below $g$ is an initial
segment, by a certificate that is not a vertex at all: smoothing $F_M$ against
the new node.

\begin{theorem}[The slide from an initial segment]\label{thm:slideinitial}
Let $y_1,\ldots,y_{c-1}\in(0,1)$ be distinct, let $y_c\in(0,\tfrac12]$ be a
further node, let $B=\{1,\ldots,k\}$ with $k=c-3$, and let $g>k$ and $M>g$.
Let $F_M$ be the full certificate on $y_1,\ldots,y_{c-1}$ with zero set
$B\cup\{M\}$.  Then some certificate on all $c$ nodes vanishing on
$B\cup\{g\}$ has tail at most $\Tail(F_M)$, so \eqref{eq:slide} holds; and
for $g=k+1$ the tail is at most $\tfrac{y_c}{1-y_c}\Tail(F_M)$.
\end{theorem}

\begin{proof}
Write $F_M(x)=\sum_dF_{M,d}x^d=N(x)/\prod_{i<c}(1-y_ix)$ with $\deg N\le c-2$,
and put
\[
  v(x)=\frac{1-y_cxF_M(x)}{1-y_cx}
...
\[
  v_d=-\sum_{1\le j<d}F_{M,j}\,y_c^{\,d-j}\qquad(d\ge1).
\]
For $b\le k$ every $j<b$ lies in $B$, so $v_b=0$: $v\in\mathcal P$.  Summing
the geometric factors,
$\Tail(v)\le\sum_{j\ge1}|F_{M,j}|\sum_{i\ge1}y_c^i
=\tfrac{y_c}{1-y_c}\Tail(F_M)\le\Tail(F_M)$, the one use of the cutoff.  At
$d=g$ the sum runs over $k<j<g$; by the zero bound $F_M$ has exactly the zeros
$B\cup\{M\}$, so it keeps one sign on $(k,M)$, and $v_g$ is either $0$, when
$g=k+1$, or of the sign opposite to $F_{M,g}$.  In the first case $v\in\ell_g$
already.  In the second the segment from $F_M$ to $v$ in $\mathcal P$ crosses
$\ell_g$ at $w=(1-\lambda)F_M+\lambda v$ with
$\lambda=F_{M,g}/(F_{M,g}-v_g)\in(0,1)$, and convexity gives
$\Tail(w)\le\max(\Tail(F_M),\Tail(v))=\Tail(F_M)$.  By
Proposition~\ref{prop:median} the minimum over $\ell_g$ is at a vertex
$p_{gN}$, which is then a full certificate with the claimed tail.
\end{proof}

Only the new node is held to the cutoff, and the others may exceed it; with
...
reach a $B$ with a gap below $g$, where $v_b=-\sum_{j<b,\,j\notin B}F_{M,j}y_c^{b-j}$
need not vanish at the first $b$ above the gap, nor a $B$ with elements above
$M$, which is where Proposition~\ref{prop:slidefails} lives; in
$24312$ instances with $B\subset(0,g)$ we found no failure.
...
The second was reduced to one inequality and the inequality is false.  By
Proposition~\ref{prop:step} it is the single step \eqref{eq:step}, deleting
the largest element of the placement; by Proposition~\ref{prop:stepeasy} the
step holds except when the minimizing zero set straddles $\max X$, and by
Proposition~\ref{prop:slide} the straddling case is the slide inequality
\eqref{eq:slide}, which Proposition~\ref{prop:slidealigned} proves when either
minimizer involved sits at or below $\max X$.  In the remaining case
\eqref{eq:slide} fails, at nine nodes clustered just below the cutoff
(Proposition~\ref{prop:slidefails}), so no argument that ignores the
minimality of the zero set being slid can prove \eqref{eq:step}.  The step
itself has no known counterexample: none in $47638$ instances to $L=6$, in
$1352$ further steps, or among all placements of size $L-2$ inside
$\{1,\ldots,15\}$ at the clustered nodes with $L\le8$.
Theorem~\ref{thm:slideinitial} proves the slide whenever the zero set below
$\max X$ is an initial segment, by smoothing the minimizing certificate
against the new node; a gap below $\max X$, or zeros above the slid one, is
what remains, and the displaced zero is then not the half-mass point.  Three
alternatives are refuted above: deleting $\min X$ rather than $\max X$ fails
at $(2,3,4,5)$; freeing $\max Z^-$ rather than the smallest zero above $g$
fails by a factor $1.40$; and adjoining $g$ to $Z^-$ without sliding, which
would need no displaced zero at all, fails in $245$ of the $379$ straddling
instances we put to it, by as much as a factor $2.01$.  Proposition~\ref{prop:head} states the same question
