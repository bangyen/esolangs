...
ordered at every degree, approaching the bound from above, and identify a partial-sum criterion on the
...
$b_2(F_n)=t_n$ decreasing to $2$~\cite[Proposition~3.5]{coefficient-mass}, so
...
paper proves the two limits below~\cite[Section~3]{coefficient-mass}; what
is added here is that the ordering holds exactly at every degree, with both
limits approached from above, and the Descartes step that the general
...
the root pair $(2,3)$ and $k=2$, \cite[Proposition~3.5]{coefficient-mass}
settles it outright: the infimum of $b_2$ over monic multiples is exactly the bound, and
...
Proposition~3.5 (The bound for $b_2$ at roots $(2,3)$ is an infimum),
Corollary~3.7 (Sharp prime-root infimum).
\end{thebibliography}
