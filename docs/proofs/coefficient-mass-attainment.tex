...
\begin{conjecture}[Slide-down rule]\label{conj:slide}
In that case put $z_0=\min\{z\in Z^-:z>g\}$ and
$A=\bigl(Z^-\setminus\{z_0\}\bigr)\cup\{g\}$, so that $A\supseteq X$.  Then,
with $F$ the full certificate on $y_{k+2},\ldots,y_L$ with zero set $A$ and $H$
as in Lemma~\ref{lem:pencil},
\begin{equation}\label{eq:slide}
  \min_{\theta\ge0}\sum_{d\ge1}\Bigl|\,|F_d|-\theta|H_d|\,\Bigr|\le\Tail(v^-);
\end{equation}
equivalently, by Proposition~\ref{prop:median}, some $N$ makes the full
certificate on $y_{k+1},\ldots,y_L$ with zero set $A\cup\{N\}$ have tail at
most $\Tail(v^-)$.
\end{conjecture}

With Propositions~\ref{prop:step} and~\ref{prop:stepeasy} the conjecture gives
$T=\tau^*$ outright.  It is the whole remaining content: one added node, one
zero slid down to $g$, one zero displaced upwards.  At $(2,3,5,7)$ with $u=1$
and $X=\{2\}$ it is the covering of Proposition~\ref{prop:values}: $Z^-=\{1,3\}$
slides to $A=\{1,2\}$ and $N=5$ gives $\tfrac{21337}{1910352}<\tfrac{31}{1704}$.
In the form \eqref{eq:slide} it leaves no search at all: the left side is one
weighted median.  We met it in $674$ further straddling instances, in every one
of which the median fell above $\max A$, so the restriction $N>\max A$ of the
earlier formulation costs nothing.
...
minimizing zero set straddles $\max X$; Conjecture~\ref{conj:slide} is what
covers the straddling case, in the closed form \eqref{eq:slide}.  We tested
...
rule, and \eqref{eq:slide} itself at a further $674$.  Three alternatives are
