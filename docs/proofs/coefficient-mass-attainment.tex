...
is an infimum in general.  The criterion is also necessary, repeated
roots included: when it fails, the infimum lies strictly above the bound.  It is
...
behind the infimum in general, and proves that, repeated roots included, the bound
...
The minor estimates of Lemma~\ref{lem:far} and
Proposition~\ref{prop:conffar} can be bypassed altogether.  What the far-zero
argument needs is only that zeros far out cannot be produced by the leading
functions, and that is a Descartes rule at infinity.  Let $\mathcal C$ be the
...
$\mathcal H_k$ is the span of $y_{k+1}^d,\ldots,y_L^d$, and (c) is
Lemma~\ref{lem:far}.
...
For distinct roots the equality case forces $u=1$, $r_1=2$ and $k=0$, and
the argument above is the one given before for that case; with repetitions
it also covers $r_1=\cdots=r_u=2$ for any $u$ and any $k<u$.

For distinct roots the proof also shows how a multiple can approach \eqref{eq:order}.  Outside
...
minimizer.  Whether \eqref{eq:order} is itself the infimum is settled: by
Proposition~\ref{prop:partial}, Theorem~\ref{thm:converse} and
Corollary~\ref{cor:charrep} it is exactly when \eqref{eq:partial} holds, as
...
$r_1=2$.  Repeated roots are settled on both sides.  In the Hermite basis $d^ay_i^d$,
ordered by growth, Descartes' rule counts with multiplicity, Hermite
interpolation on a multiset replaces the Vandermonde change of variables, and
the unisolvence and positivity steps are the confluent matrix $M(0)$ of
Proposition~\ref{prop:confluent} and the P\'olya--Szeg\H o bound.  The one
step that does not carry is the minor estimate in the proof of
Lemma~\ref{lem:far}: the constant $c=\prod_{a<b\le k}(1-y_b/y_a)$ vanishes as
soon as two nodes coincide, and $\alpha_{J^*}$ can vanish outright---at the
roots $(3,3,4,4)$ with $u=3$ the leading functions
$d(\tfrac13)^d,(\tfrac13)^d,d(\tfrac14)^d$ have the combination
$\tfrac{2187}{16384}d(\tfrac13)^d-\tfrac{15309}{8192}(\tfrac13)^d+d(\tfrac14)^d$
vanishing at $d=6,7,8$; among the triples in $\{1,\ldots,50\}$ with
$\min E<21$ or diameter at most $30$ it is the only singular one.  Proposition~\ref{prop:farrep} avoids
minors altogether: zeros far out cannot be produced by the leading
functions (Lemma~\ref{lem:infinity}), so the space of sums vanishing on
$E_\nu$ converges to $\mathcal H_k$ whatever the shape of $E_\nu$, and
$\alpha_{J^*}(E)\ne0$ with $\eta\to0$ once $\min E$ is large.  This gives
Theorem~\ref{thm:converse} and, with Theorem~\ref{thm:value},
Corollary~\ref{cor:charrep}.  It gives no rate.  Proposition~\ref{prop:conffar}
supplies the rate when $E_\nu$ has bounded diameter---$\Theta(1/\min E)$ when
the cut falls inside a multiplicity block, geometric at an aligned cut---and
throughout our computations $\eta$ stayed $\Theta(1/\min E)$ at a split cut
even with gaps $\lfloor\sqrt n\rfloor$, linear gaps and geometric spacing, but
a rate for arbitrary $E$ is not proved.
