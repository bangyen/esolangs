...
criterion on the surviving root product under which the same construction
proves attainment in general, and give root sets where the criterion holds
and where it fails; we show the standing restriction $r_1\ge2$ cannot be
dropped once $u\ge1$, already at $L=2$; we show the node cutoff $y_i\le1/2$
underlying the tail comparison behind the bound cannot be raised; and we
extend that comparison to confluent (repeated) nodes.  A repaired bound
below the cutoff is conjectured but not proved, and the converse of the
partial-sum criterion is left open.
\end{abstract}
...
\end{equation}
and \eqref{eq:partial} can fail.  At $(2,3,5,7)$ with $u=1$ the coefficients
...
excluded position, a factor $y_i/(1-y_i)$ once it is at most $1$; retaining
that factor instead gives the candidate bound
\[
  b_{u+1}\ge|f_D|\prod_{i=u+1}^L(r_i-1)\prod_{i=1}^{u}\min(r_i-1,1),
\]
which reduces to \eqref{eq:order} when every $r_i\ge2$.  It is also exactly
what the proof of Lemma~3.1 returns if the discard there is not performed: a
factor
