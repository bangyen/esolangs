...
$b_{u+1}\ge|f_D|\prod_{i=u+1}^L(r_i-1)$.  This paper studies how tight that
bound is.  We give explicit monic multiples of $(x-2)(x-3)$ and of
$(x-2)(x-3)(x-5)$ for which the bound is approached arbitrarily closely, so
it is an exact infimum at those root sets; we identify a partial-sum
criterion on the surviving root product that predicts attainment in
general, confirmed in every case tried but proved only where it holds; we
show the standing restriction $r_1\ge2$ cannot be dropped once $u\ge1$,
already at $L=2$; we show the node cutoff $y_i\le1/2$ underlying the tail
comparison behind the bound cannot be raised; and we extend that comparison
to confluent (repeated) nodes.  Numerical evidence for a repaired bound
below the cutoff, and the open cases the partial-sum criterion leaves, are
...
For positive roots $2\le r_1\le\cdots\le r_L$, repetitions allowed, and $F$ a
...
applying this with the excluded coefficient positions supplying $Z$.  Its
Lemma~2.1 evaluates the consecutive case, its Lemma~2.3 is the interlacing
step inside the comparison, its Lemma~3.1 is the certificate that converts
the tail bound into a coefficient bound, and its Corollary~3.6 sharpens the
resulting asymptotic to an exact infimum for the first $L$ primes.

This paper asks how tight \eqref{eq:order} is, and how far its hypotheses
can be relaxed.  Section~\ref{sec:extend} extends Theorem~2.2 to confluent
nodes, shows the node cutoff $y_i\le1/2$ it rests on cannot be raised, and
shows the standing restriction $r_i\ge2$ in \eqref{eq:order} cannot be
dropped once $u\ge1$.  Section~\ref{sec:attain} exhibits explicit monic
...
the numerical search backing the open cases, and data availability.
...
Lemma~3.1 returns if the discard there is not performed: a factor
