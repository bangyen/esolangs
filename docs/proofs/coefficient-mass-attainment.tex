...
$1704/31$.  For distinct roots the escaping placement is always the worst,
so the infimum is the reciprocal of the least certificate tail on the
...
small, in particular for $b_L$ whenever $r_{L-1}\le2$; with an exempt root
below $2$ it is the infimum only then, and a two-sided bound
$b_{u+1}\ge|f_D|\min(|P(1)|,|P_u(1)|)$ improves it whenever the exempt roots
straddle $2$.
\end{abstract}
...
and with it the answer to when the infimum is reached.  Write
...
The same limit argument as for Theorem~\ref{thm:repaired} carries the
non-strict inequality to confluent nodes.  Read on root sets, the maximum of
...
subsequence $X_\nu=X_0\cup E_\nu$ with $|E_\nu|=k$ and $\min E_\nu\to\infty$,
and Lemma~\ref{lem:far}, which needs only nodes in $(0,1)$, turns a
...
infimum is $1/\tau^*$; by Theorem~\ref{thm:extremal} it always closes.
...
$0.70$.  Theorem~\ref{thm:extremal} below proves \eqref{eq:step} in general.  Where the displaced zero goes is
...
For $|A|=1$ it is proved below (Corollary~\ref{cor:shiftone}), and whenever
no two elements of $A$ are consecutive by Corollary~\ref{cor:shiftrun}.  For
runs of length at least two Corollary~\ref{cor:shiftrun} proves it with a
hypothesis that moves the start of each run past its end instead;
minimality supplies that form equally, and it is the one
Theorem~\ref{thm:extremal} uses.  In the stated wording it held in every
one of $550$ further instances tried.
...
The chord does extend, once every run of $A$ is moved at once and every run
contributes its own neighbour.  Write $A$ as the disjoint union of its maximal
runs of consecutive integers $[s_1,e_1],\ldots,[s_p,e_p]$, so that
...
$a\to a+1$ of \eqref{eq:shift}.  Shifting every run at once is the left side of
\eqref{eq:shift}.
...
For $A=\{M\}$ this is Proposition~\ref{prop:chord}, with $\lambda_1=\lambda$,
$\mu=1-\lambda$ and $F_1=F_{M+1}$.
...
\emph{Counting.}  Suppose first $m\ge2$, with gap starts
$q_1<\cdots<q_{m-1}$, and $E\not\equiv0$.  As in the proof of
Proposition~\ref{prop:chord}, $R_1$ supplies $|R_1|-1$ zeros below $q_1$; for
...
$(0,y_{k+1})$.  The proof also shows why the single chord of
Proposition~\ref{prop:chord} breaks for $|A|\ge2$: with one neighbour only,
the other runs of $A$ are not shifted in $w$, and $E$ loses the zeros they
would force.
...
others on $y_{k+2},\ldots,y_L$.  When no two elements of $A$ are consecutive
this is \eqref{eq:shift}.
\end{corollary}
...
The hypothesis is again what minimality supplies.  For that we need
...
So when \eqref{eq:partial} fails the infimum is always $1/\tau^*$: the
covering search after Corollary~\ref{cor:uone} always closes, and
Corollary~\ref{cor:uone} reads $\inf_Qb_2=1/\tau^*$.  The hypothesis of
\eqref{eq:shiftrun} differs from that of \eqref{eq:shift} only on runs of
length at least two, where it moves the start of the run instead of its end;
minimality supplies both, and only the former enters the proof.
...
That it always closes can also be put analytically, as a question about one
end of a multiple.  Fix the
...
lie in $(1,2]$ and at most $u$ partial sums of $P$ exceed $|P(1)|$.  With an exempt root below $2$ and those hypotheses failing it is not
(Corollary~\ref{cor:repairedsharp}); the sharper two-sided bound
\eqref{eq:twosided} is the infimum exactly under the criterion of
Theorem~\ref{thm:twosidedsharp}, and otherwise the infimum is $1/T$
(Theorem~\ref{thm:value}), a finite computation for $u=1$
(Proposition~\ref{prop:uoneall}).  For $u\ge2$ below the cutoff the escaping
placement need not be extremal, and $1/T$ remains a search.
...
and for distinct roots Theorem~\ref{thm:extremal} shows $T=\tau^*$: the
escaping placement is always extremal, so the infimum is
$1/\tau^*=\min_M\max_{j\ge1}|S_j(P_uM)|$, computed from the surviving roots
alone, and the finite search of Theorem~\ref{thm:lift} always closes, as it
does at $(2,3,5,7)$ and $(2,\ldots,7)$ (Proposition~\ref{prop:values}).
Attainment by a full certificate with a witness as in \eqref{eq:dualpair} is
Proposition~\ref{prop:attain}.  The route to Theorem~\ref{thm:extremal} is
Proposition~\ref{prop:step}, which reduces it to the single step
\eqref{eq:step}, deleting the largest element of the placement, and
Proposition~\ref{prop:stepeasy}, which settles the step unless the minimizing
zero set $Z^-$ straddles $g=\max X$.  In the straddling case the naive
inequalities fail: the slide \eqref{eq:slide} fails at nine clustered nodes
(Proposition~\ref{prop:slidefails}), deleting $\min X$ rather than $\max X$
fails at $(2,3,4,5)$, freeing $\max Z^-$ rather than the smallest zero above
$g$ fails by a factor $1.40$, and adjoining $g$ without sliding fails by as
much as a factor $2.01$.  What works is to shift every run of zeros above $g$
up by one, and the run chord (Proposition~\ref{prop:runchord}) compares that
shift against one neighbour per run; minimality of $Z^-$ supplies the
hypothesis, and the cutoff enters only through the sign of $2-1/y_{k+1}$.
Theorem~\ref{thm:slideinitial} and Proposition~\ref{prop:chord} are the
special cases $B_0$ initial and $|A|=1$.  Proposition~\ref{prop:head}
states the same question analytically, as a bound on the $\max X-1$
coefficients of a deflated multiple; by Proposition~\ref{prop:defthresh}
that route says nothing pointwise once $r_1\le\tfrac{3+\sqrt5}2$.  Repeated roots are settled on both sides.  In the Hermite basis $d^ay_i^d$,
