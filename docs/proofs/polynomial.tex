...
length $O(T^2/\log T)$, and we conjecture that some require
$\Omega(T^2/\log T)$ source characters in every program,
independently of the program's degree, operand signs, and additional roots.
The explicit residual-DAG construction gives the upper bound.  A companion
...
$\Omega(L^2\log L)$.  The intended routing argument, sharpened by an
outerplanar block-incidence bound, would force $L=\Omega(T/\log T)$ distinct
real roots in a program for a maximal-width table; its key step is a known gap
(Remark~\ref{rem:routing-gap}), so the lower bound is not established here.
\end{abstract}
...
\]
Each nonzero coefficient appears in a monomial whose serialization supplies
...
$O(T^2/\log T)$.  The matching lower bound --- that there are $n$-input
Boolean functions for which every Polynomial program has length
$\Omega(T^2/\log T)$, so that the worst-case text complexity is
$\Theta(T^2/\log T)$ --- is conjectural; the argument offered for it below has
a known gap (Remark~\ref{rem:routing-gap}).
\end{theorem}

\begin{remark}[Known gap in the lower bound]\label{rem:routing-gap}
The upper bound of Theorem~\ref{thm:main} is proved in
Section~\ref{sec:upper}.  The lower bound is not established as written: the
bound $D_k\le1+2m_{\rm route}$ of Section~\ref{sec:routing} is false, because
a routing-free suffix may contain several reads, so one routing position and
successor does not fix the cursor at the $k$th read.  The decoded program
$[5,1],[5],[0,2],[48,2],[2],[0,2],[0,2],[0,2],[0,2],[0,1]$ has
$m_{\rm route}=1$ and four distinct cursors at its fourth read.  The valid
replacement, $N'(k+1)\le2R$ with $R$ the number of read instruction
positions, forces $R=\Omega(T/\log T)$ instructions but not
$\Omega(T/\log T)$ distinct root values; supplying that distinct-root forcing
step is open.  The tight $\Theta(T^2/\log T)$ statement is therefore a
conjecture, and Section~\ref{sec:routing} is retained only to expose the
argument that has to be repaired.
\end{remark}

The intended lower bound has two independent parts.
Section~\ref{sec:routing} would force $\Omega(T/\log T)$ distinct positive
roots (this is where the gap of Remark~\ref{rem:routing-gap} lies).
Section~\ref{sec:mass} applies
...
\emph{The bound is false; see Remark~\ref{rem:routing-gap}.  The derivation
is retained so the step to be repaired stays visible.}
...
The further step to $N'(k+1)\le12L+2$ would substitute this into the false
\eqref{eq:basic-routing}; it is exactly the gap of
Remark~\ref{rem:routing-gap} and is not proved here.

\begin{conjecture}\label{cor:roots}
Every Polynomial program for a maximal-width truth table has $L$ distinct
positive real instruction roots, with $L=\Omega(T/\log T)$.
\end{conjecture}

The intended proof combines \eqref{eq:residual-width} with
Lemma~\ref{lem:block} through \eqref{eq:basic-routing}, which is false, so the
statement is the missing distinct-root forcing step of
Remark~\ref{rem:routing-gap} rather than a consequence of the block count.
...
following degree-independent statement~\cite[Corollary~3.3]{coefficient-mass}.
...
\begin{conjecture}[Polynomial lower bound]\label{cor:lower}
...
By Conjecture~\ref{cor:roots}, the program contains
...
Repeated real roots do not weaken Conjecture~\ref{cor:lower}: conditional on
the distinct-root forcing, Theorem~\ref{thm:mass} applies to the program's
distinct-root product.  The block-incidence lemma bounds routing positions
only and does not by itself force those distinct values, as
Remark~\ref{rem:routing-gap} records.
\end{remark}
...
\]
This proves the upper bound of Theorem~\ref{thm:main}.  Adding the
conjectural lower bound, Conjecture~\ref{cor:lower}, would give the matching
bound and hence the $\Theta$ statement; that step is open
(Remark~\ref{rem:routing-gap}).
