...
cursor sequence, independent of the register, up to the $k$th read.  Under
consumption, each read cursor $c$ is executed at a single read index
$\iota(c)$: if two executions read at $c$ after $a<a'$ earlier reads, give
both the same bit there, so that they share the state $(48+b,c+1)$, and feed
the first the second's remaining $n-a'-1$ bits followed by anything; it then
follows the second's trajectory and halts after $n-(a'-a)<n$ reads,
contradicting consumption.  The reads $c_1,c_2,\ldots$ of the fixed
continuation therefore have consecutive indices
$\iota(c_1),\iota(c_1)+1,\ldots$, at most one of them is the $k$th, and the
cursor of the $k$th read is determined by $(r,s)$.  If the path contains no routing position it is
...
The bound \eqref{eq:basic-routing} is false for a decoded program whose read
count may differ from $n$.  The decoded list
\[
...
If a program's read count may differ from \(n\), every Polynomial source for
