...
\begin{equation}\label{eq:basic-routing}
  D^*_k\le 2+4m_{\rm route},
  \qquad\text{and hence}\qquad
  N^*(k)\le 4+8m_{\rm route}\qquad(k\le n-2).
\end{equation}
...
\]
has $m_{\rm route}=1$ and four distinct cursors at its fourth read, against
the $1+2m_{\rm route}=3$ that one fixed read index per cursor would allow:
...
bits satisfy the sharper $D_k\le1+2m_{\rm route}$ over all cursors, but the
lower bound does not need it.
\end{remark}
