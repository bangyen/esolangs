...
$\Omega(T^2/\log T)$ source characters in every program that consumes its
input, independently of the program's degree, operand signs, and additional
roots.
...
complexity is $\Theta(T^2/\log T)$ in this model.  Without the consumption
hypothesis the same argument gives the weaker $\Omega(T^2/\log^3 T)$.
\end{abstract}
...
Reads consume the input stream in order, and a program for an $n$-input table
consumes the whole input: every execution performs exactly $n$ reads before it
halts.  This is the model the upper-bound construction uses, and it is what
the routing argument below needs; a program that halted after fewer reads
would compute a function constant on the subcube of unread inputs.
...
program that consumes its input has length $\Omega(T^2/\log T)$.  Hence the
worst-case text complexity in the model of Section~\ref{sec:model} is
$\Theta(T^2/\log T)$.
\end{theorem}

\begin{remark}[Input consumption is needed]\label{rem:routing-gap}
The bound $D_k\le1+2m_{\rm route}$ of Section~\ref{sec:routing} is false for
a decoded program that may stop reading early.  The decoded list
\[
[5,1],[5],[0,2],[48,2],[2],[0,2],[0,2],[0,2],[0,2],[0,1]
\]
has
$m_{\rm route}=1$ and four distinct cursors at its fourth read: its loop reads
an input-dependent number of bits, so different executions reach the last
routing position at different read levels and the routing-free suffix lands
the fourth read at different offsets.  Such a list performs a different
number of reads on different inputs, so it is not a program for an $n$-input
table in the model of Section~\ref{sec:model}, and the routing proof, which
uses the exact read count, does not apply to it.  Any variable-read list is
excluded the same way.  Corollary~\ref{cor:general} covers the general model
with one extra logarithm: the free read level lets one routing position and
successor place the $k$th read at any of $k$ offsets, so $D_k\le1+2k\,m_{\rm
route}$.
\end{remark}
...
reads and register instructions do not branch; the continuation is a fixed
cursor sequence, independent of the register, and reads a fixed number
$\rho(r,s)$ of further symbols before halting.  The execution consumes its
input, so it reads exactly $n$ symbols and reached $r$ after $n-\rho(r,s)$ of
them; the $k$th read therefore sits at the fixed offset
$k-(n-\rho(r,s))$ of that fixed continuation, and its cursor is determined by
$(r,s)$.  If the path contains no routing position it is
...
the right side is \((1/2+o(1))L^2\log L\): each
\(\log(r_i-1)\) occurs \(i\) times, and \(r_i\ge i+1\).  The bound is
asymptotically sharp, attained by \(P_L=\prod_{i=1}^L(x-p_i)\); in particular
it is \(\Omega(L^2\log L)\).  The theorem applies to every real multiple,
...
\begin{corollary}[General model]\label{cor:general}
If a program may halt before consuming its input, every Polynomial source for
a maximal-width truth table still has
\[
  |f|=\Omega(T^2/\log^3 T).
\]
\end{corollary}
...
\begin{proof}
The continuation from the last routing position $(r,s)$ is still a fixed
cursor sequence, but without the consumption hypothesis the read level at $r$
is free.  The $K$th read is the $j$th read of that sequence for some
$j\in\{1,\dots,K\}$, so each routing position and successor contributes at
most $K$ cursors:
\[
  D_K\le 1+2K\,m_{\rm route}.
\]
Take the level $K=n-\lceil\log_2 n\rceil=\Theta(n)$ of
Lemma~\ref{lem:maximal-width}, where $N'(K)=\Omega(T/\log T)$.  Then
\eqref{eq:residual-cursor} gives
$m_{\rm route}=\Omega(T/(K\log T))=\Omega(T/\log^2 T)$,
Lemma~\ref{lem:block} gives $L=\Omega(T/\log^2 T)$, and
Theorem~\ref{thm:mass} gives
\[
  \Lambda(f)=\Omega(L^2\log L)=\Omega(T^2/\log^3 T).
\]
\end{proof}
