...
satisfies
$\log_{10}2/1152\le\liminf C_P(n)\,n/T^2\le105.5\log_{10}2$ and
$\log_{10}2/288\le\limsup C_P(n)\,n/T^2\le422\log_{10}2$: the order is
...
$T^2/n=T^2/\log_2T$.  The lower side is Corollary~\ref{cor:roots} with
Theorem~\ref{thm:mass}; the upper side counts the exponents of the roots
that the construction of Section~\ref{sec:upper} uses.

\begin{proposition}[Explicit bracket]\label{prop:bracket}
Let $N_n$ be as in Lemma~\ref{lem:maximal-width}, $L_n$ as in
Corollary~\ref{cor:roots}, and $s_k$ as in Section~\ref{sec:upper}.  Put
...
\begin{proof}
\emph{Lower bound.}  By Corollary~\ref{cor:roots}, every program for a
maximal-width table has $L\ge L_n$ distinct real instruction roots.  They
are distinct integers at least~$2$, so $r_i-1\ge i$ after sorting.
...
root to every $12$ first-essential residuals at one level
(Lemma~\ref{lem:block}).  The construction spends two per state.  Both
per-cursor caps behind~\eqref{eq:basic-routing} are attained, and so is the
graph bound in Lemma~\ref{lem:block}.  The loss comes from combining them.
...
step, and no sharpening of it narrows the bracket.  What remains is a
