...
$|f_D|\prod_{i=u+1}^L(r_i-1)$.  For $|f_D|\ge1$ the bounds sum to a lower bound
...
asymptotic to the infimum over monic real multiples,
...
Theorem~\ref{thm:order} itself---an explicit monic multiple of
$(x-\tfrac65)(x-\tfrac{19}{10})$ has $b_2<r_2-1$---and
...
perturbation in Theorem~\ref{thm:order}.  Fix $0\le u\le L-1$.  For every set $U$ of $u$
...
\begin{proposition}[The root restriction $r_i\ge2$ is necessary]
\label{prop:subtwo}
For $L=1$ the bound \eqref{eq:order} holds for every root $r>1$: every
nonzero multiple of $x-r$ has $b_1>|f_D|(r-1)$.  For $k\ge2$ it fails without
$r_1\ge2$.  With $r_1=6/5$ and $r_2=19/10$ there is a monic multiple $F$ of
$(x-r_1)(x-r_2)$ of degree $4$ with
\[
  b_2=\frac{740772}{1224775}=0.60482\ldots<\frac9{10}=r_2-1 .
\]
\end{proposition}
...
For the failure put $P=(x-\tfrac65)(x-\tfrac{19}{10})=x^2+p_1x+p_0$ with
$p_1=-\tfrac{31}{10}$, $p_0=\tfrac{57}{25}$, and let $Q=x^2+q_1x+q_0$ be
...
$D=\tfrac{48991}{2500}>0$, so
\[
  q_0=\frac{12996}{48991},\qquad q_1=\frac{30666}{48991},\qquad
  f_0=f_1=f_2=\frac{740772}{1224775}=0.604822\ldots,
\]
while $f_3=p_1+q_1=-\tfrac{1212061}{489910}=-2.474048\ldots$.  The four
nonleading magnitudes are $|f_3|$ and three copies of $p_0^3/D$, so
$b_2=p_0^3/D<9/10=r_2-1$, which is what \eqref{eq:order} would assert for
$u=1$.
\end{proof}
...
height: subject to $|f_D|\ge1$ they are uniform over the degree of $F$ and
over all additional real or complex roots.  In the opposite direction, the
degree of an integer polynomial of height $H\ge2$ all of whose roots are
...
Two limits are worth stating plainly.  Every coefficient bound above
requires $r_i\ge2$, inherited from the node cutoff $y_i\le1/2$ of
Theorem~\ref{thm:tail}; only Lemma~\ref{lem:interlace} is free of it.  The
restriction is not an artifact of the proof: by
Proposition~\ref{prop:subtwo}, \eqref{eq:order} is false for
roots in $(1,2)$ already at $L=2$, $k=2$.  Only the case $L=1$ survives, where
the bound holds for every $r>1$.  The method fails in the same place: the tail
bound rests on the node cutoff $y_i\le1/2$, which Proposition~\ref{prop:cutoff}
shows cannot be raised.  A bound for roots in $(1,2)$ therefore needs a
different form, not a wider proof.
