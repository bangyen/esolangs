...
$|f_D|\prod_{i=u+1}^L(r_i-1)$; the case $u=0$ needs only $r_i>1$.  At roots
$(2,3)$ an explicit family attains the $u=1$ bound in the limit, so that bound
is sharp there.
...
shows that the $u=1$ bound is approached at roots $(2,3)$, so it cannot be
...
\]
with the coefficients determined by vanishing at $2,3,5$.  The large
coefficients go in the exempt slots, with $|c_D|\sim3\cdot2^{D-1}$ and
$|e_D|,|e_D'|$ of order $3^D$, while $a_D\to7$ stays below $t_D$.  Hence
$b_2(F_D)=t_D\to8$ and $b_3(G_D)=s_D\to4$, the values \eqref{eq:order}
gives at $u=1$ and $u=2$.  The companion
paper~\cite[Proposition~3.2]{coefficient-mass-attainment} proves this, and
gives a partial-sum criterion on the surviving root product under which the
same construction shows \eqref{eq:order} is an infimum, together with root
sets where the criterion fails.  Whether the rows $k\ge2$ are sharp at every
root set is open.
...
B.~Pham,
\newblock Attainment and sharpness of the coefficient order-statistic bound,
