...
\paragraph{What remains open.}  Between the two regimes: pairs
$a_j\pm ic_j$ with $a_j\ne0$ that are not orbits of a multisection, under an
arithmetic hypothesis that excludes item 1 of
Proposition~\ref{prop:noangle} (for instance Gaussian integers with
distinct prime-power imaginary parts).  In the small linear programs we ran
(monic multiples of degree at most $2K+6$), generic such pairs, for example
$(a,c)=(1,5),(-2,7),(3,11)$, keep $b_2$ above $\prod_{j\ge2}(\rho_j^2-1)$,
but we have no proof, and
Proposition~\ref{prop:noangle} shows that any proof must use more than the
moduli and the angles' distance to the axis.
