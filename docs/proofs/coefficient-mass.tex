...
$\sum_jj\log\rho_j$, for pairs $a_j\pm ic_j\in\mathbb Z[i]$ with
$0<|a_j|<c_j$ whose moduli crowd.  If every annulus $R\le|z|\le9R$ contains at
most $g$ of the $\rho_j$ and $\rho_{\min}\ge3$, then
Corollary~\ref{cor:separated}, item~3, gives
$\Lambda(F)\ge g^{-2}\sum_jj\log(\rho_j/3)$ uniformly in $K$, for all complex
multiples; Corollary~\ref{cor:oneprime} is the case of one prime.  So the
open case is that of many pairs in one annulus, such as the register roots
$a\pm p^bi$ over many primes $p$, where the norms are coprime or squarefree.
By Corollary~\ref{cor:gaussfinite} a mass bound $X$ needs to
be proved only for $P$-irreducible multiples of degree below
$X/\log(\rho_{\min}/2)$, that is, by Lemma~\ref{lem:gausswindow}, for
$F=PQ$ with $Q$ free of runs of $2K$ zeros and $\deg Q=O(K^2)$.  In that
range the angle mechanism of Proposition~\ref{prop:noangle} is not excluded
for pairs within $\delta\gg1/K$ of the imaginary axis.  Exactly equal moduli
are excluded when the common norm is a prime power
(Corollary~\ref{cor:padiccircle}), but not for squarefree norms, and not for
nearly equal moduli; pairwise coprime norms, the case of the register roots,
get only the first row from Theorem~\ref{thm:padicmass}, and inside one
annulus Theorem~\ref{thm:archnewton} gives no more.  Carry sizes, even
at several roots, cannot close the gap (Proposition~\ref{prop:carrylimit}); what is
needed is a lower bound for the residuals of the rounding recurrence of
...
the question is a uniform statement in $K$ for crowded moduli.
