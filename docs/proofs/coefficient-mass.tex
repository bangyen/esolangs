...
explicitly~\cite[Section~2]{dj}.  Their inequality~(8) already excludes a
...
tail theorem cannot be raised, and Proposition~\ref{prop:confluent} carries
the tail comparison itself to confluent nodes.
...
$(L+1)\log|f_D|$ and collect the index-$i$ terms, which occur in
...
Two limits are worth stating plainly.  The coefficient bounds assume
$r_i\ge2$, inherited from the node cutoff $y_i\le1/2$ of
Theorem~\ref{thm:tail}; only Lemma~\ref{lem:interlace} is free of it.
Proposition~\ref{prop:cutoff} shows that cutoff cannot be raised, so roots in
$(1,2)$ lie outside the reach of this method.  It does not show that the
bounds themselves fail there; whether they extend to that range is not
settled here.
...
that even there the root product need not be the finite-$L$ minimizer.
