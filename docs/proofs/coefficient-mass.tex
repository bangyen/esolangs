...
A similar construction, omitted, gives infima $8$ and $4$ for the rows $k=2$
and $k=3$ at $(2,3,5)$, again the asserted values.  Whether the rows $k\ge2$
are sharp in general is open.  What the two cases together show is that a row
$k\ge2$ can be sharp where row $k=1$ is not: at $(2,3)$ row $2$ is an infimum,
while row $1$ is low by the factor $5/3$.
