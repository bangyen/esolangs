...
coefficient vector up to a term proportional to the number of coefficients
encoded; signs, zero entries, and the exponent encoding of a sparse
representation are accounted for separately.  For real coefficients it is a
...
original combination and whose derivative is a combination of the remaining
$r-1$ exponentials, so
Rolle's theorem and the induction hypothesis give at most $r-1$ zeros.  It is
...
$b_{u+1}(F_\varepsilon)\ge|f_D|\prod_{i=1}^L(r_{i,\varepsilon}-1)$, and letting
