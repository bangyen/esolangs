...
is sharp there.
Summing the bounds lower-bounds the
logarithmic coefficient mass $\Lambda(F)=\sum_j\log^+|f_j|$: for the first $L$
primes, and $|f_D|\ge1$, $\Lambda(F)\ge(\tfrac12+o(1))L^2\log L$, with
asymptotic equality for $F=\prod_{i=1}^L(x-p_i)$.  That consequence is sharp in
order for large roots and empty as the roots approach $2$, whereas the
order-statistic bounds hold at every root set.  The engine is a
...
$S_n$, $n\ge d$, share one sign: their $H(P)\ge|P(1)|$ holds ``provided that
all $S_n$ \ldots have the same sign''~\cite[p.~333]{dj}.  They leave that
proviso as a hypothesis, carried through their
Corollary~7~\cite[Section~2]{dj} and discharged only case by
...
that infimum for every root set here.  Proposition~\ref{prop:sharp23} shows
the $u=1$ bound is attained in the limit at roots $(2,3)$, so no factor can be
removed from it in general.
...
\paragraph{Sharpness.}
Row $k=1$ is $b_1\ge|P(1)|$, and it is an infimum rather than a minimum already
for $L=1$: in $F=(x-r)\sum_{k\le m}c_kx^k$ with $c_m=1$, every $|f_j|\le r-1$
would force $c_{j-1}=rc_j+f_j\ge rc_j-(r-1)$, hence $c_j\ge1$ downward from
$c_m$ and $|f_0|=r|c_0|\ge r$.  Nor is it the strongest bound the certificate
yields.  The consecutive certificate---the one Lemma~\ref{lem:slack} selects
when $u=0$---has no extremal tail among zero sets at fixed nodes: at nodes
$(5,3)$ the displaced $Z=\{2\}$ has the larger tail, while for a fixed $P$ a
displaced zero set can also have the smaller one and beat it.  Take $P=(x-2)(x-3)$, nodes $\rho=(3,2)$ and $Z=\{2\}$, whose tail is
$3/10$ by the example following Theorem~\ref{thm:tail}.  Then \eqref{eq:dual}
with $U=\varnothing$ gives
\[
  b_1\ge10/3
\]
for every monic multiple, against $|P(1)|=2$; their Corollary~8 evaluates this
quadratic exactly~\cite{dj}, giving $10/3$, so the row is low by the factor
$5/3$.
...
The same method reaches $(2,3,5)$.  Writing the multiple from the top---one
leading $1$, a short free prefix, a long run of a single value, and the exempt
coefficients at the bottom---put
\[
  F_D(x)=x^D-a_Dx^{D-1}+t_D\sum_{j=1}^{D-2}x^j+c_D,
  \qquad
  G_D(x)=x^D-s_D\sum_{j=2}^{D-1}x^j+e_Dx+e_D',
\]
with the coefficients fixed by vanishing at $2,3,5$.  The exempt slots take
the large coefficients---$|c_D|\sim3\cdot2^{D-1}$, and $|e_D|,|e_D'|$ of order
$3^D$---while $a_D\to7$ stays below $t_D$, so $b_2(F_D)=t_D\to8$ and
$b_3(G_D)=s_D\to4$, the values \eqref{eq:order} asserts at $u=1$ and $u=2$; we
omit the verification.  Whether the rows $k\ge2$ are sharp in
general is open.
