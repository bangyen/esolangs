...
itself comes within $O(L^2)$ of that infimum.  Proposition~\ref{prop:sharp23}
shows that the $u=1$ bound is approached at roots $(2,3)$, and the paragraph
after it does the same for $u=1,2$ at $(2,3,5)$, so these bounds cannot be
improved by any constant factor in general.
...
above $7$, and $\Delta$ is positive there.  If instead $Z'=\{2,4,5,7\}$
...
perturbation in Theorem~\ref{thm:order}.  Fix $0\le u\le L-1$, with $r_1>1$
...
$c_m$ and $|f_0|=r|c_0|\ge r$.  Nor is it the best bound the certificate
method gives.  The consecutive certificate, which Lemma~\ref{lem:slack} uses
...
\[
  b_1\ge10/3
\]
for every monic multiple, compared with $|P(1)|=2$.  Their Corollary~8
computes the reduced height of this quadratic exactly~\cite{dj} as $10/3$,
so here the row is off by a factor of $5/3$.
