...
The route to a proof, if there is one, is visible in \eqref{eq:gamma}.  Its
first inequality $\Gamma_G(z)\le\Gamma_F(z)/(1-y_c)$ needs only $y_c<1$; the
cutoff is spent entirely on the second, which converts $1/(1-y_c)$ into the
constant $2$ that \eqref{eq:split} requires.  The slack discarded there is a
factor $1/(1-y_c)$, the same shape as the factors the repair retains, so the
natural attempt is to carry the first inequality through the induction without
ever invoking the second.  None of this is proved here.
