...
$S_n$, $n\ge d$, share one sign, which they then establish
separately~\cite[Section~2]{dj}; our consecutive base case,
Lemma~\ref{lem:consecutive}, instead reads the sign off the complete
homogeneous symmetric polynomials $h_j$, and needs no cutoff on the nodes.
...
This makes the requirement explicit.  The run is the $(u+1)$-st largest
magnitude only if no partial sum exceeds the total,
