...
order for large roots and empty as the roots approach $2$; the order statistics
themselves bound at every root set.  The engine is a
...
$c_m$ and $|f_0|=r|c_0|\ge r$.  Nor is $|P(1)|$ always the right constant: for
$P=(x-2)(x-3)$ the infimum over monic multiples is $10/3$
\cite[Corollary~8]{dj}, against $|P(1)|=2$.  For $k\ge2$ we do not know the
truth.  Minimizing the $k$th largest magnitude over monic multiples of bounded
degree is a mixed-integer program; for $(3,5)$, $(5,7)$ and $(2,3,5)$ it stays
within a few percent of $\prod_{i=k}^L(r_i-1)$, well inside the factor $1.8$ by
which row $k=1$ misses at $(2,3,5)$.  Whether the rows $k\ge2$ are sharp in
order is open.
