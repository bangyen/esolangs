...
yields.  Theorem~\ref{thm:tail} bounds the tail \emph{above}, and the
consecutive zero set is where that bound is attained, so the consecutive
certificate---the one Lemma~\ref{lem:slack} selects when $u=0$---is the one
giving the weakest coefficient bound; for a fixed $P$ a displaced zero set can
beat it.  Take $P=(x-2)(x-3)$, nodes $\rho=(3,2)$ and $Z=\{2\}$, whose tail is
...
quadratic exactly~\cite{dj}, giving $10/3$, so the row is off by precisely
$5/3$.  This is also why the rows $k\ge2$ fare better: there the excluded
positions force the zero set off the consecutive one.

The rows $k\ge2$, by contrast, are sharp.  For $n\ge2$ put
\[
...
\]
Each $F_n$ is a monic multiple of $(x-2)(x-3)$: summing the geometric
progression, $F_n(2)=0$ and $F_n(3)=0$ read $a_n=2+t_n(1-2^{-n})$ and
$a_n=3+\tfrac12t_n(1-3^{-n})$, which agree exactly at the stated $t_n$.  Since
$a_n>t_n>0$, the nonleading magnitudes are $a_n$ once and $t_n$ $n$ times, so
$b_2(F_n)=t_n$, and $t_n$ decreases to $2$---the value \eqref{eq:order}
asserts at $u=1$.  The row $k=2$ is therefore an infimum there, approached but
attained by no $F_n$.  The same shape of construction settles $(2,3,5)$, whose
rows $k=2$ and $k=3$ have infima $8$ and $4$, again the asserted values.

So the rows $k\ge2$ can be sharp exactly where row $k=1$ is not: at $(2,3)$
row $2$ is an infimum, while row $1$ understates by the factor $5/3$.
