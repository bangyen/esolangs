...
$\Lambda(F)=\log\prod_{j:f_j\ne0}\max(1,|f_j|)$.  In a fixed integer base $B$
the quantity $\Lambda(F)/\log B$ approximates the total base-$B$ digit length
of the nonzero coefficients, agreeing with the encoded length of the
coefficient vector up to an additive term proportional to the number of
nonzero coefficients (it is not literal equality); signs, zero entries, and
the exponent encoding of a sparse representation are accounted for
separately.  For real coefficients it is a
mass rather than a literal encoding length.
...
that infimum for every root set here, and
...
An example shows where the extra multiplicities come from.  Take $c=5$ and
$Z'=\{2,4,6\}$, so $A=\{0,2,4,6\}$, $r=0$, and $J=\{1,2,3\}$: no marked point
forces a zero, and every marked point fixes a sign.  Those signs are
$-1,+1,-1$, so each of the gaps $(2,4)$ and $(4,6)$ carries an odd number of
zeros of the $4$-node sum $\Delta$, hence at least one, and with the zero at
$m+1=7$ that is the full budget $c-2=3$.  If instead $Z'=\{2,4,5,7\}$
($c=6$), the consecutive pair $\{4,5\}$ forces a zero, and the parity across
the longer gap $(4,7)$ forces one further unit of multiplicity in that gap,
possibly at the forced zero $5$ itself.
...
Corollary~\ref{cor:mass} concerns coefficient magnitudes, not ordinary length
or height.  Subject to $|f_D|\ge1$, it is uniform over the degree of $F$ and
all additional real or complex roots, and bounds all $L$ order statistics.
Corollary~\ref{cor:prime} shows the resulting prime-root bound $(1/2+o(1))L^2\log L$ is attained by
...
relative to reduced-height duality is the prefix-sensitive tail comparison
and its iteration; the normalized determinant sequence and its
consecutive-zero evaluation are from Dubickas and Jankauskas~\cite{dj}.  The
confluent
Proposition~\ref{prop:confluent} is supplementary: it carries the tail theorem
to confluent nodes, but Theorem~\ref{thm:order} already reaches repeated roots
by perturbation, so the main bounds do not depend on it.
