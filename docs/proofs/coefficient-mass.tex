...
$b_2$ is the exact infimum over monic multiples,
...
\begin{proposition}[The bound for $b_2$ at roots $(2,3,5)$ is an infimum]
\label{prop:sharp235}
...
\]
The system is nonsingular, each $F_D$ is a monic multiple of
$(x-2)(x-3)(x-5)$ with $b_2(F_D)=t_D$, and $t_D\to8$.  Hence
\begin{equation}\label{eq:sharp235}
  \inf_{\substack{Q\in\R[x]\\ Q\text{ monic}}}
  b_2\bigl(Q(x)(x-2)(x-3)(x-5)\bigr)=8=(r_2-1)(r_3-1),
\end{equation}
the value \eqref{eq:order} asserts for $u=1$.
\end{proposition}
...
monic multiple, and $t_D\to8$, which is \eqref{eq:sharp235}.
\end{proof}
...
Two limits of this route, both observed numerically rather than proved here.
Taking two exempt coefficients instead of one, the analogous system for
\[
  G_D(x)=x^D-s_D\sum_{j=2}^{D-1}x^j+e_Dx+e_D'
\]
at the same roots gives $s_D$ decreasing to $4=r_3-1$, with
$s_9=\tfrac{31585}{7663}=4.1217538\ldots$ matching the $b_3$ optimum of
Section~\ref{sec:scope}; here the two exempt coefficients grow like $3^D$ and
absorb both $2$ and $3$.  From $L=4$, however, the construction can fail: the
limit system determines the free prefix as well as the run, and its prefix
entries need not be bounded by the run value, as they must be if the run is to
be the $(u+1)$-st largest magnitude.  For $u=1$ and roots $(2,3,5,7)$ the
limit prefix is $(-14,57)$ against a run value of $48$, and the search returns
$\min b_2\approx54.97$, well above \eqref{eq:order}; the same happens at
$(2,3,4,5)$, prefix $(-11,36)$ against $24$ with $\min b_2\approx32.23$, and
at $(2,3,5,6)$, prefix $(-13,50)$ against $40$ with $\min b_2\approx47.55$.
At $(3,4,5,6)$ the prefix is $(-14,60)$ against $60$, just admissible, and the
search returns the bound.  Whether \eqref{eq:order} is an infimum in general
is therefore open, and these searches suggest it is not.
...
Proposition~\ref{prop:sharp235} does the same for $(2,3,5)$ and $k=2$.  From
