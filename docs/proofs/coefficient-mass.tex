...
$\prod_{i=2u+1}^L(r_i-1)$.  Summing bounds the logarithmic coefficient mass
$\Lambda(F)=\sum_j\log^+|f_j|$; for the first $L$ primes this gives
$\Lambda(F)\ge(\tfrac14+o(1))L^2\log L$.  The engine is a prefix-sensitive
comparison of the infinite $\ell^1$ tails of normalized exponential sums with
prescribed positive-integer zeros, strict unless the zeros are consecutive.
We isolate the zero count behind the comparison as an interlacing lemma.
...
norms over monic multiples~\cite{dubickas,schinzel,schinzel-complex,dj}, and
prescribed-vanishing problems of low height form a related
line~\cite{bombieri-vaaler}.  The height $\max_j|f_j|$ and the length
$\sum_j|f_j|$ are both dominated by a single coefficient.  The mass $\Lambda$
instead accumulates over all coefficients of magnitude exceeding one: it is
the logarithm of the multiplicative height,
$\Lambda(F)=\log\prod_{j:f_j\ne0}\max(1,|f_j|)$, so in any fixed base it
counts, up to an additive constant, the digits of the coefficient vector.
...
studied below: its infinite tail yields their dual lower bound for the reduced
height, and they evaluate the consecutive set $J=\{1,\ldots,c-1\}$
explicitly~\cite[Section~2]{dj}.  Our new ingredient is a sharp monotonicity
principle for the $\ell^1$-tail of this certificate under displacement of
prescribed integer zeros.  To our knowledge, earlier work uses these
certificates to control a single height parameter, whereas the iterative
exclusion argument below produces simultaneous lower bounds for successive
coefficient order statistics.  The displaced-zero comparison is strict, with
...
We prove the following.  Theorem~\ref{thm:tail} is the comparison behind
everything: it bounds the tail of a certificate by the product over its
leading consecutive zeros and shows the bound is attained only there.  For
distinct positive roots $2\le r_1<\cdots<r_L$, Lemma~\ref{lem:slack} and
Theorem~\ref{thm:order} bound the coefficient order statistics of every
multiple, and Corollary~\ref{cor:mass} sums them into a lower bound for
$\Lambda$.  Corollary~\ref{cor:prime} specializes to prime roots, and
...
power of the smallest node to make the sum a real combination of $e^{\lambda
d}$ with distinct $\lambda<0$, apply Rolle's theorem inductively to its
successive derivatives, and note that the $r$-th derivative has no
zero~\cite[Chapter~I, Theorem~4.1]{karlin-studden}.  Consequently an $r$-node
...
\]
and let $A=\{0\}\cup Z'=\{a_1=0<a_2<\cdots<a_{c-1}=m\}$.  Let $r$ be the
number of consecutive pairs $\{a_j,a_{j+1}\}=\{d-1,d\}$ in $A$, and
$J=\{j:a_{j+1}\in Z',\ a_j<a_{j+1}-1\}$, so that $|J|=c-2-r$.  Then
...
$p=|J|=c-2-r$.  Take consecutive $j<j'$ in $J$.  Strictly between
...
the number of zeros in $(a_{j+1},a_{j'+1})$ has the parity of $q+1$ and is at
least $q$; hence it is at least $q+1$, one more than the forced zeros.  The
$p-1$ gaps between consecutive members of $J$ therefore supply
$r+(p-1)+1=c-2$ units of zero multiplicity in all, once the zero at $m+1$ is
added, and this exhausts the zero-multiplicity budget of $\Delta$; in
particular there are no other zeros.  Here the $q+1$ zeros in a gap are
counted with multiplicity, and the $p-1$ units beyond the forced $q$ are
distinct from those forced zeros and from one another.
...
\[
  |F_d|>\mu|G_d|\quad(1\le d<z),
  \qquad
  |F_d|<\mu|G_d|\quad(d>z),
\]
with equality only where $|u_d|=0$.  For an exponential sum $V$, write
\[
...
Assume $|f_D|=1$.  Fix $0\le u\le(L-1)/2$.  For every set $U$ of $u$
...
\begin{proof}
Put $c=L-u$ and use the $c$ largest roots $\rho_1=r_L>\rho_2=r_{L-1}>\cdots>
\rho_c=r_{u+1}$.  A position $j$ has backward distance $d=D-j$.  Let
$W\subseteq\N_{>0}$ be the set of distances of the positions in $U$, and let
$w$ be the certificate on these $c$ nodes, with $w_0=1$ and with zero set $Z$
consisting of $W$ together with the first $L-2u-1$ positive distances outside
$W$.  The $u$ positions have distinct distances, so $|W|=u$ and this gives
$|Z|=c-1$, as required; write $w_d=\sum_i a_iy_i^d$.

The dual-certificate identity is explicit.  Since $F(r_i)=0$ for the $c$
chosen roots,
\[
  0=\sum_{i=1}^c a_ir_i^{-D}F(r_i)
...
The set $Z$ contains $\{1,\ldots,L-2u-1\}$: a distance $t\le L-2u-1$ is in
$W\subseteq Z$ if occupied by $U$, and otherwise is among the first
$L-2u-1$ distances outside $W$.  Thus the leading run of $Z$ is at least
$L-2u-1$, and Theorem~\ref{thm:tail} bounds the tail by the product over the
$L-2u$ largest nodes.  These are $\rho_1,\ldots,\rho_{L-2u}$, that is,
$r_L,r_{L-1},\ldots,r_{2u+1}$; the last index is
$L-(L-2u)+1=2u+1$, which is why the product below starts there.  Therefore
\[
  \sum_{j<D,\ j\notin U}|w_{D-j}|
  \le\sum_{d\ge1}|w_d|=\Tail(w)
  \le\prod_{i=1}^{L-2u}\frac{\rho_i^{-1}}{1-\rho_i^{-1}}
  =\frac{1}{\prod_{i=2u+1}^L(r_i-1)}.
\]
Substituting into \eqref{eq:dual} gives
$1\le\bigl(\max_{j<D,j\notin U}|f_j|\bigr)/\prod_{i=2u+1}^L(r_i-1)$, which is
the claim.
\end{proof}

\begin{theorem}[Coefficient order statistics]\label{thm:order}
If $|f_D|\ge1$, then for every $0\le u\le(L-1)/2$,
\begin{equation}\label{eq:order}
  b_{u+1}\ge\prod_{i=2u+1}^L(r_i-1).
\end{equation}
\end{theorem}
...
$(u+1)$-st largest avoids.  Apply this with $T=\prod_{i=2u+1}^L(r_i-1)$ to
...
$\Lambda(F)\ge\sum_{u=0}^{\lfloor(L-1)/2\rfloor}\log b_{u+1}$, the terms on
...
the $i$th prime.  Then
\[
 \Lambda(F)\ge\Bigl(\frac14+o(1)\Bigr)L^2\log L .
\]
In particular $\Lambda(F)=\Omega(L^2\log L)$.
\end{corollary}

\begin{proof}
Write $p_i$ for the $i$th prime, so $r_i\ge p_i\sim i\log i$ and
$\log(r_i-1)=\log p_i+O(1/p_i)$.  A term with index $i$ occurs in
\eqref{eq:mass} once for each $u$ with $2u+1\le i$, that is, exactly
$\lceil i/2\rceil$ times.  Hence
\[
 \Lambda(F)\ge\sum_{i=1}^L\lceil i/2\rceil\log(r_i-1)
 =\sum_{i=1}^L\Bigl(\frac i2+O(1)\Bigr)\bigl(\log i+\log\log i+o(1)\bigr),
\]
and $\sum_{i\le L}i\log i=(\tfrac12+o(1))L^2\log L$, so the remaining terms
are $o(L^2\log L)$.  This is $\bigl(\tfrac14+o(1)\bigr)L^2\log L$.
\end{proof}
...
\begin{proof}
For $\varepsilon>0$ and $i=1,\ldots,c$ put $y_{i,k}(\varepsilon)=y_i
(1-k\varepsilon)$ for $k=0,\ldots,e_i-1$.  For small $\varepsilon$ these $m$
values lie in $(0,1/2]$, are distinct, and stay separated between distinct
groups.  For each such $\varepsilon$ let $u^{(\varepsilon)}$ be the unique
exponential sum on these $m$ distinct nodes with $u^{(\varepsilon)}_0=1$ and
...
It remains to pass to the limit.  For each $\varepsilon$ the coefficients
$a^{(\varepsilon)}_{i,k}$ solve $M(\varepsilon)a^{(\varepsilon)}=e_0$, where
$M(\varepsilon)$ is the $m\times m$ matrix with rows indexed by the
interpolation nodes $d\in\{0\}\cup Z$ and columns by $(i,k)$, with entry
$d^k y_{i,k}(\varepsilon)^d$, and $e_0$ is the first standard basis vector;
$M(\varepsilon)$ is invertible for $\varepsilon>0$ because the nodes are
distinct.  Its entries are polynomials in $\varepsilon$, so
$M(\varepsilon)\to M(0)$ as $\varepsilon\to0$, where $M(0)$ is the confluent
matrix with entry $d^ky_i^d$.  The functions $d\mapsto d^ky_i^d$ form an
extended Chebyshev system on $\R$, for which confluent Hermite interpolation
is unisolvent~\cite[Chapter~I, Theorem~4.1]{karlin-studden}; equivalently
$M(0)$ is invertible.  Matrix inversion is continuous on the open set of
invertible matrices, so $a^{(\varepsilon)}\to M(0)^{-1}e_0$, and for every
fixed $d$,
\[
  u^{(\varepsilon)}_d=\sum_{i,k}a^{(\varepsilon)}_{i,k}d^ky_{i,k}
  (\varepsilon)^d\longrightarrow\sum_{i,k}a_{i,k}d^ky_i^d=u_d .
\]
...
degree of $F$ and all additional real or complex roots.  The contribution
