...
distinct real roots greater than one.  At a single root $r\ge2$ every row is
a top row: the $k$-th row for $(x-r)^L$ equals the top row for
$(x-r)^{L-k+1}$.  For $1<r<2$ the rows are given by prefix-constrained
problems whenever that top row is at most one, and in all but finitely many
rows of each diagonal.  Complex roots in full orbits of $x\mapsto\zeta x$
...
Section~\ref{sec:singleroot} takes all $L$ roots equal to one real $r>1$.
Let $V_r(L,k)$ be the infimum of $b_k(F)$ over monic multiples $F$ of
$(x-r)^L$, and $\beta_r(n)=V_r(n,1)$.  For every $r\ge2$,
\[
  V_r(L,k)=\beta_r(L-k+1)\qquad(1\le k\le L)
\]
(Theorem~\ref{thm:allrowstwo}); at $r=2$ this sharpens
Theorem~\ref{thm:everyrow}.  The proof inserts one zero at the first gap of a
minimizer, bounds the cost of the insertion by first-order optimality
(Lemma~\ref{lem:optins}), and covers every later exempted position at once by
a crossing argument (Theorem~\ref{thm:crossing}).  For $1<r<2$ the rows lie
within a factor tending to $1$ as $k$ grows of prefix-constrained values
(Theorem~\ref{thm:prefixrows}), and equal them whenever $\beta_r(L-k+1)\le1$
(Theorem~\ref{thm:onepolyrows}) or $k$ is large compared with $L-k$
(Theorem~\ref{thm:longdiag}).
...
\section{Rows at a single root}\label{sec:singleroot}

This section determines the rows when all $L$ prescribed roots are equal to
one real $r>1$.  Write $V_r(L,k)$ for the infimum of $b_k(F)$ over monic
multiples $F$ of $(x-r)^L$ and $\beta_r(n)=V_r(n,1)$ for the top row.  For
$r\ge2$ every row equals a top row, $V_r(L,k)=\beta_r(L-k+1)$
(Theorem~\ref{thm:allrowstwo}): exempting $k-1$ coefficient positions costs
exactly $k-1$ of the $L$ roots.  For $1<r<2$ the rows are determined up to a
factor tending to $1$ by prefix-constrained problems
(Theorem~\ref{thm:prefixrows}), and exactly whenever $\beta_r(n)\le1$
(Theorem~\ref{thm:onepolyrows}) or $k$ is large compared with $n$
(Theorem~\ref{thm:longdiag}).  The first subsections set up the reduction to
exempted sets and zero sets, together with finite certificates, prefix values
and the crossing argument; Sections~\ref{sec:allrowstwo}--\ref{sec:longdiag}
contain the main theorems.  Appendix~\ref{sec:holeproducts} collects
the earlier route through the hole product of a top-row optimum.
...
\subsection{Monotone prefix values}\label{sec:prefixmono}

The conjectured value of the rows is $\min_{i\le k}1/\nu_i(n)$ (after
Corollary~\ref{cor:belowtwo}).  This subsection shows that the prefix values
...
\subsection{Crossing an exempted position}\label{sec:crossing}
...
be paid for.  This subsection shows that the last exempted position costs at
...
\subsection{All rows at every root at least \texorpdfstring{$2$}{2}}\label{sec:allrowstwo}
...
near $r=3.55$.  This subsection takes another route and proves the
...
Theorem~\ref{thm:everyrow} to $b_k(F)\ge\beta_2(L-k+1)$, as anticipated in
...
\subsection{Rows below the root \texorpdfstring{$2$}{2} from one polynomial}\label{sec:onepoly}

Theorem~\ref{thm:allrowstwo} settles the rows at every $r\ge2$.  For
$1<r<2$ it gives only $V_r(L,k)\ge(r-1)^{k-1}\beta_r(n)$, while the
conjectured value is $\min_{i\le k}1/\nu_i(n)$.  This subsection proves the
conjecture for $1<r<2$ whenever $\beta_r(n)\le1$, for every $k$
...
\subsection{Long diagonals below the root \texorpdfstring{$2$}{2}}\label{sec:longdiag}
...
backward distance $j$.  As in the proof of Proposition~\ref{prop:quadratic},
