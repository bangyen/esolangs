...
$\Lambda(F)=(\tfrac12+o(1))L^2\log L$, asymptotically sharp.  The engine is a
...
line~\cite{bombieri-vaaler,bbbp-multiple,bbbp-ray}.  The height $\max_j|f_j|$ and the length
...
Theorem~\ref{thm:order} bound all $L$ coefficient order statistics of every
multiple, and Corollary~\ref{cor:mass} sums them into a lower bound for
$\Lambda$.  Corollary~\ref{cor:prime} specializes to prime roots and gives an
asymptotically sharp bound, and Proposition~\ref{prop:confluent} removes the
distinctness of the nodes.
...
$\{z_{i,j}^{d}\}_{j<e_i}$ to
$\{\phi_{i,k,\varepsilon}\}_{k<e_i}$ is triangular with unit diagonal,
hence invertible, and the same function space is spanned either way.
...
standard basis vector.  The entries are polynomials in $\varepsilon$, so
$M(\varepsilon)\to M(0)$ as $\varepsilon\to0$, where
$\phi_{i,k,0}(d)=d^ky_i^d$ and $M(0)$ is the confluent matrix.  The functions
...
direction, the degree of a polynomial with all roots rational is at most a
constant times the logarithm of its height~\cite{htv}, and the minimal height
of a degree-$n$ polynomial splitting over a fixed number field grows
exponentially in $n$~\cite{tromp}; the bound here constrains neither degree
nor height but the additive coefficient mass.  The contribution
