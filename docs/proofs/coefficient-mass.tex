...
\[
  ...
  \qquad \det M=\tfrac14\ne0 .
\]
Hence the matrix is invertible for all large $D$ and its inverse converges to
...
$5\le D\le25$.  Theorem~\ref{thm:order} with $u=1$ gives $b_2\ge8$ for every
...
$6\le D\le30$.  Theorem~\ref{thm:order} with $u=2$ gives $b_3\ge4$, and
...
$S_j=c_0+\cdots+c_j$ of the coefficients of $P_u$, and they stabilize at
...
$57>48$; the search returns $\min b_2\approx54.97$, above \eqref{eq:order}.
...
\eqref{eq:partial} holds with equality and the search returns the bound.
...
every $c$ with ratio $a/c$, while the stated bound $a/(1-a)$ is already
...
$k=3$.  From
$L=4$ the construction behind both fails, and the remark after
Proposition~\ref{prop:sharp235} records root sets where the search stays well
above \eqref{eq:order}.
...
not certified values; the one exact statement among them is
\eqref{eq:sharp23}.
...
that description; the polynomials of Proposition~\ref{prop:sharp23} are exact
