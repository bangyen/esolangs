...
Proposition~\ref{prop:cutoff} shows that the node cutoff behind the tail
theorem---equivalently the standing restriction $r_i\ge2$---cannot be
relaxed, and Proposition~\ref{prop:confluent} carries the tail comparison
itself to confluent nodes.
...
pair $\{4,5\}$ forces a zero of $\Delta$ at $5$, and the sign change across
the longer gap $(4,7)$ forces one further unit of multiplicity in it, possibly
at $5$ itself.
...
$r_i\ge2$, and that restriction is not an artifact of the proof: the tail
bound rests on the node cutoff $y_i\le1/2$, which
Proposition~\ref{prop:cutoff} shows cannot be raised, so roots in $(1,2)$ lie
outside the reach of this method rather than merely outside its current
statement.
