...
$k\le r$, is nowhere zero~\cite[Chapter~I,
Theorem~4.1]{karlin-studden}; the full Wronskian alone would not suffice, as
...
signs~\cite[Part~V]{polya-szego}.  Consequently an $r$-node sum with $r-1$
...
$p_0^3/\delta$, so $b_2=p_0^3/\delta<2=r_2-1$, which is what \eqref{eq:order} would assert for
$u=1$; the second root obeys $r_2\ge2$, so only $r_1$ is out of range.
\end{proof}
...
\]
Both systems are nonsingular and $F_D$, $G_D$ are monic multiples of
$(x-2)(x-3)(x-5)$; for all large $D$ one has $b_2(F_D)=t_D$ and
$b_3(G_D)=s_D$, and $t_D\to8$, $s_D\to4$.  Hence
...
The nonleading coefficient magnitudes of $F_D$ are $a_D$ once, $t_D$ exactly
$D-2$ times, and $|c_D|$ once.  Since $a_D\to7$, $t_D\to8$ and
$|c_D|\to\infty$, for large $D$ the largest is $|c_D|$ and the second largest
is $t_D$, so $b_2(F_D)=t_D$; this ordering is verified directly for every
$5\le D\le120$.  Theorem~\ref{thm:order} with $u=1$ gives $b_2\ge8$ for every
monic multiple, and $t_D\to8$.
...
each.  The run occupies $D-2\ge3$ positions, so whatever the two exempt
magnitudes do, the third largest is $s_D$: $b_3(G_D)=s_D$ for every $D\ge6$
with $s_D>0$, which the limit $s_D\to4$ and a direct check on $6\le D\le120$
confirm.  Theorem~\ref{thm:order} with $u=2$ gives $b_3\ge4$, and
...
exact family of \eqref{eq:wgen} giving $b_2=60.05$ at degree $28$ and
$60.002$ at degree $40$.  The first three are cap-$28$ search values, falling
geometrically with the cap; at $(3,4,5,6)$ the double-precision program
instead stalls past degree $25$, so there the family and not the search
certifies the descent.
Across the $15$ root sets tried, \eqref{eq:partial} agreed with attainment
every time, in both directions.

When \eqref{eq:partial} holds and the roots are distinct, the construction is
available and \eqref{eq:order} is an infimum: the argument of
Proposition~\ref{prop:sharp235} carries over, its Descartes step now reading
$L+1$ terms against the $L+1$ positive roots $1,r_1,\ldots,r_L$.  Distinctness
is what the exempt block needs: if $r_u=r_{u+1}$ then $P_u$ vanishes at the
exempt root $r_u$, and the divergence that puts those coefficients above the
run is lost.  Whether \eqref{eq:order} is still an infimum there is open; the
search does not settle it, stalling well above the bound at $(3,3)$.  The converse---that failure of
...
$k=3$.  Beyond those root sets the construction behind both can fail, already at
$L=3$: at $(2,3,\tfrac72)$ with $u=1$ the partial sums are $1,-\tfrac{11}2,5$,
so \eqref{eq:partial} fails and the search returns $5.42$ at degree cap $28$
against the bound $5$.  It can also still succeed at $L=4$, as $(3,4,5,6)$ shows.  The remark
...
is the order-statistic optimum.  Running that search in double precision over
monic multiples of degree at most $L+6$ gives, for roots $(2,3,5)$,
$\min b_2=8.5178$ against $8$ and $\min b_3=4.1218$ against $4$, both within
$6.5\%$ of \eqref{eq:order}; for $(2,3)$ it returns $2.0308$.  All three are
members of the exact families of Propositions~\ref{prop:sharp23}
and~\ref{prop:sharp235}, which carry them to the bound, so for these three
root sets the gap the search reports is an artifact of its degree cap.  These are floating-point optima at a degree cap,
not certified values; the exact statements are \eqref{eq:sharp23} and
\eqref{eq:sharp235}.
...
degree, solved in double-precision floating point---and are reproducible from
that description; the polynomials of Propositions~\ref{prop:sharp23}
and~\ref{prop:sharp235} are exact and need no computation.  The certificate
