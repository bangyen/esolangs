...
\emph{Across annuli.}  Theorem~\ref{thm:thingauss} charges each thin tail
of the roots, $K_s^2\log s$ for the $K_s$ roots of modulus at least $s$, but not
the cross terms $K_sK_t\log t$ ($s<t$) of $\sum_jj\log\rho_j$ between
different annuli; Corollary~\ref{cor:separated} gets cross terms only between
single representatives of separated annuli.  For $F$ real and the two rays
$\arg z=\pi/2\pm\delta'$ the two crossing polynomials of
Lemma~\ref{lem:sectorcount} are one: with $H(r)=F(re^{i(\pi/2-\delta')})$,
$F(re^{i(\pi/2+\delta')})=\overline{H(-r)}$, so the real polynomial
$\operatorname{Im}(e^{-i\theta}H(r))$ has at least $2K-2dE(\delta')/\pi-2$
real zeros $r$ with $r_1<|r|<r_2$, of both signs.  A mass bound for real
multiples with prescribed real roots of both signs, of the form
$c\sum_ii\log(|r_{(i)}|-1)$ over the roots ordered by modulus (the pairs
$\pm r$ of Corollary~\ref{cor:imag} show $c\le\frac12$), is the natural
missing ingredient: together with intermediate arcs that localize the
crossings at each scale, it would charge the cross terms for register roots
that are thin at every scale, whereas Theorem~\ref{thm:order} charges only
the larger of the two sign classes.
