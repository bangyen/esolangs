...
$(2,3)$ an explicit family attains the $u=1$ bound in the limit, so the
statistics are sharp there.
...
\]
with the coefficients fixed by vanishing at $2,3,5$.  Then $b_2(F_D)=t_D\to8$
and $b_3(G_D)=s_D\to4$, the values \eqref{eq:order} asserts at $u=1$ and
$u=2$; we omit the verification.  Whether the rows $k\ge2$ are sharp in
general is open.
