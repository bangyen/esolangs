...
that infimum for every root set here.  A companion paper~\cite{coefficient-mass-attainment}
takes up attainment: explicit monic multiples approaching \eqref{eq:order}
at the root sets $(2,3)$ and $(2,3,5)$, the necessity of the node cutoff
$y_i\le1/2$ and of the standing restriction $r_i\ge2$ for $k\ge2$, and the
confluent extension of Theorem~\ref{thm:tail}.
...
$\Tail(u)=364/2835>1/9=y_1/(1-y_1)$; the companion
paper~\cite{coefficient-mass-attainment} shows that no cutoff larger than
$1/2$ is universal.
...
A companion paper~\cite{coefficient-mass-attainment} takes up attainment:
whether \eqref{eq:order} can be approached by an explicit multiple, whether
the node cutoff $y_i\le1/2$ in Theorem~\ref{thm:tail} and the standing
restriction $r_i\ge2$ in Theorem~\ref{thm:order} are necessary, and whether
the tail comparison extends to confluent nodes.
...
Theorem~2.2 (Tail comparison), Theorem~3.2 (Coefficient order statistics).
\end{thebibliography}
