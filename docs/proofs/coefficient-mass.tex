...
carried through their Corollary~7 and discharged only case by
case~\cite[Section~2]{dj}; our consecutive base case,
...
each, and both exempt coefficients grow like $3^D$ while $s_D\to4$, so they
are the two largest and $b_3(G_D)=s_D$; this is verified directly for every
$6\le D\le120$.  Theorem~\ref{thm:order} with $u=2$ gives $b_3\ge4$, and
...
$60$, reaching $60.12$ at that cap.  These figures fall geometrically with the
cap and are limits, not values at any finite degree.
...
so \eqref{eq:partial} fails and the search returns $5.42$ against the bound
$5$.  It can also still succeed at $L=4$, as $(3,4,5,6)$ shows.  The remark
...
and~\ref{prop:sharp235}, which carry them to the bound, so at $L\le3$ the
gap the search reports is an artifact of its degree cap.  These are floating-point optima at a degree cap,
