...
\end{equation}
The row $k=1$ is Proposition~\ref{prop:rowone}; for roots of one sign
\eqref{eq:bothrows} is weaker than Theorem~\ref{thm:order}; for pairs $\pm s_j$
it follows from the multisection transfer, since
$s^2-1\ge(s-1)^2$; and it is what \eqref{eq:bothrowsbound} gives, up to the
factors $\rho$ and $(1-\rho)^{-k}$, when all moduli are equal.  In exact
