...
separately~\cite[Section~2]{dj}; our consecutive base case instead reads the
sign off the complete homogeneous symmetric polynomials $h_j$.  Their inequality~(8) already excludes a
prescribed set of positions from the certificate.  The new ingredient is a
...
nothing is discarded: there $Z=\{1,\ldots,L-1\}$, so $\Tail(w)$ is the
consecutive base case of Theorem~\ref{thm:tail}, an exact evaluation needing
only $y_i<1$, and the bound holds already for $r_1>1$.  Therefore
\[
  \sum_{j<D,\ j\notin U}|w_{D-j}|
...
For $u=0$ the certificate of Lemma~\ref{lem:slack} is the consecutive base
case of Theorem~\ref{thm:tail}, which needs only $y_i<1$ and discards no
...
Lemma~\ref{lem:slack}, and both matter only for $u\ge1$; the $k=1$ bound and
the consecutive base case of Theorem~\ref{thm:tail} are free of it, as is
...
that even there the root product need not be the finite-$L$ minimizer.  Linear
programming over monic multiples of degree at most $L+6$ puts the minimum of
$b_k$ within $1.5$--$6.5\%$ of \eqref{eq:order} for $k\ge2$---for roots
$(2,3)$, $\min b_2=2.031$ against $2$, and for $(2,3,5)$, $\min b_2=8.518$
against $8$ and $\min b_3=4.122$ against $4$---so in those cases the bound
is nearly attained.
