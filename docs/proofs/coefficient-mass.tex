...
For $|f_D|\ge1$ the bounds sum to a lower bound
...
multiple, and for $|f_D|\ge1$ Corollary~\ref{cor:mass} sums them into a lower
bound for $\Lambda$:
\[
...
at most the product over the $L-u$ largest nodes.  These are
...
Let $r_1\le\cdots\le r_L$ be real with $r_1\ge2$, repetitions allowed, and
let $F$ be a nonzero multiple of $\prod_{i=1}^L(x-r_i)$ with $|f_D|\ge1$.
Then
...
\begin{proof}
Each product in \eqref{eq:order} is at least one because $r_i\ge2$, and
$|f_D|\ge1$, so the leading coefficient and the $L$ largest nonleading
magnitudes are all at least one.  These are $L+1$ distinct positions, so
\[
...
\[
  q_0=\frac{p_0^2}{D},\qquad q_1=\frac{p_0(p_0-p_1)}{D},\qquad
  D=(p_0-p_1)^2+p_0(p_1-1),
\]
whence the common value is $f_0=f_1=f_2=p_0^3/D$.  Here
$D=\tfrac{225}4>0$, so
\[
...
The four nonleading magnitudes are $|f_3|$ and three copies of $p_0^3/D$, so
$b_2=p_0^3/D<2=r_2-1$, which is what \eqref{eq:order} would assert for
...
\]
Every $D$-dependent entry is $O((3/5)^{D})$, so as before the solution
...
\eqref{eq:partial} holds with equality and the search descends to the bound
$60$, reaching $60.12$ at that cap.  All four are cap-$28$ values, still
falling geometrically with the cap toward infima no finite degree attains.
Across the $15$ root sets tried, \eqref{eq:partial} agreed with attainment
every time, in both directions.

When \eqref{eq:partial} holds the construction is available and
\eqref{eq:order} is an infimum, by the argument of
Proposition~\ref{prop:sharp235}.  The converse---that failure of
...
order-statistic bounds are summed into $\Lambda$.  In the opposite direction,
