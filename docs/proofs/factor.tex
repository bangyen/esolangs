...
$24.6\,T\log T$ at every arity, falling to $35/\log 10<15.21$.  Running the
count with the instruction primes themselves makes the lower bound explicit
too, at $1/(3\log10)$, so the constant is bracketed within a factor of $105$;
we show that a factor of at most $2$ of that gap is number theory and locate
the rest in the decision tree.
\end{abstract}
...
Corollary~\ref{cor:grh} and Proposition~\ref{prop:floor} bracket the
constant of $\mathcal C_F(T)/(T\log T)$ between $1/(3\log10)$ and
$35/\log10$, a ratio of exactly $105$ in any base.  It factors.  Since there
are only $8^{\ell}$ programs of length $\ell$, a $T$-bit table needs at least
$T/3$ instructions, while Lemma~\ref{lem:tree} emits $\frac{35}{2}T$ of
them: a factor of $52.5$.  The remaining factor of $2$ is
...
pay $\log U$.  So at most a factor of $2$ of the gap is number theory, and
the rest is the decision tree.
\end{remark}
