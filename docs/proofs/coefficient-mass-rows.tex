...
For $r\ge2$ the loss $E_k(r)$ is not small.  We conjecture that the upper
bound of~\eqref{eq:prefixsandwich} is always attained:
\[
  V_r(L,k)=\min_{1\le i\le k}\frac1{\nu_i(n)}\qquad(r>1,\ 1\le k\le L,\ n=L-k+1).
\]
For $r\ge2$ this follows from Theorem~\ref{thm:allrowstwo} (see the remark
...
This section proves the conjecture stated after
Corollary~\ref{cor:belowtwo} at every root $r\ge2$
...
also gives $\min_{i\le k}1/\nu_i(n)=\beta_r(n)$ and proves the conjecture
stated after Corollary~\ref{cor:belowtwo} there.  So the rows at a single
root $r\ge2$ are
\[
  V_r(L,k)=\beta_r(L-k+1)\qquad(1\le k\le L),
\]
the top row of a multiple of $(x-r)^{L-k+1}$.  At $r=2$ this sharpens
\cite[Theorem~4.16]{coefficient-mass} to $b_k(F)\ge\beta_2(L-k+1)$.  What
...
conjectured value $\min_{i\le k}1/\nu_i(n)$ is at least
$(r-1)^{k-1}\beta_r(n)$.  Section~\ref{sec:onepoly} proves the conjecture
...
This subsection proves the conjecture for $1<r<2$ whenever $\beta_r(n)\le1$,
...
Beyond $\beta_r(n)\le1$ the conjecture remains open for $1<r<2$ where no single
polynomial serves all prefixes.  Section~\ref{sec:longdiag} shows that a
