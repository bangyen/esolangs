...
| Polynomial | tree, cap | each finite instruction list has a finite prime-product encoding | open: multiplicity is priced at Omega(T**2 / log**2 T) by the confluent certificate; the extra log needs the sharpened routing lemma (L_real = Omega(T/log T) distinct real roots), unproved ([polynomial](polynomial.md)) |
