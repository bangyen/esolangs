...
by state-cycle detection for forty-three string-based step-capable machines
...
Nevermind, BF-PDA, 3x, Sophie, Jaune, SLOW ACV MAMMALIAN, ZTOALC L), and
keeps the SIGALRM backstop for the rest (Painfuck's `y` is
non-deterministic).
