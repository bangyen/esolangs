...
by state-cycle detection for forty string-based step-capable machines
...
Nevermind, BF-PDA, 3x, Sophie), and keeps the SIGALRM backstop for the
rest (Painfuck's `y` is non-deterministic).
