...
by state-cycle detection for forty-one string-based step-capable machines
...
Nevermind, BF-PDA, 3x, Sophie, Jaune), and keeps the SIGALRM backstop for
the rest (Painfuck's `y` is non-deterministic).
