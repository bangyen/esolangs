...
| 3 | `-7` | `d`: read cell 3 is `*d`; `-7` reads as the constant `0`. |
| 4 | `65` | data: the byte that gets printed. |

The interpreter executes the single instruction at address 0:

1. Read `*d = *(-7) = 0`.  `0` is not positive, so take the **else**
   branch: `*a += *b`, i.e. `*(-1) += *(4) = 65`.  Writing to `-1` prints
   the value being written, so the byte `65` (`'A'`) is output.
