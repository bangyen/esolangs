...
`Forþ` keeps a stack of signed 32-bit integers.  A digit `0`-`9` pushes its
value; `+`/`*` (and the other arithmetic ops) replace the top two values
with their result (the top goes on the right); `.` prints the top's low
byte; `[` loops while the top is nonzero; and any other character is
ignored.
