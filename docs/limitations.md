...
| WII2D | `n <= 4` exact (exhaustive through three, sampled dense at four); symmetric tables of any arity via closed forms; dense non-symmetric sampled built at `n == 5` and `n == 6`, rejected from `n == 7` | **Cost guard, not a wall — liftable by paying build time and width.** The `n == 7` refusal is `_WII2D_MAX_INDEX_DOMAIN = 32`, checked against the decode domain `2 ** (n - 1)` before `_wii2d_decode` runs, so it never established that anything fails. It does not: with the constant raised to 64 a dense non-symmetric `n == 7` table builds in 1.74s, interpreter-verified on all 128 input combinations. What the guard buys is now **width, not time**. The build tail it used to buy was a single failure mode — the *doubling trap*, where `s` squaring outruns a `_wii2d_compress` that cannot halve without colliding two values needing different bits, leaving the search doing arithmetic on thousand-digit integers. A magnitude-first retry (`_WII2D_MAX_STATE_BITS`) clears it: 14 sampled `D == 64` patterns now all decode, median 1.28s and worst 8.07s, against four exceeding a 120s budget and a 55.73s worst before, and one pattern that had not finished in 595s takes 2.73s. It is a fallback rather than the default ranking because magnitude-first is no strict improvement (8 wins, 6 losses, 9 ties over 25 patterns), and shipped domains never reach the threshold, so every shipped program is byte-for-byte unchanged. What still argues for the default is size — an `n == 7` decode is thousands of cells (median 4762, worst 19448 at `D == 64`) — and a second knob: the fold squares, so peak `|acc|` reaches hundreds to low thousands of bits on sampled `n == 7` decodes; the wiki states no accumulator bound and this interpreter uses arbitrary-precision integers, so wide builds rest on spec silence — though shipped `n == 5` already passes one byte. Symmetric tables never reach the check (majority-of-12 is 397 characters, instant). Completeness past the exhaustively-tested `D == 16` is open, not refuted. Measurements in [`docs/walls.md`](walls.md). |
