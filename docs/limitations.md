...
| 6-5 | none — **total**: every truth table renders at every `n`, bounded only by program size (the tree is `O(2**n)`) | **Wall lifted.** This row used to read `n <= 5`, on the premise that 6-5 has "exactly 35 branch labels (`0..9`, `A..Z`)", one per internal node the constant-subtree fold leaves standing. The label *accounting* was right — `8n` scans from the start of the program for the n-th `4`, so a label is a global ordinal and no label can ever be reused — but there is no 35. The interpreter decodes a `7n`/`8n` operand as `num(c) = ord(c.upper()) - 55`, an arithmetic decode over *every* character rather than a lookup in a 36-entry alphabet, and the tokenizer merges whatever follows a `7`/`8` without inspecting it; `0..9A..Z` merely happens to be where that decode lands on 1..35, and the range continues past `Z` without end. The only gaps are ordinals whose character would be lowercase, which `.upper()` aliases down (`num("a") == 10`), and those are skipped by emitting one inert `4` apiece — a bare `4` is a no-op the marker scan still counts, so padding walks past a dead ordinal at one character each. `_label_for` decides nameability by round-tripping through the interpreter's own `num`, so the generator cannot drift from it. Parity at `n == 6/7/8` — the shape no input reordering folds, and the old refusal witness — now builds and was executed through the interpreter on all `2**n` combinations. The input-order search survives as a pure size choice. The retired arithmetic fallback stays retired, now redundant twice over. Full argument in `docs/walls.md`. |
