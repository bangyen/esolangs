...
| --- | --- |
| 123 | **A generator ships; the cap is arity, not expressiveness.** It is parameterized (`2` at location -3 reads real stdin, so a decision tree cannot read its inputs) and answers with the termination convention — halt for 0, a proven loop for 1 — the same one ArrowQueue and Point Break use. All four one-input and all sixteen two-input tables are built. The fill is `1` for a one and `2` for a zero, which moves the pointer in opposite directions, so displacement after the embeds is `(#zeros - #ones)` and the `-4 -> 0` wrap reduces it modulo four: a tail of `1`s decodes that counter, giving a function of popcount alone (XNOR at two ones, XOR at four), and `3`'s TRUE-backward re-run supplies the asymmetric tables. Three-input and wider tables are **rejected**, including those depending on only two of their inputs: an ignored input still has to be embedded, every fill moves the pointer, and the pointer phase *is* the computed value here, so a trailing inert embed shifts the very quantity the plan decodes — the projection trick that lifts other generators' caps does not apply. Whether a wider construction exists is open; see `docs/walls.md`. |
...
| --- | --- | --- |
| NoComment | `n <= 11` | Liftable by host config, and the old `n <= 8` was not a wall at all. That row read "the `s` skip is byte-indexed, capping every jump at 255", which is true of a *single* jump and says nothing about a composition of them — and the generator's decode was a single skip carrying the whole index, so it broke at nine inputs. Skips compose two ways: `s` peeks the stack rather than popping and does not move the pointer, so a guarded region of any length is a chain of sub-256 chunks re-testing the same guard; and entering a staircase of `L` `l`s by skipping `c` moves the pointer `L - c`, so consecutive staircases *add* their displacements and a byte-sized skip per stage reaches any index. Past eight inputs the generator now splits the index into byte-sized summands and walks one staircase each. What binds instead is the interpreter's tape, which the `2**n` output cells plus a guard apron exhaust at `n == 12` against the default 4096 cells — and while the wiki does require the memory space to be static (pointer overflow moves to the opposite end), it never specifies a *size*, so like the Factor row this is the interpreter's configuration, not the language.  Both `run` and the generator take a `tape` argument for that reason: `n == 12` builds and runs at `tape=16384`, and the default stays 4096 because the size is observable through the wrap. The wiki specifies the peek (`s` is "peek (do not pop)") and the byte-sized cell, so the chain is spec-legal rather than a tolerated implementation quirk: instrumented runs show no skip amount and no written cell ever exceeds 255. Verified by exhaustive interpreter runs over all `2**n` inputs at `n == 9`, `10`, and `11`; `n <= 8` still renders byte-identical programs. Full argument in [`docs/walls.md`](walls.md). |
