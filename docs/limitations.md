...
| %^2^-1 | Only control flow is `t` (rewind on a nonzero accumulator); a whole-program while loop that cannot count passes. No program that *reads* its inputs computes any two-input function (proved in Lean), but the shipped boolean generator is parameterized and builds all sixteen: it embeds the bits, composes one affine setter per input into a product-weighted accumulator, and prints with `l`, which spells the accumulator in decimal so `0`/`1` need no branch. The setters are derived from the table (each column's slope is read off it, and the offsets are then solved) rather than searched. Total at `n <= 2`.  Above two inputs a second construction takes over, the **minterm cascade**: the accumulator is loaded with 1 (`ips`) and passed through one setter per input, the identity (`pp`) where the bit matches the minterm and an erase (`'p`) where it does not, so the accumulator survives as 1 exactly when every bit matches.  Both branches are two characters, so nothing leaks through `len()`, and the program is `2n + 4` characters at any arity.  This lifts the old `n <= 2` cap for the single-minterm family (AND-`n` and every one-row table) with no search.  Tables with several ones are still rejected rather than risking a wrong program: OR-ing indicators needs a running total to survive a gadget that erases, and there is one register. |
...
| %^2^-1 | all tables at `n <= 2`; single-minterm tables at any `n` | Partly lifted.  The derived path composes one affine map per input into a shared value, which forces each cofactor of the table to be constant or an affine image of one shared function -- 88 of the 256 three-input tables satisfy that, which is why it capped at two.  The minterm cascade escapes it by making the *erase position* input-dependent, a branch realised arithmetically.  What remains walled is the multi-minterm OR, on the one-register obstruction; the printing tail is provably maximal at two values (without the over-3003 reset every tail is affine hence injective, and the reset merges values only into the same class -- an exhaustive sweep of every tail to length 10 never separated more than two). |
...
| ZTOALC L | `n <= 3` exact; popcount-symmetric tables lifted further (e.g. XOR4 at `n == 4`); *some* dense non-symmetric `n >= 4` rejected | Genuine wall: every Collatz trajectory converges to the `16, 8, 4, 2, 1` tail, so a dense non-symmetric tree has leaf collisions past `n == 3`; symmetric tables fall back to a branch-free linear program instead. The refused set is narrower than "all dense non-symmetric `n == 4`", which this row used to claim: choosing the input split order gives the search a differently-shaped tree, and `1010001000011000` — formerly refused — now places under a reordered one (`test_dense_non_symmetric_places_under_a_reordered_tree`). The wall stands for the tables no order rescues. |
