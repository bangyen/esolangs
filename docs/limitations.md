...
| Inject | Not an alphabet wall — `send` writes a label's block verbatim, so any *individual* character is emittable as literal program text. It fails the "arbitrary bytes" bar on shape rather than alphabet: `send` appends `\n` to **every** line it writes (`inject.py:86`), so the only texts it can emit are those ending in a newline, and a payload line matching `\w+;` is re-parsed as a label delimiter. Over a 20-text probe corpus it accepts 1 (the one ending in `\n`); the harness's own `"Hello, World!"` comes out as `"Hello, World!\n"` (executed). A generator here would reject nearly everything, so this is a wall, not a near miss. |
| Jaune | `^` prints the current cell as a decimal integer, so only digits are spellable. |
| Lamfunc | Not an alphabet wall — `_print_value` falls through to `str(value)`, and a trailing bare token becomes a string, so `p Hello` really does print `Hello` (executed). It fails the "arbitrary bytes" bar on tokenization: a token is whitespace-split, so no text containing a space, tab or newline is spellable at all (`p Hello, World!` raises `HaltError: calling undefined function 'Hello,'`), an all-digit token parses as an int and prints in *binary* (`p 42` gives `101010`), and there is no concatenation to assemble a text from several tokens. Acceptance therefore decays geometrically with length — for English-like text (space rate ~1/6) roughly 40% at 5 characters, 2.6% at 20, and effectively nothing at 80 — so a generator would be partial by a very wide margin rather than blocked on one string. |
...
| --- | --- |
| 123 | Parameterized (only reads real stdin at one fixed location, so a decision tree cannot read its inputs); answers with the termination convention. All 276 tables through `n == 3` build and replay correctly, constructed like every other arity (the stored plan tables are retired; small arities build from a bare-fill seed and one derived table-independent separation law per arity, swept exhaustively in the suite). Every stage of the pipeline — embed scrub, planned separation, shield-and-sweep verdict, endgame — is planned and total by argument; nothing searches (see [`docs/walls.md`](walls.md) for the verdict's closed form). Four inputs: exhaustive, all 65536 tables build and replay (re-swept under the tight per-arity geometry, which one table-independent reference run admits per arity, with the proven doubling base as fallback). Five and six inputs: random samples build in well under a second per table, replayed row by row. The work budget remains as a divergence guard so a stage invariant breaking at an unprobed arity raises rather than hanging; it scales with row count and bounds divergence, not arity. |
...
and all six random-drawing ones get
