...
| --- | --- |
| 123 | Parameterized (only reads real stdin at one fixed location, so a decision tree cannot read its inputs); answers with the termination convention. All 276 tables through `n == 3` build and replay correctly, constructed like every other arity (the stored plan tables are retired; small arities build from a bare-fill seed and a few frozen table-independent separation schedules, swept exhaustively in the suite). Every stage of the pipeline — embed scrub, planned separation, shield-and-sweep verdict, endgame — is planned and total by argument; nothing searches (see [`docs/walls.md`](walls.md) for the verdict's closed form). Four inputs: exhaustive, all 65536 tables build and replay (re-swept under the tight per-arity geometry, which one table-independent reference run admits per arity, with the proven doubling base as fallback). Five and six inputs: random samples build in well under a second per table, replayed row by row. The work budget remains as a divergence guard so a stage invariant breaking at an unprobed arity raises rather than hanging; it scales with row count and bounds divergence, not arity. |
