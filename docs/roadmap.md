...
| Toolchain | Languages |
| --- | --- |
| RISC-V assembly | BF-PDA (bit stack + tape), RAM0 (register machine), BIO (register counter loops), Minsky Swap (two registers, a swap pointer, and a jump-on-zero `~`; a decision-tree cascade boolean generator and a trivial ``r0 r1`` dump).  All four are no-input parameterized generators; BF-PDA and RAM0 already have RISC-V compilers to reuse for the syscall/ELF scaffolding, and BIO and Minsky Swap map 1:1 onto RISC-V registers/branches. |
