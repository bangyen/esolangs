...
column marks the boolean-function generators; Back, BIO, Eval, NoComment's, and Dotlang are parameterized (the harness substitutes input bits into a template), and Dotlang reads its answer from termination (halt = 0, hang = 1) rather than output.
