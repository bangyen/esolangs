...
The generator is valid only when its program reaches the same cycle state for every input row.
...
Tests cover the constructed arities; extending arity requires checking both rendered output and cycle stability, not merely source size.
