...
commands, and the sync tests compare that wrapped form character for
character.  Languages with semantic newlines (2D grid ones) or that reject
them (NoComment) must be committed unwrapped, as must any token longer than
the width.
...
(`esolangs.tools.text`).  Run one with the language's interpreter:
...
That version is verified against the Python interpreter like every other,
not a replacement for the example here.
...
no entry in `esolangs.tools.boolean.examples` has no committed file; the
`tests/tools/test_boolean_*.py` modules cover its generator instead.
