...
embed — 7/16 at `n == 2` once both reads are available (5/16 with only the
plain read), each verified end-to-end through the real interpreter with equal
...
`0111` and `1000` resist this route even with 40 candidates at depth 15, and
`1000` builds through the original direct scan, so the two routes are
complementary rather than one dominating the other.

## Every two-input table, via the read polarity

The endgame prints `NOT(v XOR c)`, where `v` is the accumulator column and
`c` is cell 7.  Every pool the walk-and-clamp `pool_fix` reaches conserves
`v XOR c`, so chasing a different pool to print a complement is futile: both
reachable configurations land on the same effective column.  Two earlier
readings here called a pool pattern "unreachable" — both were limits of
`pool_fix`'s *shape*, not of the language.  A direct search over `<[x` finds
`0011000|0` at depth 16 (`minifuck_pool_reach.py`, `minifuck_oriented.py`).

The free variable is the **read**.  Alongside `[<`, which leaves the pointer
at `(acc-1) + v`, there is `[x<[<`: it leaves the pointer at
`(acc-1) + NOT v`, **restores** the cell, and flips its neighbour
unconditionally, so its residue is input-independent.  Swapping the read
flips the printed digit, which makes any table printable whenever its
complement is.

That closes `0111` and with it **every two-input table — 16/16**, each
verified through the real interpreter with a single instantiation length.
The direct scan alone went from 5/16 to 7/16, and programs got shorter
because cheap routes now cover tables that previously needed the expensive
one (`0001` fell from 239 characters to 148).

Because the polarity is free, the endgame's assertion on the pointer offset
had to go: the offset is `v` or `NOT v` depending on the read, so what is
checked is the *printed output* instead.
