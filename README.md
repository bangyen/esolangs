...
truth-table program for 58 languages with a boolean generator (57 built by
their generator, plus one hand-written Minifuck program).  Both sets are
regenerated by `scripts/write_examples.py`.
...
is not passed separately.  63 of the languages in the suite have such a
...
registered in `src/esolangs/registry.py`.  Run `just test` (lint, pytest,
bandit, and the Python verify scripts, scoped to this branch) to verify a
change; `just test-full` runs every step over the whole tree.
`just install-dev` already wires `scripts/verify.py` into a pre-push hook.
