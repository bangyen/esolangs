...
# reach, which is what makes the catalog below finite.
_EVAL_MAX_OPS = 16
# Every reorder program worth emitting: for each arrangement the cap
# admits, the shortest op string reaching it, listed by length and then by
# ``~`` < ``*`` < ``=`` -- the discovery order of the breadth-first search
# this catalog replaced, kept so ties stay byte-stable.  The set is finite
# and complete for *every* ``n``: a non-identity program spends two ops on
# toggles and one on a reversal, so its travel is at most
# ``_EVAL_MAX_OPS - 3`` ops of ``=`` -- it never moves more than six values
# between the stacks, and values deeper than that ride along only in rigid
# whole-stack reversals.  New arrangements therefore stop appearing once
# the stack outgrows that reach: the catalog grows 620, 691, 717, 728,
# 733, 735 over ``n == 7..12``, then never changes (checked through
# ``n == 16``).  ``test_reorder_catalog_matches_search`` replays the
# replaced search as the specification and asserts byte equality.
_EVAL_REORDERS = (
    "",
    *[
        "~*~",
