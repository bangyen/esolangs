...
# Shortest programs from ``'`` (acc = 0) to each byte for %^2^-1, over the
# four ops (``s`` -= 2, ``i`` -= 3, ``m`` *= 2, ``p`` = -acc), staying within
# the interpreter's ``acc > 3003`` reset.  Generated once by a BFS and
# hardcoded so generation is a plain lookup (see verify_extra_generators.py).
_PCT_PATHS: dict[int, str] = {
    0: "",
    1: "ips",
    2: "sp",
    3: "ip",
...
def pct_squared_minus_one(text: str) -> str:
    ...
    ``'`` resets the accumulator to zero, the lookup-table paths move it to
    each byte, and ``e`` prints it as a character.  Every byte's path stays
    within the interpreter's ``acc > 3003`` bound.
    """
    _require_bytes(text, "%^2^-1")
    return "".join("'" + _PCT_PATHS[ord(c)] + "e" for c in text)
