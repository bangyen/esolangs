...
routes the computation.  The Boolean answer is the **origin's colour** after
a whole cycle (white is one, black is zero), read by the test's semantic
grid model (the interpreter's own output is the visited-cell bounding box).

Supported tables (verified exhaustively by running the interpreter):
- ``n == 1``: all four one-input functions.
- ``n == 2``: all sixteen two-input functions.
- ``n >= 3``: not reachable; the discriminator's conditional read splits the
  ant's position and the branches cannot re-converge on a stable path, so
  multi-gate composition has not been found.  Like Minifuck's ``n <= 2``
  cap, this is a documented wall (``docs/walls.md``).

The two-input templates were found by search and verified exhaustively
against the interpreter for every instantiation; they are all fixed points
(cycle-stable).  Each base string has two slot positions, and each slot is
filled with ``{Xi}`` (bit ``i``: ``n`` for 0, ``N`` for 1) or ``{Ci}`` (the
complement: ``N`` for 0, ``n`` for 1), chosen so the emitted template
computes the requested table.
"""
...
# Two-input tables: table -> (base string, slot indices, (slot0 mode, slot1
# mode)) where a mode is "X" (plain bit) or "C" (complemented bit).
...
# One-input tables: table -> (template with one {X0} or {C0} slot).
...
def a_painter_ant(truth_table: str) -> str:
    ...
    :class:`ValueError` (the discriminator's conditional read splits the
    ant's position and stable branches cannot re-converge).
    """
    ...
    raise ValueError(
        "the A Painter Ant boolean generator supports n <= 2: the "
        "discriminator's conditional read splits the ant's position and "
        "stable branches cannot re-converge",
    )
