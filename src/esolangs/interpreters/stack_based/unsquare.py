...
- an empty-stack pop, a swap with fewer than two elements, an ``o`` on an
  ...
  :class:`HaltError`;
- ``i`` raises :class:`EOFError` when input runs out, where the Rust
  reference spins and the Ruby one exits with status 3;
- blank input lines re-prompt without the Rust reference's stale-newline
  bug (it appends to the same buffer).
"""
