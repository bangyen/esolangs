...
0 or ``y``, ``e``/``f`` copy ``x`` into ``y`` or clear ``y``, ``g``/``h``
multiply ``x`` by ``y`` or square ``x``, and ``i`` prints ``x`` as the
Unicode character whose code point it is.  Any other character is ignored.
...
NUL instead, matching Lean's ``Char.ofNat``.  The values are unbounded
natural numbers and the output is that character's UTF-8 encoding.

The reference (``extra/lean/esolangs/Albabet.lean``) is total: every
character is a defined operation or a no-op, so there is no malformed
program (no :class:`ValueError`) and no invalid runtime operation (no
