...
character is ignored as a comment.  Cells hold nonnegative integers; the
tape is unbounded in both directions.  The wiki instead specifies a tape of
256 wrapping cells, which the reference implementation does not enforce.
...
- the reference's ``<`` bound check compares a cell address against a cell
  count, never fires, and lets the pointer move past cell 0 into memory
  that overlaps the program text (a self-modifying quirk); this
  interpreter instead models an unbounded tape of clean zero cells;
