...
int index(std::string prog, int ind, char start, char end) {
  int match = 1;
  char c;
...
int *move(std::vector<int> &ptr, std::vector<int> &prime, int dim) {
  while (dim + 1 > ptr.size()) {
    int n = prime.back() + 1;

    while (true) {
      bool p =
          std::all_of(prime.begin(), prime.end(), [n](int k) { return n % k; });
...
char *value(std::vector<int> &ptr, std::vector<int> &prime,
...
int main(int argc, char *argv[]) {
  ...
  auto val = [&]() { return value(ptr, prime, tape); };

  auto mov = [&]() { return move(ptr, prime, dim); };
  ...
  while (ind < prog.size()) {
    ...
    case ':':
      *val() = prog[ind++];
      break;
    case '=':
      *val() = stoi(prog.substr(ind), nullptr, 16);
