...
def test_reads_wiki_fixtures_as_pillow_did(
    ...
    r"""Each fixture decodes to the shape and ink count Pillow reported."""
...
def test_roundtrip_preserves_every_byte(shape: tuple[int, int]) -> None:
    r"""Writing then reading arbitrary greyscale rows is the identity."""
    height, width = shape
    rng = random.Random(0)
    original = [
...
def test_roundtrip_through_a_file(tmp_path: Path) -> None:
    r"""The file-level helpers agree with the in-memory pair."""
...
def _encode(
    ...
    r"""Build a PNG from already-filtered rows (each prefixed by its filter."""
...
def test_every_row_filter_decodes(filter_type: int) -> None:
    r"""All five spec filters reconstruct the same image."""
    want = [
...
def test_sub_byte_depths_unpack_and_scale(
    ...
    r"""Narrow greyscale depths expand to the full 0-255 range."""
    width = len(expected)
...
def test_palette_is_resolved_through_plte() -> None:
    r"""A palette image maps indices through PLTE, not straight to."""
...
def test_colour_types_reduce_to_grey_as_pillow_does(
    ...
    r"""Colour and alpha images reduce to the grey level Pillow produces."""
    blob = _encode([bytes([0, *pixel])], 1, 1, colour=colour)
    assert png.read_grey(blob) == [bytearray([expected])]


def test_multi_channel_filters_step_by_a_whole_pixel() -> None:
    r"""A colour row's Sub filter predicts from the pixel left, not the."""
...
def test_rejects_a_sixteen_bit_palette() -> None:
    r"""Palette indices are at most 8 bits, so 16-bit palette is malformed."""
...
def test_rejects_sub_byte_depth_on_a_colour_image() -> None:
    r"""Sub-byte samples are single-channel only, per the spec."""
...
def _adam7_encode(pixels: list[list[int]], width: int, height: int) -> bytes:
    r"""Encode 8-bit greyscale as an interlaced PNG, filter 0 throughout."""
    raw = bytearray()
...
def test_interlaced_reassembles_to_the_same_image(width: int, height: int) -> None:
    r"""An Adam7 image decodes to what the same pixels say non-interlaced."""
...
def test_sixteen_bit_scales_down_rather_than_clipping() -> None:
    r"""16-bit samples scale onto 0-255; they are not clipped there."""
...
def test_sixteen_bit_colour_reduces_through_luma() -> None:
    r"""A 16-bit RGB pixel scales per channel, then reduces like any colour."""
    row = bytearray([0])
...
def test_rejects_a_non_png() -> None:
    r"""Bytes that are not a PNG at all are refused up front."""
    with pytest.raises(ValueError, match="signature"):
        png.read_grey(b"not a png at all")


def test_a_jpeg_is_refused_with_a_usable_message(tmp_path: Path) -> None:
    r"""A JPEG names itself and the fix, rather than failing on the."""
    import extract
...
def test_rejects_an_unknown_interlace_method() -> None:
    r"""Only the spec's two interlace methods exist; anything else is."""
...
def test_rejects_an_unknown_colour_type_by_number() -> None:
    r"""A colour type outside the spec is refused rather than guessed at."""
...
def test_rejects_an_unknown_row_filter() -> None:
    r"""A filter byte outside 0-4 is corruption, not something to guess at."""
...
def test_rejects_ragged_rows_on_write() -> None:
    r"""Rows of differing lengths are not an image; the writer must say so."""
