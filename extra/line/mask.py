r"""Boolean image masks as one integer per row, with no third-party."""
...
class Mask:
    r"""A 2-D boolean image, one row bitmask per scanline."""

    __slots__ = ("height", "rows", "width")

    def __init__(self, height: int, width: int, rows: list[int] | None = None) -> None:
        r"""Create a ``height`` x ``width`` mask, blank unless ``rows`` is."""
        self.height = height
        self.width = width
    ...
    def shape(self) -> tuple[int, int]:
        r"""``(height, width)``, matching the ndarray attribute it replaces."""
    ...
    def __and__(self, other: Mask) -> Mask:
        r"""Pixelwise AND with a same-shaped mask."""
        self._check(other)
        return Mask(
            self.height,
            self.width,
    ...
    def __or__(self, other: Mask) -> Mask:
        r"""Pixelwise OR with a same-shaped mask."""
        self._check(other)
        return Mask(
            self.height,
            self.width,
    ...
    def __xor__(self, other: Mask) -> Mask:
        r"""Pixelwise XOR with a same-shaped mask."""
        self._check(other)
        return Mask(
            self.height,
            self.width,
    ...
    def bounds(self) -> tuple[int, int, int, int] | None:
        r"""Ink's bounding box as ``(top, left, bottom, right)``, or ``None``."""
        top = bottom = None
        left = self.width
        right = -1
        for y, row in enumerate(self.rows):
    ...
    def crop(self, top: int, left: int, height: int, width: int) -> Mask:
        r"""Cut out the ``height`` x ``width`` sub-mask cornered at ``(top,."""
        keep = (1 << width) - 1
    ...
    def subsample(self, step: int, off_y: int = 0, off_x: int = 0) -> Mask:
        r"""Take every ``step``-th pixel, starting from ``(off_y, off_x)``."""
        rows = []
    ...
    def erode(self) -> Mask:
        r"""Keep only ink whose whole 8-neighborhood is also ink."""
        full = (1 << self.width) - 1
        rows = []
    ...
    def from_rows(cls, rows: list[list[bool]] | list[str]) -> Mask:
        r"""Build from a list of rows, each a bool sequence or a '.#' string."""
        height = len(rows)
...
def from_grey(grey: list[bytearray], threshold: int = 128) -> Mask:
    r"""Threshold greyscale rows into an ink mask (ink = darker than."""
    height = len(grey)
