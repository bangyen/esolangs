...
reasoning for how each module works; this file records only open
questions, constraints a change could silently violate, and decisions
...
is the separate, narrower round-trip check for `extract()` alone.
`test_simulate.py`'s hand-built `Stroke` fixtures never exercise `render.py`'s
loop-drawing geometry (`_layout`/`_loop_return_legs`); only
`test_bf_to_line.py` does.
...
- `lattice.star`'s probe `length` is hardcoded to 15px while `lattice.UNIT`
  ...
  segment. A future change supporting smaller units should derive the
  default from `UNIT` (e.g. `UNIT - 5`, preserving today's 20 -> 15) rather
  than adjusting `_UNIT_TOLERANCE`, which is not the limiting factor.
  Nothing in this tree renders below 20 today.
...
- **`_STEM_LEN` (10 units) has ~1 unit of measured slack** (9 and 8 both
  ...
  `lattice.star`'s probe geometry apart. Slack here serves robustness, not
  waste.
...
level, not searched): treat "make the router smarter" as a dead end already
measured twice, not an untried option.
...
- **No non-PNG image formats.** JPEG, BMP, GIF and TIFF went with Pillow
  ...
  20 lines regardless of who decodes them. A future need for JPEG input
  should reconsider full progressive support as a deliberate scope
  expansion, not resurrect the partial decoder.
- **No step limit or cycle-hang detection in `simulate.run`.** Matches every
  ...
  already as a wrong-fit pattern).
