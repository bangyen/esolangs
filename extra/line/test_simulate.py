...
class TestBasicOps:
    r"""Opcode basics through a real render -> extract -> simulate."""

    def test_plus_repeats_merge_into_one_count(self, tmp_path: Path) -> None:
        r"""Three consecutive `+` render as one merged kink, still count=3."""
    ...
    def test_pointer_movement_and_increment_across_cells(self, tmp_path: Path) -> None:
        r"""`>` moves the pointer; `+` increments whichever cell it lands on."""
...
class TestRenderScale:
    ...
    def test_scale_multiplies_the_canvas_exactly(self, scale: int) -> None:
        r"""Pixel replication, so dimensions are an exact integer multiple."""
        base = render(chain("+", "+", "+"))
    ...
    def test_scale_is_recoverable_by_the_extractor(self, tmp_path: Path) -> None:
        r"""detect_scale reads back the exact factor rendered at."""
    ...
    def test_a_nonsense_scale_is_refused(self, bad: int) -> None:
        r"""Zero or negative would silently produce an empty or absurd canvas."""
        with pytest.raises(ValueError, match="at least 1"):
            render(chain("+"), scale=bad)


class TestConditionalBranch:
    r"""Confirms the zero/nonzero swap documented in run()'s docstring."""

    @pytest.fixture
    def tree(self, tmp_path: Path) -> Stroke:
        r"""Render a `?` with distinct, easily-told-apart zero/nonzero arms."""
...
class TestWikiFixtures:
    r"""Both wiki fixtures compute correct results across several real."""
    ...
    def test_addition(self, a: int, b: int, expected: int) -> None:
        r"""addition.png computes a + b for several input pairs, including 0."""
        io, outputs = _io([a, b])
    ...
    def test_multiplication(self, a: int, b: int, expected: int) -> None:
        r"""multiplication.png computes a * b for several input pairs."""
        io, outputs = _io([a, b])
...
def _build_decrement_loop() -> Stroke:
    r"""Build a synthetic ``+++`` seed forking into a decrementing loop or."""
    unit = 20
    heading = 0
...
def _build_growing_loop() -> Stroke:
    r"""Like _build_decrement_loop, but the loop arm increments -- never."""
    unit = 20
    heading = 0
    turn_plus = (heading + 1) % 8
...
class TestSyntheticLoopMechanism:
    r"""The loop-back mechanism itself, independent of any real fixture's."""

    def test_decrementing_loop_terminates_at_zero(self) -> None:
        r"""A synthetic loop decrementing its seed cell halts exactly at 0."""
        io, _ = _io([])
    ...
    def test_non_halting_loop_hangs_rather_than_returning(self) -> None:
        r"""A loop with no reachable dead end hangs, per run()'s own docstring."""
    ...
    def test_loop_back_does_not_match_an_unrelated_sibling_branch(self) -> None:
        r"""Regression: a bare vertex match once matched an untaken sibling."""
        io, _ = _io([])
        tape = run(_build_decrement_loop(), io=io)
        assert tape == {0: 0}
